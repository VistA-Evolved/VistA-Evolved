...
export function validateAgainstTemplate(
  ...
  if (
    mshVersion &&
    template.hl7Version &&
    !mshVersion.startsWith(template.hl7Version.split('.').slice(0, 2).join('.'))
  ) {
    issues.push({
      severity: 'warning',
...
function validateFieldConstraint(
  ...
  if (
    ...
    fieldValue.length > constraint.maxLength
  ) {
    issues.push({
      severity: 'warning',
      category: 'field',
...
export function getConformanceSummary(template: MessageTemplate): {
  ...
  const requiredSegments = template.segments.filter((s) => s.usage === 'R').map((s) => s.segmentId);

  const totalFieldConstraints = template.segments.reduce((sum, s) => sum + s.fields.length, 0);
