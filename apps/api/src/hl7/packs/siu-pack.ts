...
function buildSiuMessage(
  ...
  const location = schedule.location
    ? `AIL${sep}1${sep}${sep}${schedule.location}`
    : "";
...
export const siuPack: MessagePack = {
  ...
  description: "Scheduling messages: new appointment (S12), reschedule (S13), modify (S14), cancel (S15)",
  ...
  getRouteTemplate: () => ({
    ...
    transforms: [],
    destination: { type: "dead-letter" as const, id: "siu-dlq", name: "SIU Dead Letter", target: "" },
  }),
};
