    ...
    if (domain === 'diagnosis') {
      ...
      if (code && !/^[A-Z]\d{2}(\.\d{1,4})?$/.test(code)) {
        results.push({
          ...
          severity: 'error',
          code: 'US-DX-001',
          message: 'Diagnosis code must be ICD-10-CM format (e.g. J06.9)',
          country: cc,
          framework: 'HIPAA',
        });
      }
    ...
    if (domain === 'procedure') {
      ...
      if (cpt && !/^\d{5}$/.test(cpt)) {
        results.push({
          ...
          severity: 'error',
          code: 'US-PX-001',
          message: 'Procedure code must be 5-digit CPT format',
          country: cc,
          framework: 'HIPAA',
        });
      }
    ...
    if (domain === 'medication') {
      ...
      if (ndc && !/^\d{4,5}-\d{3,4}-\d{1,2}$/.test(ndc) && !/^\d{10,11}$/.test(ndc)) {
        results.push({
          ...
          severity: 'warning',
          code: 'US-RX-001',
          message: 'Medication code should be NDC format (NNNNN-NNNN-NN or 11-digit)',
          country: cc,
        });
      }
    }
    ...
    if (domain === 'billing') {
      ...
      if (!npi) {
        results.push({
          ...
          severity: 'error',
          code: 'US-BL-001',
          message: 'NPI is required for US billing claims',
          country: cc,
          framework: 'HIPAA',
        });
      ...
      if (taxId && !/^\d{2}-?\d{7}$/.test(taxId)) {
        results.push({
          ...
          severity: 'warning',
          code: 'US-BL-003',
          message: 'Tax ID should be EIN format (NN-NNNNNNN)',
          country: cc,
        });
      }
    }
    ...
    if (domain === 'identifier') {
      ...
      if (ssn && !/^\d{3}-?\d{2}-?\d{4}$/.test(ssn)) {
        results.push({
          ...
          severity: 'error',
          code: 'US-ID-001',
          message: 'SSN must be 9 digits (NNN-NN-NNNN)',
          country: cc,
          framework: 'HIPAA',
        });
      }
    ...
    if (domain === 'demographics') {
      if (!record.dateOfBirth && !record.dob) {
        results.push({
          ...
          severity: 'warning',
          code: 'US-DM-001',
          message: 'Date of birth recommended for patient identification',
          country: cc,
        });
      }
    }
    ...
    if (domain === 'diagnosis') {
      ...
      if (code && !/^[A-Z]\d{2}(\.\d{1,4})?$/.test(code)) {
        results.push({
          ...
          severity: 'error',
          code: 'PH-DX-001',
          message: 'Diagnosis code must be ICD-10-WHO format',
          country: cc,
          framework: 'DPA_PH',
        });
      }
    ...
    if (domain === 'billing') {
      ...
      if (!pin) {
        results.push({
          ...
          severity: 'error',
          code: 'PH-BL-001',
          message: 'PhilHealth PIN is required for PH billing claims',
          country: cc,
          framework: 'DPA_PH',
        });
        ...
        results.push({
          ...
          severity: 'warning',
          code: 'PH-BL-002',
          message: 'PhilHealth PIN format: NN-NNNNNNNNN-N',
          country: cc,
          framework: 'DPA_PH',
        });
      }
      ...
      if (!record.facilityCode && !record.phicFacilityCode) {
        results.push({
          ...
          severity: 'error',
          code: 'PH-BL-003',
          message: 'PHIC facility code required for claims',
          country: cc,
          framework: 'DPA_PH',
        });
      }
    ...
    if (domain === 'consent') {
      ...
      if (!record.consentGiven && record.consentGiven !== true) {
        results.push({
          ...
          severity: 'error',
          code: 'PH-CN-001',
          message: 'Explicit data processing consent required under DPA 2012',
          country: cc,
          framework: 'DPA_PH',
        });
      }
    ...
    if (domain === 'identifier') {
      ...
      if (philId && !/^\d{4}-\d{4}-\d{4}$/.test(philId)) {
        results.push({
          ...
          severity: 'info',
          code: 'PH-ID-001',
          message: 'Philippine national ID format: NNNN-NNNN-NNNN',
          country: cc,
        });
      }
    }
    ...
    if (domain === 'demographics') {
      if (!record.dateOfBirth && !record.dob) {
        results.push({
          ...
          severity: 'error',
          code: 'PH-DM-001',
          message: 'Date of birth required for PhilHealth claims',
          country: cc,
          framework: 'DPA_PH',
        });
      }
    ...
    if (domain === 'billing') {
      ...
      if (!nhiaId) {
        results.push({
          ...
          severity: 'error',
          code: 'GH-BL-001',
          message: 'NHIA membership number required for GH billing',
          country: cc,
          framework: 'DPA_GH',
        });
      }
      ...
      if (record.claimType === 'inpatient' && !record.gDrgCode) {
        results.push({
          ...
          severity: 'warning',
          code: 'GH-BL-002',
          message: 'G-DRG code recommended for inpatient claims',
          country: cc,
        });
      }
    }
    ...
    if (domain === 'identifier') {
      ...
      if (ghCardNumber && !/^GHA-\d{9}-\d$/.test(ghCardNumber)) {
        results.push({
          ...
          severity: 'info',
          code: 'GH-ID-001',
          message: 'Ghana Card format: GHA-NNNNNNNNN-N',
          country: cc,
        });
      }
    }
