...
export async function globalRoutingRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/platform/routing/ingresses", async (request: FastifyRequest, reply: FastifyReply) => {
    const body = (request.body as any) || {};
    if (!body.region || !body.clusterId || !body.endpoint) {
      return reply.code(400).send({ ok: false, error: "region, clusterId, and endpoint are required" });
    }
    try {
  ...
  server.get("/platform/routing/ingresses/:id", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/platform/routing/ingresses/:id/health", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const ingress = getIngress(id);
    if (!ingress) return reply.code(404).send({ ok: false, error: "Ingress not found" });
    const body = (request.body as any) || {};
  ...
  server.post("/platform/routing/dns", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!body.tenantId || !body.targetValue || !body.region) {
      return reply.code(400).send({ ok: false, error: "tenantId, targetValue, and region are required" });
    }
    try {
  ...
  server.get("/platform/routing/dns/:tenantId", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as any;
    const record = getDnsRecord(tenantId);
    if (!record) return reply.code(404).send({ ok: false, error: "DNS record not found for tenant" });
    return { ok: true, record };
  });

  server.post("/platform/routing/dns/:tenantId/update", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as any;
    const body = (request.body as any) || {};
    if (!body.targetValue) {
  ...
  server.post("/platform/routing/failover", async (request: FastifyRequest, reply: FastifyReply) => {
    const body = (request.body as any) || {};
    if (!body.tenantId || !body.toRegion || !body.reason) {
      return reply.code(400).send({ ok: false, error: "tenantId, toRegion, and reason are required" });
    }
    try {
  ...
  server.post("/platform/routing/failover/:id/complete", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const body = (request.body as any) || {};
    if (!body.toClusterId || !body.newDnsTarget) {
      return reply.code(400).send({ ok: false, error: "toClusterId and newDnsTarget are required" });
    }
    try {
  ...
  server.get("/platform/routing/failover/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
