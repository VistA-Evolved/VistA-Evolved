...
export default async function vistaRcmRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/vista/rcm/icd-search', async (request) => {
    ...
    if (!text || text.length < 2) {
      return { ok: false, error: 'Query text must be at least 2 characters', hint: 'Use ?text=diabetes' };
    }
  ...
  server.get('/vista/rcm/charges', async (request) => {
    ...
    return integrationPendingResponse('/vista/rcm/charges', dfn, {
      ...
      migrationPath: 'When IB billing is configured, read ^IB(350) actions for patient via IB RPCs or FileMan. IB ACTION entries are created by CPRS encounter checkout (IBD RPCs) and pharmacy billing (IBARXM RPCs).',
      sandboxNote: 'WorldVistA Docker sandbox has 0 entries in ^IB(350). 122 IB Action Type definitions exist in ^IBE(350.1).',
    });
  });
  ...
  server.get('/vista/rcm/claims-status', async (request) => {
    ...
    return integrationPendingResponse('/vista/rcm/claims-status', dfn, {
      ...
      migrationPath: 'Claims are generated by IB GENERATE CLAIM workflow after IB ACTION entries exist. Read ^DGCR(399) via FileMan or IBCF RPCs. Requires IB Actions (File 350) to be populated first.',
      sandboxNote: 'WorldVistA Docker sandbox has 0 entries in ^DGCR(399). Claims generation requires IB configuration + IB Action entries.',
    });
  });
  ...
  server.get('/vista/rcm/ar-status', async (request) => {
    ...
    return integrationPendingResponse('/vista/rcm/ar-status', dfn, {
      ...
      migrationPath: 'AR transactions are created when claims are billed and payments posted. Query via PRCA RPCs (PRCAFN, PRCASER routines present in sandbox). Requires Claims (File 399) pipeline to be active.',
      sandboxNote: 'WorldVistA Docker sandbox has 0 entries in ^PRCA(430). 2 entries in ^PRCA(433) (seed payment data).',
    });
  });
