...
export async function tenantSecurityRoutes(app: FastifyInstance): Promise<void> {
   ...
   */
  app.get("/tenant-security/policies/:tenantId", async (request: FastifyRequest, reply: FastifyReply) => {
   ...
   */
  app.put("/tenant-security/policies/:tenantId", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as { tenantId: string };
   ...
   */
  app.delete("/tenant-security/policies/:tenantId", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as { tenantId: string };
    const session = (request as any).session;
