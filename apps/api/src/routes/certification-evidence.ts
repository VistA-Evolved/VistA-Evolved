...
export default async function certificationEvidenceRoutes(server: FastifyInstance) {
  ...
  server.get("/admin/certification/status", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/admin/certification/generate", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;

    const body = (request.body as any) || {};
    const buildId = body.buildId || `cert-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}`;
  ...
  // GET /admin/certification/bundles — list generated bundles
  server.get("/admin/certification/bundles", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
      ...
      if (existsSync(bundlesDir)) {
        ...
        for (const dir of dirs) {
          ...
          bundles.push({
            ...
            manifestHash: manifest ? createHash("sha256").update(JSON.stringify(manifest)).digest("hex").slice(0, 16) : "none",
          });
        }
      }
    } catch { /* ignore */ }
  ...
  server.get("/admin/certification/bundle/:buildId", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
