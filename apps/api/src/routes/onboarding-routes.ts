...
export default async function onboardingRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/admin/onboarding/:id/advance", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    ...
    if (!stepResult.ok) {
      return reply.code(400).send({ ok: false, error: stepResult.error, step: current.currentStep });
    }
  ...
  server.patch("/admin/onboarding/:id/step", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    ...
    if (!body.step || !STEP_ORDER.includes(body.step)) {
      return reply.code(400).send({ ok: false, error: `Invalid step. Must be one of: ${STEP_ORDER.join(", ")}` });
    }
  ...
  server.post("/admin/onboarding/:id/probe", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
