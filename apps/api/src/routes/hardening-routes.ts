...
async function verifyIamAudit() {
  try {
    ...
    let fileResult: { valid: boolean; totalEntries: number; error?: string } = {
      valid: false,
      totalEntries: 0,
      error: 'not checked',
    };
    try {
      ...
      fileResult = { valid: false, totalEntries: 0, error: 'file sink not available' };
    }
...
export default async function hardeningRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/hardening/audit-verify', async (_request, _reply) => {
    ...
    const allValid =
      iam.memory.valid !== false && imaging.memory.valid !== false && rcm.memory.valid !== false;
  ...
  server.get('/hardening/rc-checklist', async (_request, _reply) => {
    ...
    checks.push({
      ...
      status: 'pass',
      detail: 'CSP + HSTS + X-Frame-Options + Referrer-Policy + Permissions-Policy',
    });
    ...
    checks.push({
      name: 'rate_limiter',
      status: 'pass',
      detail: `general=${Number(process.env.RATE_LIMIT_GENERAL_MAX) || 200}/min`,
    });
    ...
    checks.push({
      ...
      status: 'pass',
      detail: 'Session-bound synchronizer token (Phase 132)',
    });
    ...
    checks.push({
      ...
      detail: secureCookies
        ? 'Secure flag enabled'
        : 'No production NODE_ENV or runtime mode (secure flag off)',
    });
