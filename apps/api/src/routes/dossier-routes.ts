...
export async function dossierRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get("/rcm/dossiers/by-payer/:payerId", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/rcm/dossiers", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { payerId, tenantId = "default", enrichmentJson, contactJson, timingJson, complianceJson } = body;
    ...
    if (existing) return reply.code(409).send({ ok: false, error: "Dossier already exists", dossierId: existing.id });
  ...
  server.patch("/rcm/dossiers/:dossierId/tasks/:taskId", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post("/rcm/dossiers/:dossierId/tasks/:taskId/complete", async (request: FastifyRequest, reply: FastifyReply) => {
    const { dossierId, taskId } = request.params as { dossierId: string; taskId: string };
    const body = (request.body as any) || {};
    ...
    const updated = await completeOnboardingTask(taskId, body.reason || "Completed via API", body.actor || "system");
    const score = await refreshCompletenessScore(dossierId);
    ...
    if (score === 100 && dossier && dossier.status === "draft") {
      await updateDossier(dossierId, { status: "active" }, "All onboarding tasks completed", body.actor);
    }
  ...
  server.post("/rcm/dossiers/:id/seed-tasks", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
    ...
    const tasks = await seedOnboardingTasks(id, mode, dossier.tenantId, dossier.payerId, body.actor);
