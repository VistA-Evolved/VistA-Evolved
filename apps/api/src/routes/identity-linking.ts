...
function startLinkRequestCleanup(): void {
  ...
  cleanupTimer = setInterval(() => {
    ...
    if (linkRequests.size > 1000) {
      const oldest = [...linkRequests.entries()].sort(
...
export default async function identityLinkingRoutes(server: FastifyInstance) {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!validRelationships.includes(relationship)) {
        return reply.code(400).send({
          ok: false,
          error: `Invalid relationship. Valid: ${validRelationships.join(', ')}`,
        });
      }
      ...
      const existing = Array.from(linkRequests.values()).find(
        (r) =>
          r.requesterId === session.duz && r.patientDfn === patientDfn && r.status === 'pending'
      );
      if (existing) {
        return reply
          .code(409)
          .send({ ok: false, error: 'Pending request already exists', requestId: existing.id });
      }
      ...
      immutableAudit(
        ...
        { sub: session.duz },
        {
          ...
          detail: {
            ...
            relationship,
            patientDfnHash: hashSensitive(patientDfn),
          },
        }
      );
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (linkReq.tenantId !== session.tenantId)
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (linkReq.status !== 'pending') {
        return reply
          .code(409)
          .send({ ok: false, error: `Request is ${linkReq.status}, not pending` });
      }
  ...
  server.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (link.tenantId !== session.tenantId)
        return reply.code(403).send({ ok: false, error: 'Forbidden' });
  ...
  server.get('/admin/identity/links', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    let links = Array.from(identityLinks.values()).filter((l) => l.tenantId === session.tenantId);
