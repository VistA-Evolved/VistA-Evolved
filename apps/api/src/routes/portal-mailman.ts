...
export function initPortalMailman(
  lookup: (request: FastifyRequest) => PortalSessionData | null
) {
...
function portalAuditActor(session: PortalSessionData): { sub: string; name: string; roles: string[] } {
  return {
...
export default async function portalMailmanRoutes(server: FastifyInstance) {
  ...
  server.post("/portal/mailman/send", async (request, reply) => {
    ...
    try {
      const { createDraft, sendMessage: sendDraft } = await import("../services/portal-messaging.js");
