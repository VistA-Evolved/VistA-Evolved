...
const VALID_STATUSES: ClusterStatus[] = ["active", "draining", "standby", "offline", "decommissioned"];
const VALID_PLAN_TIERS: PlanTier[] = ["free", "starter", "professional", "enterprise"];
const VALID_REASONS: PlacementReason[] = ["initial", "migration", "failover", "data_residency", "manual"];
...
export async function multiClusterRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/platform/clusters", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { name, region, regionTier, kubeContextRef, pgConnectionRef, vistaPlacementMode, maxTenants, metadata } = body;
    ...
    try {
      const cluster = registerCluster(
        { name, region, regionTier, kubeContextRef, pgConnectionRef, vistaPlacementMode, maxTenants, metadata },
        getActor(request),
      );
  ...
  server.get("/platform/clusters", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (status && !VALID_STATUSES.includes(status)) {
      return reply.code(400).send({ ok: false, error: `Invalid status. Valid: ${VALID_STATUSES.join(", ")}` });
    }
  ...
  /** GET /platform/clusters/summary — registry summary */
  server.get("/platform/clusters/summary", async (_request: FastifyRequest, _reply: FastifyReply) => {
    return { ok: true, ...getRegistrySummary() };
  });

  /** GET /platform/clusters/health — all health snapshots */
  server.get("/platform/clusters/health", async (_request: FastifyRequest, _reply: FastifyReply) => {
  ...
  /** POST /platform/clusters/:id/status — update cluster status */
  server.post("/platform/clusters/:id/status", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const body = (request.body as any) || {};
    const { status } = body;
    if (!status || !VALID_STATUSES.includes(status)) {
  ...
  /** POST /platform/clusters/:id/metadata — update cluster metadata */
  server.post("/platform/clusters/:id/metadata", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const body = (request.body as any) || {};
    const { metadata } = body;
  ...
  /** POST /platform/clusters/:id/health — record health snapshot */
  server.post("/platform/clusters/:id/health", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const cluster = getCluster(id);
    if (!cluster) return reply.code(404).send({ ok: false, error: "Cluster not found" });
  ...
  server.get("/platform/clusters/:id/health", async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as any;
    const snapshot = getHealthSnapshot(id);
    if (!snapshot) return reply.code(404).send({ ok: false, error: "No health data for cluster" });
  ...
  server.post("/platform/tenants/:tenantId/place", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as any;
    const body = (request.body as any) || {};

    if (body.planTier && !VALID_PLAN_TIERS.includes(body.planTier)) {
      return reply.code(400).send({ ok: false, error: `Invalid planTier. Valid: ${VALID_PLAN_TIERS.join(", ")}` });
    }
    if (body.reason && !VALID_REASONS.includes(body.reason)) {
      return reply.code(400).send({ ok: false, error: `Invalid reason. Valid: ${VALID_REASONS.join(", ")}` });
    }
  ...
  server.get("/platform/tenants/:tenantId/placement", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as any;
    const placement = getTenantPlacement(tenantId);
    if (!placement) return reply.code(404).send({ ok: false, error: "No active placement for tenant" });
    const cluster = getCluster(placement.clusterId);
    return { ok: true, placement, cluster: cluster || null };
  });

  /** DELETE /platform/tenants/:tenantId/placement — deactivate placement */
  server.delete("/platform/tenants/:tenantId/placement", async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = request.params as any;
  ...
  /** POST /platform/tenants/simulate-placement — dry-run placement */
  server.post("/platform/tenants/simulate-placement", async (request: FastifyRequest, reply: FastifyReply) => {
    const body = (request.body as any) || {};
    if (!body.tenantId) {
