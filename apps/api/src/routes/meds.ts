...
export default async function medsRoutes(server: FastifyInstance): Promise<void> {
  ...
  /** ORWPCE SAVE -- Save PCE encounter / sign meds (write) */
  ...
  server.post('/vista/meds/sign', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { dfn, noteIen, encounterStr, pceData } = parsed.data;
    const rpcParams = [dfn, noteIen, encounterStr, ...(pceData || [])];

    try {
      const result = await safeCallRpc('ORWPCE SAVE', rpcParams, { idempotent: false });
      return reply.send({
        ok: true,
        source: 'vista',
        data: result,
      ...
      log.warn('ORWPCE SAVE failed', { err: err?.message || String(err) });
      return reply.code(502).send({
        ok: false,
        source: 'vista',
        error: `ORWPCE SAVE failed: ${err?.message || 'RPC error'}`,
        rpcUsed: ['ORWPCE SAVE'],
      });
    }
  });
