...
async function fetchNotes(dfn: string): Promise<TimelineEvent[]> {
  try {
    ...
    return lines.filter((l: string) => l.trim() && !l.startsWith("^")).map((line: string) => {
      const parts = line.split("^");
      return {
...
async function fetchLabs(dfn: string): Promise<TimelineEvent[]> {
  try {
    ...
    return lines.filter((l: string) => l.trim()).slice(0, 50).map((line: string, i: number) => {
      const parts = line.split("^");
      return {
...
export default async function longitudinalRoutes(server: FastifyInstance) {
  ...
  server.get("/vista/longitudinal/timeline", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const allEvents = [
        ...allergies, ...problems, ...vitals, ...notes,
        ...meds, ...labs, ...consults, ...surgery,
      ];
      ...
      return {
        ...
        events: allEvents,
        rpcsUsed: [
          "ORQQAL LIST", "ORQQPL PROBLEM LIST", "ORQQVI VITALS",
          "TIU DOCUMENTS BY CONTEXT", "ORWPS ACTIVE", "ORWLRR INTERIM",
          "ORQQCN LIST", "ORWSR LIST",
        ],
  ...
  server.get("/vista/longitudinal/summary", async (request: FastifyRequest, reply: FastifyReply) => {
    if (!requireSession(request, reply)) return;
    const { dfn } = request.query as { dfn?: string };
    if (!dfn) return reply.code(400).send({ ok: false, error: "dfn query param required" });
    ...
    try {
      ...
      return {
        ...
        totalEvents: allergies.length + problems.length + vitals.length +
          notes.length + meds.length + labs.length + consults.length + surgery.length,
        generatedAt: new Date().toISOString(),
      };
    } catch (err: any) {
  ...
  server.get("/vista/longitudinal/meds-summary", async (request: FastifyRequest, reply: FastifyReply) => {
    if (!requireSession(request, reply)) return;
    const { dfn } = request.query as { dfn?: string };
    if (!dfn) return reply.code(400).send({ ok: false, error: "dfn query param required" });
    ...
    try {
      ...
      const historical = meds.filter((m) => m.detail?.toLowerCase()?.includes("expired") || m.detail?.toLowerCase()?.includes("discontinued"));
      const other = meds.filter((m) =>
        !active.includes(m) && !historical.includes(m)
      );
