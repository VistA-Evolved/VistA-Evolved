...
export default async function writebackRoutes(server: FastifyInstance) {
  // GET /scheduling/writeback/policy — current writeback policy
  server.get("/scheduling/writeback/policy", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  // GET /scheduling/writeback/entries — tracked writeback entries
  server.get("/scheduling/writeback/entries", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
  ...
  server.post("/scheduling/writeback/verify/:ref", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    ...
    if (!patientDfn) {
      return reply.code(400).send({ ok: false, error: "patientDfn required (body or query param 'dfn')" });
    }
  ...
  server.get("/scheduling/writeback/readiness", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
