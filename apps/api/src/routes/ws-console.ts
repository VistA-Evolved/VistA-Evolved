...
function auditWs(session: SessionData, action: string, rpcName?: string, result?: string): void {
  ...
  const outcome =
    action === 'DENIED' || action === 'BLOCKED_RPC' || action === 'RPC_ERROR'
      ? ('denied' as const)
      : ('success' as const);
  centralAudit(
    mapped,
  ...
  server.get('/ws/console', { websocket: true }, async (socket, request) => {
    ...
    if (!allowedRoles.includes(session.role)) {
      ...
      socket.send(
        JSON.stringify({
          type: 'error',
          message: `Console access requires one of: ${allowedRoles.join(', ')}. Your role: ${session.role}`,
          ts: new Date().toISOString(),
        })
      );
    ...
    socket.on('message', async (raw: Buffer | string) => {
      ...
      try {
        ...
        socket.send(
          JSON.stringify({ type: 'error', message: 'Invalid JSON', ts: new Date().toISOString() })
        );
        return;
      }
      ...
      if (msg.type === 'rpc') {
        ...
        if (blocked.includes(rpcName.toUpperCase())) {
          ...
          socket.send(
            JSON.stringify({
              type: 'error',
              message: `RPC "${rpcName}" is blocked for security`,
              ts: new Date().toISOString(),
            })
          );
          return;
        ...
        if (!path || typeof path !== 'string') {
          socket.send(
            JSON.stringify({ type: 'error', message: 'Missing path', ts: new Date().toISOString() })
          );
          return;
        }
        ...
        try {
          ...
          socket.send(
            JSON.stringify({ type: 'error', message: safeErr(err), ts: new Date().toISOString() })
          );
        }
