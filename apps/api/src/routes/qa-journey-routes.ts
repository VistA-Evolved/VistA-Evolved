...
export default async function qaJourneyRoutes(server: FastifyInstance) {
  ...
  server.post("/admin/qa/journeys/:journeyId/run", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
  ...
  server.delete("/admin/qa/journeys/results", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    clearJourneyResults();
  ...
  server.get("/admin/qa/journeys/:journeyId/rpc-trace", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    const { journeyId } = (request as any).params;
    const journey = ALL_JOURNEYS.find((j) => j.id === journeyId);
