...
const DOCUMENT_TYPES = [
  {
    ...
    description: "Complete health record summary including allergies, medications, problems, and vitals",
    source: "vista",
  },
  {
...
async function fetchVistaHealthSummary(dfn: string): Promise<string> {
  try {
    ...
    try {
      ...
      if (lines.length === 0 || (lines.length === 1 && !lines[0]?.trim())) {
        ...
        for (const l of lines) {
          const p = l.split("^");
          if (p[1]?.trim()) sections.push(`- ${p[1].trim()}${p[2]?.trim() ? ` (${p[2].trim()})` : ""}`);
        }
      }
    } catch { sections.push("=== ALLERGIES ===\nUnable to retrieve."); }
    ...
    } catch { sections.push("=== ACTIVE MEDICATIONS ===\nUnable to retrieve."); }
    ...
    try {
      ...
      if (lines.length === 0) {
        ...
        for (const l of lines) {
          const p = l.split("^");
          if (p[0]?.trim()) sections.push(`- ${p[1]?.trim() || p[0].trim()} (${p[2]?.trim() || ""})`);
        }
      }
    } catch { sections.push("=== IMMUNIZATIONS ===\nUnable to retrieve."); }
...
export default async function portalDocumentsRoutes(server: FastifyInstance) {
  ...
  server.get("/portal/documents", async (request, reply) => {
    ...
    immutableAudit("portal.document.list", "success",
  ...
  server.post("/portal/documents/generate", async (request, reply) => {
    ...
    immutableAudit("portal.document.generate", "success",
      { sub: session.patientDfn, name: "portal-patient", roles: ["patient"] },
  ...
  server.get("/portal/documents/download/:token", async (request, reply) => {
    ...
    immutableAudit("portal.document.download", "success",
  ...
  server.get("/portal/consents", async (request, reply) => {
    ...
    immutableAudit("portal.consent.view", "success",
      { sub: session.patientDfn, name: "portal-patient", roles: ["patient"] },
  ...
  server.post("/portal/consents", async (request, reply) => {
    ...
    if (isPgConfigured()) {
      try {
        ...
        immutableAudit("portal.consent.update", "success",
          { sub: session.patientDfn, name: "portal-patient", roles: ["patient"] },
