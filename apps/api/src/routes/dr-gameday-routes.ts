...
export default async function drGamedayRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/platform/dr/scenarios", async (request, reply) => {
    ...
    if (!b.name || !b.description || !Array.isArray(b.drillTypes) || !Array.isArray(b.targetRegions)) {
      return reply.code(400).send({ ok: false, error: "name, description, drillTypes[], targetRegions[] required" });
    }
