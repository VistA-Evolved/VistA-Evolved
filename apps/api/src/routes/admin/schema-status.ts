...
} from "../../platform/pg/pg-migrate.js";

export default async function schemaStatusRoutes(
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        dbVersion = dbMigrations.length > 0
          ? Math.max(...dbMigrations.map(m => m.version))
          : 0;
      } catch {
      ...
      const appliedVersions = new Map(
        dbMigrations.map(m => [m.version, m])
      );
