...
export function initDraftStoreRepo(repo: typeof draftDbRepo): void {
  draftDbRepo = repo;
}
let draftSeq = 0;
...
export default async function writeBackRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/vista/orders/sign', async (request, reply) => {
    ...
    if (check.available) {
      ...
      try {
        ...
        auditWrite({
          ...
          status: 'success',
          rpc: 'ORWDX SAVE',
          detail: `Order ${orderId} signed via RPC`,
        });
        return {
          ok: true,
          mode: 'real',
    ...
    return {
      ...
      draftId: draft.id,
      orderId,
      status: 'signed-locally',
      syncPending: true,
      message: 'Order signed locally. VistA sync will occur when RPC becomes available.',
    };
  });
  ...
  server.post('/vista/labs/ack', async (request, reply) => {
    ...
    if (check.available) {
      try {
        ...
        auditWrite({
          ...
          status: 'success',
          rpc: 'ORWLRR ACK',
          detail: `${labIds.length} results acknowledged`,
        });
        return {
          ok: true,
          mode: 'real',
    ...
    auditWrite({
      ...
      user: acknowledgedBy || 'unknown',
      status: 'draft-stored',
      detail: `${labIds.length} acks stored server-side`,
    });
    ...
    return {
      ...
      syncPending: true,
      message: 'Lab acknowledgements stored server-side. VistA sync pending.',
    };
  });
  ...
  server.post('/vista/consults/create', async (request, reply) => {
    ...
    if (check.available) {
      try {
        ...
        await connect();
        const duz = getDuz();
        const resp = await callRpc('ORQQCN2 MED RESULTS', [
          String(dfn),
          duz,
          service,
          urgency || 'Routine',
    ...
    return {
      ...
      status: 'draft-stored',
      syncPending: true,
      message: 'Consult request stored as draft. VistA sync pending.',
    };
  });
  ...
  server.post('/vista/surgery/create', async (request, reply) => {
    ...
    auditWrite({
      ...
      status: 'draft-stored',
      detail: 'No write-back RPC available for surgery',
    });
    ...
    return {
      ...
      status: 'draft-stored',
      syncPending: true,
      message: 'Surgery record stored as draft. No standard write-back RPC exists.',
    };
  });
  ...
  server.post('/vista/problems/save', async (request, reply) => {
    ...
    return {
      ...
      status: 'sync-pending',
      syncPending: true,
      message: `Problem saved as server-side draft. ${rpcName} sync pending.`,
    };
  });
  ...
  server.get('/vista/write-audit', async (request, reply) => {
    ...
    return {
      ...
      entries,
      note: 'Write-back events also available at /audit/events',
    };
  });
}
