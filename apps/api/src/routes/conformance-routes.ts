...
export async function conformanceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/conformance/run', async (request) => {
    ...
    const targets = filterPack ? packsList.filter((p) => p.countryCode === filterPack) : packsList;
    ...
    for (const packMeta of targets) {
      ...
      gates.push(
        gateCheck(
          ...
          !!profile,
          profile
            ? `${profile.framework} profile with ${profile.policies.length} policies`
            : 'No matching profile'
        )
      );
  ...
  app.get('/conformance/gates', async () => {
    return {
      ok: true,
      gates: [
        { id: 'pack-load', phase: '491', description: 'Country pack loads and parses' },
        { id: 'regulatory-framework', phase: '492', description: 'Regulatory framework is set' },
        { id: 'consent-required', phase: '494', description: 'Consent required flag is boolean' },
        { id: 'consent-granularity', phase: '494', description: 'Consent granularity is set' },
        {
          id: 'consent-profile',
          phase: '494',
          description: 'Consent profile exists for framework',
        },
        { id: 'residency-region', phase: '495', description: 'Data residency region is valid' },
        { id: 'residency-cross-border', phase: '495', description: 'Cross-border flag is boolean' },
        { id: 'retention-min-years', phase: '496', description: 'Retention minimum > 0' },
        { id: 'dsar-erasure', phase: '496', description: 'Right to erasure is boolean' },
        { id: 'dsar-portability', phase: '496', description: 'Data portability is boolean' },
        { id: 'locales-defined', phase: '497', description: 'Supported locales are defined' },
        { id: 'terminology-dx', phase: '497', description: 'Diagnosis code system is set' },
        { id: 'terminology-proc', phase: '497', description: 'Procedure code system is set' },
      ],
    };
  });
}
