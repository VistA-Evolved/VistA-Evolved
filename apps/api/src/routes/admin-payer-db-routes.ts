...
import {
  ...
  exportAuditEntries,
  getRetentionPolicy,
} from '../platform/pg/audit-integrity.js';
import {
  ingestJsonSnapshot,
  ingestPdfEvidence,
  computeSnapshotDiff,
  promoteSnapshot,
} from '../platform/payers/evidence-ingest.js';
...
const adminPayerDbRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
  ...
  async function ensureDb(): Promise<void> {
    ...
    if (!pgInitPromise) {
      pgInitPromise = (async () => {
        ...
        if (pgResult.ok) {
          server.log.info(
            `Platform PG initialized (migrations: ${pgResult.migrations?.applied ?? 0} applied)`
          );
        } else {
  ...
  server.post('/admin/payer-db/evidence/upload-pdf', async (request, reply) => {
    ...
    if (!body.base64 && !body.filename) {
      return reply
        .code(400)
        .send({ ok: false, error: "Provide 'base64' PDF content and 'filename'" });
    }
  ...
  server.put('/admin/payer-db/payers/:id/capabilities', async (request, reply) => {
    ...
    if (!body.reason) {
      return reply
        .code(400)
        .send({ ok: false, error: 'reason is required for capability changes' });
    }
