...
export async function dbUpdateFeatureFlags(
  ...
  try {
    ...
    if (result.rows.length === 0) return null;
    const merged = typeof result.rows[0].feature_flags === "string"
...
export async function dbUpdateUiDefaults(
  ...
  try {
    ...
    if (result.rows.length === 0) return null;
    const merged = typeof result.rows[0].ui_defaults === "string"
...
export async function seedDefaultTenantToDb(defaultRow: TenantConfigRow): Promise<void> {
  ...
  try {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT tenant_id FROM tenant_config WHERE tenant_id = $1",
