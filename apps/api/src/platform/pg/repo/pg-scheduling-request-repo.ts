...
export async function findSchedulingRequestById(id: string): Promise<SchedulingRequestRow | undefined> {
  const db = getPgDb();
  const rows = await db.select().from(pgSchedulingWaitlistRequest)
    .where(eq(pgSchedulingWaitlistRequest.id, id));
  return rows[0];
}

export async function findSchedulingRequestsByPatient(patientDfn: string): Promise<SchedulingRequestRow[]> {
  const db = getPgDb();
  return db.select().from(pgSchedulingWaitlistRequest)
...
export async function updateSchedulingRequest(id: string, updates: Partial<{
  status: string;
  reason: string;
}>): Promise<SchedulingRequestRow | undefined> {
  const db = getPgDb();
  const now = new Date().toISOString();
  await db.update(pgSchedulingWaitlistRequest)
