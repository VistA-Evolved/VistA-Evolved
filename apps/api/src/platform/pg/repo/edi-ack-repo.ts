...
export async function countAcks(tenantId: string): Promise<number> {
  ...
  const result = await db
    .select({ count: sql<number>`count(*)` })
    .from(pgEdiAck)
    .where(eq(pgEdiAck.tenantId, tenantId));
...
export async function findStatusByIdempotencyKey(
  ...
  const rows = await db
    .select()
    .from(pgEdiClaimStatus)
    .where(
      and(
        eq(pgEdiClaimStatus.tenantId, tenantId),
        eq(pgEdiClaimStatus.idempotencyKey, idempotencyKey)
      )
    );
  return rows[0];
}
...
export async function countStatusUpdates(tenantId: string): Promise<number> {
  ...
  const result = await db
    .select({ count: sql<number>`count(*)` })
    .from(pgEdiClaimStatus)
    .where(eq(pgEdiClaimStatus.tenantId, tenantId));
  return result[0]?.count ?? 0;
}
