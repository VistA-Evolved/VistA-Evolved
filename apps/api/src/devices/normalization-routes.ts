...
export default async function normalizationRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get("/devices/normalize/mappings", async () => {
    return {
      ok: true,
      tables: [
        { name: "MDC_TO_LOINC", description: "IEEE 11073 MDC to LOINC (vital signs, bedside monitors)", count: MDC_TO_LOINC.length },
        { name: "LAB_TO_LOINC", description: "Lab analyte to LOINC (chemistry, blood gas, hematology, coag)", count: LAB_TO_LOINC.length },
        { name: "UNIT_TO_UCUM", description: "Device units to UCUM (with conversion factors)", count: UNIT_TO_UCUM.length },
      ],
    };
  });
