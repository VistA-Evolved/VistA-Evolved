...
export default async function infusionBcmaRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get("/devices/infusion/pump-events/:id", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  server.patch("/devices/infusion/pump-events/:id/verify", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as any;
    const body = (req.body as any) || {};
  ...
  server.post("/devices/bcma/sessions/:id/patient-scan", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as any;
    const body = (req.body as any) || {};
    if (!body.barcodeData) {
  ...
  server.post("/devices/bcma/sessions/:id/medication-scan", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as any;
    const body = (req.body as any) || {};
    if (!body.barcodeData) {
      return reply.code(400).send({ ok: false, error: "barcodeData required" });
  ...
  server.post("/devices/bcma/sessions/:id/right6-check", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as any;
    const body = (req.body as any) || {};
  ...
  /** POST /devices/bcma/sessions/:id/complete — complete session */
  server.post("/devices/bcma/sessions/:id/complete", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as any;
    const body = (req.body as any) || {};
