...
export default async function clinicalReasoningRoutes(server: FastifyInstance) {
  ...
  server.get("/clinical-reasoning/libraries", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, libraries: listCqlLibraries(session.tenantId) };
  });

  server.post("/clinical-reasoning/libraries", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
  ...
  server.get("/clinical-reasoning/libraries/:id", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.put("/clinical-reasoning/libraries/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
  ...
  server.delete("/clinical-reasoning/libraries/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    if (!deleteCqlLibrary(id)) return reply.code(404).send({ ok: false, error: "Library not found" });
    return { ok: true };
  });

  // ---- Quality Measures ----

  server.get("/clinical-reasoning/measures", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, measures: listQualityMeasures(session.tenantId) };
  });

  server.post("/clinical-reasoning/measures", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as any) || {};
    const { measureId, name, description, scoring, type, program, cqlLibraryId, populations, measurementPeriodStart, measurementPeriodEnd, improvementNotation, status, contentPackId } = body;
  ...
  server.put("/clinical-reasoning/measures/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
  ...
  server.delete("/clinical-reasoning/measures/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    if (!deleteQualityMeasure(id)) return reply.code(404).send({ ok: false, error: "Measure not found" });
    return { ok: true };
  });

  // ---- Measure Evaluation ----

  server.post("/clinical-reasoning/measures/:id/evaluate", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
  ...
  server.get("/clinical-reasoning/evaluations", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, evaluations: listMeasureEvalResults(session.tenantId) };
  });

  server.get("/clinical-reasoning/evaluations/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
  ...
  server.get("/clinical-reasoning/patient-results", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { measureId, patientDfn } = request.query as { measureId?: string; patientDfn?: string };
    return {
      ok: true,
  ...
  server.post("/clinical-reasoning/patient-results", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as any) || {};
  ...
  server.get("/clinical-reasoning/plan-definitions", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, planDefinitions: listPlanDefinitions(session.tenantId) };
  });

  server.post("/clinical-reasoning/plan-definitions", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as any) || {};
  ...
  server.put("/clinical-reasoning/plan-definitions/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
  ...
  server.delete("/clinical-reasoning/plan-definitions/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    if (!deletePlanDefinition(id)) return reply.code(404).send({ ok: false, error: "PlanDefinition not found" });
    return { ok: true };
  });

  // ---- Activity Definitions ----

  server.get("/clinical-reasoning/activity-definitions", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, activityDefinitions: listActivityDefinitions(session.tenantId) };
  });

  server.post("/clinical-reasoning/activity-definitions", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as any) || {};
  ...
  server.put("/clinical-reasoning/activity-definitions/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
    const updated = updateActivityDefinition(id, body);
    if (!updated) return reply.code(404).send({ ok: false, error: "ActivityDefinition not found" });
    return { ok: true, activityDefinition: updated };
  });

  server.delete("/clinical-reasoning/activity-definitions/:id", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    if (!deleteActivityDefinition(id)) return reply.code(404).send({ ok: false, error: "ActivityDefinition not found" });
    return { ok: true };
  });

  // ---- Reports ----

  server.get("/clinical-reasoning/reports", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    return { ok: true, reports: listMeasureReports(session.tenantId) };
  });

  server.post("/clinical-reasoning/reports", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as any) || {};
  ...
  server.get("/clinical-reasoning/dashboard", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
