...
export default async function interopGatewayRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/interop-gateway/channels', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { name, description, direction, source, destination, transformPipelineId, status, tags } =
      body;
    if (!name || !direction || !source || !destination) {
      return reply
        .code(400)
        .send({ ok: false, error: 'name, direction, source, and destination required' });
    }
    try {
      ...
      log.error('Interop channel creation failed', {
  ...
  server.put(
    '/interop-gateway/channels/:id/status',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        log.error('Transform pipeline creation failed', {
          error: err instanceof Error ? err.message : String(err),
        });
        return reply.code(409).send({ ok: false, error: 'Internal error' });
      }
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const session = await requireSession(request, reply);
      const { channelId, status, limit } = request.query as {
        channelId?: string;
        status?: string;
        limit?: string;
      };
  ...
  server.get(
    '/interop-gateway/transactions/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        log.error('Mediator creation failed', {
          error: err instanceof Error ? err.message : String(err),
        });
        return reply.code(409).send({ ok: false, error: 'Internal error' });
      }
  ...
  server.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
      if (!deleteMediator(id))
        return reply.code(404).send({ ok: false, error: 'Mediator not found' });
      return { ok: true };
    }
  );
