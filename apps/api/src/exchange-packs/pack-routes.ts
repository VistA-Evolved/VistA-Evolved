...
export default async function exchangePackRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/exchange-packs/connectors', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return {
      ok: true,
      connectors: listConnectors(session.tenantId, { packId: qs.packId, status: qs.status }),
    };
  });

  server.get(
    '/exchange-packs/connectors/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        log.error('Exchange connector creation failed', {
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return {
        ok: true,
        transactions: listExchangeTransactions(session.tenantId, {
  ...
  server.get(
    '/exchange-packs/transactions/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
