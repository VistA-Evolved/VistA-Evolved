...
export function listConnectors(
  tenantId: string,
  opts?: { packId?: string; status?: string }
): ExchangeConnector[] {
...
export function updateConnector(
  id: string,
  patch: Partial<ExchangeConnector>
...
export function listExchangeTransactions(
  tenantId: string,
  opts?: { connectorId?: string; packId?: string; status?: string; limit?: number }
): ExchangeTransaction[] {
...
export function updateExchangeTransaction(
  id: string,
...
export function simulateExchange(
  ...
  connectorId: string,
  payload: string
): ExchangeTransaction | { error: string } {
...
export function getExchangePackDashboardStats(tenantId: string): ExchangePackDashboardStats {
  ...
  for (const t of txs) {
    byPack[t.packId] = (byPack[t.packId] || 0) + 1;
  }
