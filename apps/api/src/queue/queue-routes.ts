...
export async function queueRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/queue/tickets", async (request, reply) => {
    ...
    if (!department || !patientDfn || !patientName) {
      return reply.code(400).send({ ok: false, error: "department, patientDfn, patientName required" });
    }
    const input: CreateTicketInput = { department, patientDfn, patientName, priority, appointmentIen, notes };
  ...
  server.post("/queue/tickets/:id/call", async (request, reply) => {
    ...
    if (!ticket) return reply.code(400).send({ ok: false, error: "Cannot call ticket (invalid status or not found)" });
    return { ok: true, ticket };
  });
  ...
  server.post("/queue/call-next", async (request, reply) => {
    ...
    const ticket = callNextTicket(department, windowNumber, "default", (request as any).session?.duz);
    if (!ticket) return reply.code(404).send({ ok: false, error: "No waiting tickets in this department" });
    return { ok: true, ticket };
  });
  ...
  server.post("/queue/tickets/:id/serve", async (request, reply) => {
    ...
    if (!ticket) return reply.code(400).send({ ok: false, error: "Cannot start serving (invalid status)" });
    return { ok: true, ticket };
  });
  ...
  server.post("/queue/tickets/:id/transfer", async (request, reply) => {
    ...
    if (!targetDepartment) return reply.code(400).send({ ok: false, error: "targetDepartment required" });
