...
type CapabilitySecurity = NonNullable<
  NonNullable<FhirCapabilityStatement['rest']>[number]['security']
>;
...
export function buildCapabilityStatement(baseUrl: string): FhirCapabilityStatement {
  ...
  if (oidc.enabled) {
    ...
    security.description =
      'SMART-on-FHIR OAuth2 with OIDC bearer token support. Session cookie auth also accepted.';
  } else {
    security.description =
      'Session-based authentication. OIDC/SMART bearer token support available when OIDC_ENABLED=true.';
  }
          ...
          },
          {
            ...
            searchParam: [
              { name: 'patient', type: 'reference', documentation: 'Patient DFN' },
              {
                name: 'clinical-status',
                type: 'token',
                documentation: 'active | resolved | inactive',
              },
              { name: '_count', type: 'number' },
              { name: '_offset', type: 'number' },
            ],
          },
          {
            ...
            searchParam: [
              ...
              { name: 'code', type: 'token', documentation: 'LOINC code or display name' },
              {
                name: 'date',
                type: 'date',
                documentation: 'Observation date (supports prefixes: eq, gt, lt, ge, le)',
              },
              { name: '_count', type: 'number' },
              { name: '_offset', type: 'number' },
            ],
          ...
          },
          {
            ...
            searchParam: [
              { name: 'patient', type: 'reference', documentation: 'Patient DFN' },
              {
                name: 'date',
                type: 'date',
                documentation: 'Encounter date (supports prefixes: eq, gt, lt, ge, le)',
              },
              {
                name: 'status',
                type: 'token',
                documentation: 'planned | in-progress | finished | cancelled',
              },
              { name: '_count', type: 'number' },
              { name: '_offset', type: 'number' },
            ],
