...
import {
  isPgConfigured,
  pgHealthCheck,
} from "../platform/pg/index.js";
import {
  RPC_REGISTRY,
  RPC_EXCEPTIONS,
  getFullRpcInventory,
} from "../vista/rpcRegistry.js";
import {
  queryUnifiedAudit,
  getUnifiedAuditStats,
} from "../lib/unified-audit.js";
import {
  getConnectorCbStats,
  resetConnectorCb,
...
export function registerInlineRoutes(server: FastifyInstance): void {
  ...
  server.get("/vista/rpc-catalog", async (request, reply) => {
    ...
    try {
      ...
      if (err.message?.includes("not found") || err.message?.includes("not registered") || err.message?.includes("Application")) {
        return { ok: true, rpc: "VE LIST RPCS", count: 0, catalog: [], hint: "VE LIST RPCS not installed. Run scripts/install-rpc-catalog.ps1" };
      }
  ...
  server.get("/vista/patient-search", async (request) => {
    ...
    try {
      ...
      disconnect();
      return { ok: false, error: safeErr(err), hint: "Ensure VistA RPC Broker is running on 127.0.0.1:9430 and credentials are correct" };
    }
  });
  ...
  server.get("/vista/allergies", async (request) => {
    ...
    try {
      ...
      audit("phi.allergies-view", "success", auditActor(request), {
        patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { count: results.length },
      });
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) {
      disconnect();
  ...
  server.post("/vista/allergies", async (request) => {
    ...
    try {
      ...
      for (const line of matchLines) {
        ...
        if (source.includes("GMRD(120.82")) { matchEntry = { ien, name, source, allergyType }; break; }
        if (!matchEntry) matchEntry = { ien, name, source, allergyType };
      }
      if (!matchEntry) { disconnect(); return { ok: false, error: `No matching allergen found for "${allergyText.trim()}"`, hint: "Try a different allergen name" }; }
      const duz = getDuz();
      ...
      const oredited: Record<string, string> = { "GMRAGNT": gmragnt, "GMRATYPE": allergyType, "GMRANATR": "U^Unknown", "GMRAORIG": duz, "GMRAORDT": fmDate, "GMRAOBHX": "h^HISTORICAL" };
      const saveLines = await callRpcWithList("ORWDAL32 SAVE ALLERGY", [
        { type: "literal", value: "0" }, { type: "literal", value: String(dfn) }, { type: "list", value: oredited },
      ]);
      disconnect();
      ...
      audit("clinical.allergy-add", "success", auditActor(request), {
        patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { allergen: matchEntry.name },
      });
      return { ok: true, message: "Allergy created", allergen: matchEntry.name, result, rpcUsed: "ORWDAL32 SAVE ALLERGY" };
    } catch (err: any) {
      disconnect();
      return { ok: false, error: safeErr(err), hint: "Ensure VistA RPC Broker is running on 127.0.0.1:9430 and credentials are correct" };
    }
  ...
  server.get("/vista/vitals", async (request) => {
    const dfn = (request.query as any)?.dfn;
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn", hint: "Use ?dfn=1" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    ...
    try {
      ...
      disconnect();
      const results = lines
        .map((line) => { const parts = line.split("^"); const id = parts[0]?.trim(); const type = parts[1]?.trim() || ""; const value = parts[2]?.trim() || ""; const takenAtFM = parts[3]?.trim() || ""; if (!id) return null; let takenAt = takenAtFM; if (takenAtFM && takenAtFM.length >= 7) { const datePart = takenAtFM.split(".")[0] || ""; const timePart = takenAtFM.split(".")[1] || ""; if (/^\d{7}$/.test(datePart)) { const y = parseInt(datePart.substring(0, 3), 10) + 1700; const m = datePart.substring(3, 5); const d = datePart.substring(5, 7); let timeStr = ""; if (timePart && timePart.length >= 4) timeStr = " " + timePart.substring(0, 2) + ":" + timePart.substring(2, 4); takenAt = `${y}-${m}-${d}${timeStr}`; } } return { type, value, takenAt }; })
        .filter((r) => r !== null);
      audit("phi.vitals-view", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { count: results.length } });
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });

  server.post("/vista/vitals", async (request) => {
    const body = request.body as any;
    const dfn = body?.dfn; const type = (body?.type || "").toUpperCase().trim(); const value = (body?.value || "").trim();
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn" };
    if (!type || !VITAL_TYPE_IEN[type]) return { ok: false, error: `Invalid vital type. Must be one of: ${VALID_VITAL_TYPES.join(", ")}` };
    if (!value || value.length < 1) return { ok: false, error: "Missing value" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    try {
      await connect();
      const duz = getDuz();
      const now = new Date(); const fmYear = now.getFullYear() - 1700;
      ...
      const vitalTypeIen = VITAL_TYPE_IEN[type]; const hospitalLocation = 2;
      const gmvData = `${fmDate}^${dfn}^${vitalTypeIen};${value};^${hospitalLocation}^${duz}`;
      const lines = await callRpc("GMV ADD VM", [gmvData]);
      disconnect();
      const allText = lines.join("\n");
      if (allText.includes("ERROR")) return { ok: false, error: allText.trim() || "VistA returned an error" };
      audit("clinical.vitals-add", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { type, value } });
  ...
  server.get("/vista/notes", async (request) => {
    ...
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn query parameter" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    ...
    try {
      ...
      disconnect();
      const seenIens = new Set<string>(); const allLines: string[] = [];
      for (const line of [...unsignedLines, ...signedLines]) { const ien = line.split("^")[0]?.trim(); if (ien && /^\d+$/.test(ien) && !seenIens.has(ien)) { seenIens.add(ien); allLines.push(line); } }
      const errorLine = [...signedLines, ...unsignedLines].find(l => l.startsWith("-1"));
      if (errorLine && allLines.length === 0) { const errMsg = errorLine.split("^").slice(1).join("^") || "Unknown VistA error"; return { ok: false, error: errMsg, rpcUsed: RPC_NAME }; }
      const results = allLines.map((line) => { const parts = line.split("^"); if (parts.length < 7) return null; const id = parts[0].trim(); if (!id || !/^\d+$/.test(id)) return null; const title = (parts[1] || "").replace(/^\+\s*/, "").trim(); const fmDate = parts[2] || ""; const authorField = parts[4] || ""; const location = parts[5] || ""; const status = parts[6] || ""; let date = fmDate; if (fmDate && fmDate.length >= 7) { const [datePart, timePart] = fmDate.split("."); const y = parseInt(datePart.substring(0, 3), 10) + 1700; const m = datePart.substring(3, 5); const d = datePart.substring(5, 7); date = `${y}-${m}-${d}`; if (timePart && timePart.length >= 4) date += ` ${timePart.substring(0, 2)}:${timePart.substring(2, 4)}`; } const authorParts = authorField.split(";"); const author = authorParts.length >= 3 ? authorParts[2] : authorParts[1] || authorField; return { id, title, date, author, location, status }; }).filter(Boolean);
      audit("phi.notes-view", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { count: results.length } });
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });

  server.post("/vista/notes", async (request) => {
    const body = request.body as any; const dfn = body?.dfn; const title = (body?.title || "").trim(); const text = (body?.text || "").trim();
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn" };
    if (!title || title.length < 1) return { ok: false, error: "Missing title" };
    if (!text || text.length < 1) return { ok: false, error: "Missing text" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    try {
      await connect(); const duz = getDuz();
      const now = new Date(); const fmYear = now.getFullYear() - 1700;
      const fmDate = `${fmYear}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}.${String(now.getHours()).padStart(2, "0")}${String(now.getMinutes()).padStart(2, "0")}`;
      const TITLE_IEN = "10"; const VLOC = "2";
      const tiux: Record<string, string> = { "1202": String(duz), "1301": fmDate };
      const createLines = await callRpcWithList("TIU CREATE RECORD", [
        { type: "literal", value: String(dfn) }, { type: "literal", value: TITLE_IEN }, { type: "literal", value: fmDate }, { type: "literal", value: VLOC }, { type: "literal", value: "" }, { type: "list", value: tiux }, { type: "literal", value: "" }, { type: "literal", value: "1" }, { type: "literal", value: "0" },
      ]);
      const createResult = createLines[0] || "";
      if (createResult.startsWith("0^") || createResult.startsWith("-1")) { const errMsg = createResult.split("^").slice(1).join("^") || "Failed to create note record"; disconnect(); return { ok: false, error: errMsg }; }
      const noteId = createResult.split("^")[0].trim();
      if (!noteId || !/^\d+$/.test(noteId)) { disconnect(); return { ok: false, error: `Unexpected response: ${createResult}` }; }
      const bodyLines = text.split(/\r?\n/); const allTextLines = [title, "", ...bodyLines];
      const textData: Record<string, string> = { "HDR": "1^1" };
      allTextLines.forEach((line, i) => { textData[`TEXT,${i + 1},0`] = line; });
      const textResult = await callRpcWithList("TIU SET DOCUMENT TEXT", [
        { type: "literal", value: noteId }, { type: "list", value: textData }, { type: "literal", value: "0" },
      ]);
      disconnect();
      const textResp = textResult[0] || "";
      if (textResp.startsWith("0^")) return { ok: false, error: `Note ${noteId} created but text save failed: ${textResp}`, id: noteId };
      audit("clinical.note-create", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { noteId, titleIen: TITLE_IEN } });
      return { ok: true, id: noteId, message: "Note created", rpcUsed: "TIU CREATE RECORD + TIU SET DOCUMENT TEXT" };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });
  ...
  server.get("/vista/medications", async (request) => {
    ...
    try {
      ...
      if (activeLines.length > 0 && activeLines[0].startsWith("-1")) { const errMsg = activeLines[0].split("^").slice(1).join("^") || "Unknown VistA error"; disconnect(); return { ok: false, error: errMsg, rpcUsed: "ORWPS ACTIVE" }; }
      interface MedEntry { orderIEN: string; rxId: string; type: string; drugName: string; status: string; qty: string; sig: string; }
      const meds: MedEntry[] = []; let current: MedEntry | null = null;
      for (const line of activeLines) {
        if (line.startsWith("~")) { const typeEnd = line.indexOf("^"); const type = line.substring(1, typeEnd); const fields = line.substring(typeEnd + 1).split("^"); current = { orderIEN: fields[7]?.trim() || "", rxId: fields[0]?.split(";")[0] || "", type, drugName: fields[1]?.trim() || "", status: fields[8]?.trim() || "", qty: fields[11]?.trim() || "", sig: "" }; meds.push(current); }
        else if (current) { const trimmed = line.trim(); if (trimmed.startsWith("\\ Sig:") || trimmed.startsWith("\\Sig:")) current.sig = trimmed.replace(/^\\\s*Sig:\s*/i, "").trim(); else if (trimmed.startsWith("Qty:")) current.qty = trimmed.replace(/^Qty:\s*/, "").trim(); }
      }
      for (const med of meds) {
        if (!med.drugName && med.orderIEN && /^\d+$/.test(med.orderIEN)) { try { const txtLines = await callRpc("ORWORR GETTXT", [med.orderIEN]); if (txtLines.length > 0 && !txtLines[0].startsWith("-1")) { med.drugName = txtLines[0].trim(); if (!med.sig && txtLines[1]) med.sig = txtLines[1].trim(); } } catch { /* non-fatal */ } }
      }
      disconnect();
      const results = meds.map((m) => ({ id: m.rxId || m.orderIEN, name: m.drugName || "(unknown medication)", sig: m.sig, status: m.status.toLowerCase() || "active" }));
      audit("phi.medications-view", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { count: results.length } });
      return { ok: true, count: results.length, results, rpcUsed: "ORWPS ACTIVE" };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });

  server.post("/vista/medications", async (request) => {
    const body = request.body as any; const dfn = body?.dfn; const drug = body?.drug;
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn" };
    if (!drug || typeof drug !== "string" || drug.trim().length < 2) return { ok: false, error: "drug must be at least 2 characters" };
    const qo = matchQuickOrder(drug);
    if (!qo) { const available = QUICK_ORDERS.map((q) => q.name).join(", "); return { ok: false, error: `No matching quick order for "${drug.trim()}"`, availableDrugs: available }; }
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    try {
      await connect(); const duz = getDuz();
      const lockLines = await callRpc("ORWDX LOCK", [String(dfn)]);
      if ((lockLines[0]?.trim() || "") !== "1") { disconnect(); return { ok: false, error: "Could not lock patient for ordering" }; }
      const LOCATION_IEN = "2";
      let autoackLines: string[];
      try { autoackLines = await callRpc("ORWDXM AUTOACK", [String(dfn), duz, LOCATION_IEN, String(qo.ien)]); }
      catch (autoackErr: any) { try { await callRpc("ORWDX UNLOCK", [String(dfn)]); } catch {} disconnect(); return { ok: false, error: "AUTOACK failed: " + safeErr(autoackErr) }; }
      try { await callRpc("ORWDX UNLOCK", [String(dfn)]); } catch {}
      disconnect();
      const raw = autoackLines.join("\n").trim();
      if (!raw || raw === "0" || raw.startsWith("-1")) return { ok: false, error: "Order was not created. AUTOACK returned: " + (raw || "(empty)") };
      const orderIEN = (raw.split(/[\^;]/)[0]?.trim() || raw).replace(/^~/, "");
      audit("clinical.medication-add", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { quickOrder: qo.name, orderIEN } });
      return { ok: true, message: `Medication order created (unsigned): ${qo.name}`, orderIEN, quickOrder: qo.name, raw: autoackLines, rpcUsed: "ORWDXM AUTOACK" };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });
  ...
  server.get("/vista/default-patient-list", async (request) => {
    ...
    try {
      ...
      const results = lines.map((line) => { const [dfn, name] = line.split("^").map((s) => s.trim()); if (dfn && name) return { dfn, name }; return null; }).filter((r) => r !== null);
      disconnect();
      audit("phi.patient-list", "success", auditActor(request), { detail: { count: results.length } });
  ...
  server.get("/vista/problems", async (request) => {
    ...
    try {
      ...
      disconnect();
      if (lines.length > 0 && lines[0].startsWith("-1")) { const errMsg = lines[0].split("^").slice(1).join("^") || "Unknown VistA error"; return { ok: false, error: errMsg, rpcUsed: RPC_NAME }; }
      const results = lines.map((line) => { if (!line || line.trim() === "") return null; const parts = line.split("^"); if (parts.length < 2) return null; const ien = parts[0]?.trim(); const text = parts[1]?.trim(); const status = parts[2]?.trim() || "Unknown"; const onset = parts[3]?.trim() || ""; if (!ien || !text) return null; let displayStatus = "active"; if (status.toUpperCase().includes("I") || status === "0") displayStatus = "inactive"; else if (status.toUpperCase().includes("R") || status === "2") displayStatus = "resolved"; return { id: ien, text, status: displayStatus, onset: onset || undefined }; }).filter((r) => r !== null);
      audit("phi.problems-view", "success", auditActor(request), { patientDfn: String(dfn), requestId: (request as any).requestId, sourceIp: request.ip, detail: { count: results.length } });
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });
  ...
  server.post("/vista/problems", async (_request) => {
    ...
    const dfn = body?.dfn; const text = body?.text;
    if (!dfn || !/^\d+$/.test(String(dfn))) return { ok: false, error: "Missing or non-numeric dfn" };
    if (!text || typeof text !== "string" || text.trim().length < 2) return { ok: false, error: "text must be at least 2 characters" };
    return { ok: false, error: "Problem creation is not yet implemented in this MVP. VistA problem entry requires ICD-9/ICD-10 diagnosis codes, provider validation, and service condition flags.", blocker: { reason: "Complex CPOE validation", recommendation: "Use VistA CPRS GUI which handles validation safely" } };
  });

  server.get("/vista/icd-search", async (request) => {
    const { q } = request.query as any;
    if (!q || typeof q !== "string" || q.trim().length < 2) return { ok: false, error: "Search query must be at least 2 characters" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    const RPC_NAME = "ORQQPL4 LEX";
    try {
      await connect(); const lines = await callRpc(RPC_NAME, [q.trim()]); disconnect();
      const results = (Array.isArray(lines) ? lines : [lines]).filter((l: string) => l && l.trim()).map((line: string, idx: number) => { const parts = line.split("^"); if (parts.length >= 3) return { id: parts[0], description: parts[1], icd: parts[2] }; else if (parts.length === 2) return { id: String(idx), description: parts[0], icd: parts[1] }; return { id: String(idx), icd: "", description: line }; });
      return { ok: true, rpc: RPC_NAME, count: results.length, results };
    } catch (err: any) { disconnect(); return { ok: false, rpc: RPC_NAME, error: safeErr(err) }; }
  });
  ...
  server.get("/vista/consults", async (request) => {
    ...
    try {
      await connect(); const lines = await callRpc(RPC_NAME, [String(dfn), "", "", "", ""]); disconnect();
      if (lines.length === 0 || (lines.length === 1 && lines[0].startsWith("<"))) return { ok: true, count: 0, results: [], rpcUsed: RPC_NAME };
      const results = lines.map((line) => { const p = line.split("^"); if (p.length < 5) return null; const id = p[0]?.trim(); if (!id || !/^\d+$/.test(id)) return null; let date = p[1] || ""; if (date && date.length >= 7) { const [dp, tp] = date.split("."); const y = parseInt(dp.substring(0, 3), 10) + 1700; const m = dp.substring(3, 5); const d = dp.substring(5, 7); date = `${y}-${m}-${d}`; if (tp && tp.length >= 4) date += ` ${tp.substring(0, 2)}:${tp.substring(2, 4)}`; } const status = (p[2] || "").trim(); const service = (p[3] || "").trim(); const typeStr = (p[4] || "").trim(); const typeCode = (p[8] || "").trim(); return { id, date, status, service, type: typeStr || "Consult", typeCode }; }).filter(Boolean);
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err), rpcUsed: RPC_NAME }; }
  });

  server.get("/vista/consults/detail", async (request) => {
    const { id } = request.query as any;
    if (!id || !/^\d+$/.test(String(id))) return { ok: false, error: "Missing or non-numeric consult id" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    try { await connect(); const lines = await callRpc("ORQQCN DETAIL", [String(id)]); disconnect(); return { ok: true, text: lines.join("\n"), rpcUsed: "ORQQCN DETAIL" }; }
    catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });
  ...
  server.get("/vista/surgery", async (request) => {
    ...
    try {
      await connect(); const lines = await callRpc(RPC_NAME, [String(dfn), "", "", "-1", "999"]); disconnect();
      if (lines.length === 0 || (lines.length === 1 && !lines[0].trim())) return { ok: true, count: 0, results: [], rpcUsed: RPC_NAME };
      const results = lines.map((line) => { const p = line.split("^"); if (p.length < 3) return null; const id = p[0]?.trim(); if (!id) return null; const procedure = (p[1] || "").trim(); let date = (p[2] || "").trim(); if (date && date.length >= 7 && !date.includes("-")) { const [dp, tp] = date.split("."); const y = parseInt(dp.substring(0, 3), 10) + 1700; const m = dp.substring(3, 5); const d = dp.substring(5, 7); date = `${y}-${m}-${d}`; if (tp && tp.length >= 4) date += ` ${tp.substring(0, 2)}:${tp.substring(2, 4)}`; } const surgeonField = (p[3] || "").trim(); const surgeon = surgeonField.includes(";") ? surgeonField.split(";")[1] || surgeonField : surgeonField; return { id, procedure, date, surgeon, status: "Complete" }; }).filter(Boolean);
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err), rpcUsed: RPC_NAME }; }
  });
  ...
  server.get("/vista/dc-summaries", async (request) => {
    ...
    try {
      ...
      const unsignedLines = await callRpc(RPC_NAME, ["244", "2", String(dfn), "", "", "0", "0", "D"]);
      disconnect();
      const seenIens = new Set<string>(); const allLines: string[] = [];
      for (const line of [...unsignedLines, ...signedLines]) { const ien = line.split("^")[0]?.trim(); if (ien && /^\d+$/.test(ien) && !seenIens.has(ien)) { seenIens.add(ien); allLines.push(line); } }
      const results = allLines.map((line) => { const parts = line.split("^"); if (parts.length < 7) return null; const id = parts[0].trim(); if (!id || !/^\d+$/.test(id)) return null; const title = (parts[1] || "").replace(/^\+\s*/, "").trim(); const fmDate = parts[2] || ""; let date = fmDate; if (fmDate && fmDate.length >= 7) { const [datePart, timePart] = fmDate.split("."); const y = parseInt(datePart.substring(0, 3), 10) + 1700; const m = datePart.substring(3, 5); const d = datePart.substring(5, 7); date = `${y}-${m}-${d}`; if (timePart && timePart.length >= 4) date += ` ${timePart.substring(0, 2)}:${timePart.substring(2, 4)}`; } const authorField = parts[4] || ""; const authorParts = authorField.split(";"); const author = authorParts.length >= 3 ? authorParts[2] : authorParts[1] || authorField; const location = parts[5] || ""; const status = parts[6] || ""; return { id, title, date, author, location, status }; }).filter(Boolean);
      return { ok: true, count: results.length, results, rpcUsed: RPC_NAME };
    } catch (err: any) { disconnect(); return { ok: false, error: safeErr(err), rpcUsed: RPC_NAME }; }
  });
  ...
  server.get("/vista/tiu-text", async (request) => {
    const { id } = request.query as any;
    if (!id || !/^\d+$/.test(String(id))) return { ok: false, error: "Missing or non-numeric document id" };
    try { validateCredentials(); } catch (err: any) { return { ok: false, error: safeErr(err) }; }
    try { await connect(); const lines = await callRpc("TIU GET RECORD TEXT", [String(id)]); disconnect(); return { ok: true, text: lines.join("\n"), rpcUsed: "TIU GET RECORD TEXT" }; }
    catch (err: any) { disconnect(); return { ok: false, error: safeErr(err) }; }
  });
  ...
  server.get("/vista/labs", async (request) => {
    ...
    try {
      ...
      let currentSpecimen = ""; let currentDate = "";
      for (const line of lines) {
        const trimmed = line.trim(); if (!trimmed) continue;
        if (/^Specimen:/i.test(trimmed)) { currentSpecimen = trimmed.replace(/^Specimen:\s*/i, "").trim(); continue; }
        if (/^(Collection\s+Date|Collected):/i.test(trimmed)) { currentDate = trimmed.replace(/^(Collection\s+Date|Collected):\s*/i, "").trim(); continue; }
        if (trimmed.includes("^")) { const p = trimmed.split("^"); if (p.length >= 2) results.push({ testName: (p[0] || "").trim(), result: (p[1] || "").trim(), units: (p[2] || "").trim(), refRange: (p[3] || "").trim(), flag: (p[4] || "").trim(), specimen: currentSpecimen, collectionDate: currentDate }); }
      }
      const rawText = lines.join("\n");
      if (results.length === 0) return { ok: true, count: 0, results: [], rawText, rpcUsed: RPC_NAME, note: "Lab results returned as free text; structured parsing found no caret-delimited entries" };
  ...
  server.get("/vista/reports", async (_request) => {
    ...
    try {
      await connect(); const lines = await callRpc(RPC_NAME, []); disconnect();
      const reports: { id: string; heading: string; qualifier: string; remote: string; rpcName: string; category: string }[] = [];
      const dateRanges: string[] = []; const hsTypes: string[] = []; let currentSection = "";
      for (const line of lines) {
        ...
        if (currentSection === "reportList" && line.trim()) { const p = line.split("^"); reports.push({ id: (p[0] || "").trim(), heading: (p[1] || "").trim(), qualifier: (p[2] || "").trim(), remote: (p[6] || "").trim(), rpcName: (p[9] || "").trim(), category: (p[8] || "").trim() }); }
      }
  ...
  server.get("/vista/reports/text", async (request) => {
    ...
    try {
      await connect(); const lines = await callRpc(RPC_NAME, [String(dfn), String(id), String(hsType || ""), "", "0", "", ""]); disconnect();
