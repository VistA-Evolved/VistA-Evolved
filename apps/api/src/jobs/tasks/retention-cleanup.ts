 ...
 *  4. Report results
 */
export async function handleRetentionCleanup(payload: Record<string, unknown>): Promise<void> {
  ...
  try {
    ...
    if (dryRun) {
      ...
      const res = await pool.query('DELETE FROM auth_session WHERE expires_at < $1', [cutoffIso]);
  ...
  try {
    ...
    if (!dryRun) {
      const res = await pool.query('DELETE FROM idempotency_key WHERE expires_at < $1', [
        cutoffIso,
      ]);
  ...
  try {
    ...
    if (!dryRun) {
      const res = await pool.query('DELETE FROM job_run_log WHERE ok = true AND finished_at < $1', [
