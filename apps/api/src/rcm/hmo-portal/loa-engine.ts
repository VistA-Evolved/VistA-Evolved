...
const SPECIALTY_TEMPLATES: Record<DepartmentSpecialty, LoaPacketTemplate> = {
  ...
  surgery: {
    specialty: "surgery",
    requiredFields: ["diagnoses", "procedures", "encounterDate", "attendingPhysician", "estimatedDays"],
  ...
  obstetrics_gynecology: {
    ...
    payerSpecificNotes: "Normal spontaneous delivery (NSD) usually covered under LOA. Caesarean may require additional approval.",
  },
  pediatrics: {
  ...
  oncology: {
    specialty: "oncology",
    requiredFields: ["diagnoses", "procedures", "encounterDate", "attendingPhysician", "estimatedDays"],
    recommendedAttachments: ["clinical_note", "biopsy_report", "imaging_report", "treatment_protocol"],
  ...
  emergency: {
    ...
    payerSpecificNotes: "Emergency LOA may be waived for 24-48 hours. Confirm payer's emergency policy.",
  },
  rehabilitation: {
...
export function buildLoaPacket(
  opts: LoaPacketBuildOptions,
): { ok: boolean; packet?: LoaPacket; errors?: string[] } {
  ...
  if (opts.requestedServices.length === 0) errors.push("At least one requested service is required.");
...
function generateLoaPdfText(packet: LoaPacket): LoaPacketExport {
  ...
  lines.push(line("Estimated Charges", packet.estimatedCharges ? `PHP ${packet.estimatedCharges.toFixed(2)}` : undefined));
  lines.push("");
