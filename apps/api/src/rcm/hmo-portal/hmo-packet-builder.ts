...
export function buildHmoClaimPacket(
  opts: HmoPacketBuildOptions,
): { ok: boolean; packet?: HmoClaimPacket; errors?: string[] } {
  const { claim } = opts;
  const errors: string[] = [];
  ...
  const partial: Omit<HmoClaimPacket, "contentHash"> = {
    ...
    patient: {
      ...
      sex: (claim.patientGender === "M" || claim.patientGender === "F")
        ? claim.patientGender : undefined,
...
function sep(): string { return "-".repeat(72); }
...
export function exportHmoPacketText(packet: HmoClaimPacket): {
  ...
  for (const ch of packet.charges) {
    lines.push(`  ${pad(ch.description, 30)} ${ch.quantity}x ${ch.unitCharge.toFixed(2)} = ${ch.netAmount.toFixed(2)}  (HMO: ${ch.hmoCoverage.toFixed(2)})`);
  }
  lines.push("");
