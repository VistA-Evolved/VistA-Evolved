...
function generatePdfTextExport(packet: ClaimPacket): ExportArtifact {
  ...
  lines.push(separator());
  lines.push(line("Name", `${packet.patient.lastName}, ${packet.patient.firstName} ${packet.patient.middleName ?? ""}`));
  ...
  lines.push(line("Relationship", packet.patient.memberRelationship === "S" ? "Self" : packet.patient.memberRelationship === "D" ? "Dependent" : "Parent"));
  ...
  if (packet.professionalFees.length > 0) {
    ...
    for (const fee of packet.professionalFees) {
      ...
      lines.push(`    Fee: PHP ${fee.feeAmount.toFixed(2)}  Date: ${fee.serviceDate}  Proc: ${fee.procedureCode ?? "N/A"}`);
    }
  ...
  if (packet.charges.length > 0) {
    ...
    lines.push(separator());
    lines.push(`  ${"Category".padEnd(20)} ${"Description".padEnd(25)} ${"Qty".padStart(4)} ${"Net Amt".padStart(12)}`);
    lines.push(`  ${"─".repeat(20)} ${"─".repeat(25)} ${"─".repeat(4)} ${"─".repeat(12)}`);
    for (const c of packet.charges) {
      lines.push(`  ${c.category.padEnd(20)} ${c.description.slice(0, 25).padEnd(25)} ${String(c.quantity).padStart(4)} ${("PHP " + c.netAmount.toFixed(2)).padStart(12)}`);
    }
    lines.push("");
  }
