...
export async function philhealthTransportRoutes(server: FastifyInstance): Promise<void> {
  ...
  /* Submit claim bundle */
  server.post("/rcm/philhealth/transport/submit", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  /* Check claim status */
  server.get("/rcm/philhealth/transport/status/:claimRefNo", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.get("/rcm/philhealth/transport/eligibility/:pin", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  /* Upload attachment */
  server.post("/rcm/philhealth/transport/attachments/:claimRefNo", async (request: FastifyRequest, reply: FastifyReply) => {
    const { claimRefNo } = request.params as { claimRefNo: string };
    const body = (request.body as any) || {};
