...
export class StediConnector implements RcmConnector {
        ...
        errors: [
          {
            code: 'STEDI-DISABLED',
            description:
              'Stedi connector is feature-flagged off. Set STEDI_ENABLED=true to activate.',
            severity: 'info',
          },
        ],
        ...
        errors: [
          {
            ...
            description:
              'Stedi connector enabled but not configured. Set STEDI_API_KEY. Docs: https://www.stedi.com/docs',
            severity: 'error',
          },
        ],
        metadata: {
  ...
  async healthCheck(): Promise<{ healthy: boolean; details?: string }> {
    ...
    if (!this.configured) {
      return {
        healthy: false,
        details: 'Stedi: enabled but STEDI_API_KEY not set. Docs: https://www.stedi.com/docs',
      };
    }
    return {
      healthy: false,
      details: 'Stedi: configured but health probe requires live API call.',
    };
  }
