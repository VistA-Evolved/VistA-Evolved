...
export default async function paymentRoutes(server: FastifyInstance): Promise<void> {
  ...
  /* ── Create Batch ──────────────────────────────────────── */
  server.post('/payerops/payments/batches', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post('/payerops/payments/batches/:id/upload', async (request: FastifyRequest, reply: FastifyReply) => {
    const { duz, tenantId } = getSession(request);
    ...
    if (batch.tenantId !== tenantId) return reply.status(403).send({ ok: false, error: 'Access denied' });

    if (!body.content) {
      return reply.status(400).send({ ok: false, error: 'content is required (CSV text or base64)' });
    }
  ...
  /* ── Import (parse CSV) ────────────────────────────────── */
  server.post('/payerops/payments/batches/:id/import', async (request: FastifyRequest, reply: FastifyReply) => {
    const { duz, tenantId } = getSession(request);
    const { id } = request.params as { id: string };
    ...
    if (!csvContent) {
      return reply.status(400).send({ ok: false, error: 'No file uploaded. Upload first via /upload endpoint.' });
    }
  ...
  /* ── Match batch ───────────────────────────────────────── */
  server.post('/payerops/payments/batches/:id/match', async (request: FastifyRequest, reply: FastifyReply) => {
    const { duz, tenantId } = getSession(request);
    const { id } = request.params as { id: string };
  ...
  server.get('/payerops/payments/batches/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = getSession(request);
    const { id } = request.params as { id: string };
    const q = (request.query as any) || {};
    ...
    return reply.send({ ok: true, batch, lines: linesResult.items, linesTotal: linesResult.total });
  });

  /* ── Reconciliation worklist ───────────────────────────── */
  server.get('/payerops/payments/reconciliation', async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = getSession(request);
    const q = (request.query as any) || {};
  ...
  /* ── Manual link claim ─────────────────────────────────── */
  server.post('/payerops/payments/reconciliation/:lineId/link-claim', async (request: FastifyRequest, reply: FastifyReply) => {
    const { duz, tenantId } = getSession(request);
    ...
    if (line.tenantId !== tenantId) return reply.status(403).send({ ok: false, error: 'Access denied' });
  ...
  /* ── Payer Intelligence ────────────────────────────────── */
  server.get('/payerops/analytics/payer-intelligence', async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = getSession(request);
    const q = (request.query as any) || {};
  ...
  /* ── Export ────────────────────────────────────────────── */
  server.get('/payerops/exports/payments/:batchId', async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = getSession(request);
    ...
    if (!result) {
      return reply.status(404).send({ ok: false, error: 'Batch not found or export format unavailable' });
    }
  ...
  /* ── Underpayments ─────────────────────────────────────── */
  server.get('/payerops/payments/underpayments', async (request: FastifyRequest, reply: FastifyReply) => {
    const { tenantId } = getSession(request);
    const q = (request.query as any) || {};
  ...
  /* ── Store Info ────────────────────────────────────────── */
  server.get('/payerops/payments/store-info', async (_request: FastifyRequest, reply: FastifyReply) => {
