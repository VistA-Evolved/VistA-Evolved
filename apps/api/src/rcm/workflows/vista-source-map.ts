...
export const VISTA_SOURCE_MAP: VistaSourceEntry[] = [
  ...
  // --- Remittance Fields -----------------------------------
  {
    ...
    sandboxNote:
      'Remittance data comes from payer (EOB/SOA), not from VistA. Posted to VistA AR after reconciliation.',
  },
  {
...
export function getVistaSourceStats(): {
  ...
  const stats = {
    ...
    notApplicable: 0,
    byCategory: {} as Record<string, { available: number; pending: number; na: number }>,
  };
