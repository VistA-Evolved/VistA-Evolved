...
describe("consent-posture", () => {
  ...
  it("recordConsent is idempotent (updates existing)", () => {
    ...
    const updated = recordConsent("room-c2", "patient-hash", "patient", "telehealth_video", "granted");
  ...
  it("DEFAULT_CONSENT_REQUIREMENTS has video as required", () => {
    const videoReq = DEFAULT_CONSENT_REQUIREMENTS.find(
      (r) => r.category === "telehealth_video",
    );
  ...
  it("DEFAULT_CONSENT_REQUIREMENTS has recording as NOT required", () => {
    const recReq = DEFAULT_CONSENT_REQUIREMENTS.find(
