...
function loadConfig(): SamlBrokerConfig {
  return {
    ...
    brokerIssuer:
      process.env.IDP_SAML_BROKER_ISSUER ||
      "http://localhost:8180/realms/vista-evolved",
    brokerClientId:
      process.env.IDP_SAML_BROKER_CLIENT_ID || "vista-evolved-saml-broker",
...
export class SamlBrokerIdentityProvider implements IdentityProvider {
  ...
  async healthCheck(): Promise<{ ok: boolean; detail?: string }> {
    ...
    try {
      ...
      return { ok: resp.ok, detail: resp.ok ? "broker discovery reachable" : `HTTP ${resp.status}` };
    } catch (err: any) {
