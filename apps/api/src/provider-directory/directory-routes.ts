...
export default async function providerDirectoryRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return {
        ok: true,
        practitioners: listPractitioners(session.tenantId, {
  ...
  server.get(
    '/provider-directory/practitioners/search',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const session = await requireSession(request, reply);
      const qs = (request.query || {}) as Record<string, string>;
  ...
  server.get(
    '/provider-directory/practitioners/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  server.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        log.error('Practitioner creation failed', {
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return {
        ok: true,
        organizations: listOrganizations(session.tenantId, {
          type: qs.type,
          active: qs.active === 'true' ? true : qs.active === 'false' ? false : undefined,
        }),
      };
    }
  );
  ...
  server.get(
    '/provider-directory/organizations/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return {
        ok: true,
        locations: listLocations(session.tenantId, {
          organizationId: qs.organizationId,
          status: qs.status,
        }),
      };
  ...
  server.get(
    '/provider-directory/locations/:id',
    async (request: FastifyRequest, reply: FastifyReply) => {
      await requireSession(request, reply);
      const { id } = request.params as { id: string };
