...
export function createPractitioner(input: Omit<DirectoryPractitioner, "id" | "createdAt" | "updatedAt">): DirectoryPractitioner {
  enforceMax(practitionerStore, MAX_PRACTITIONERS);
  const now = new Date().toISOString();
  const rec: DirectoryPractitioner = { ...input, id: genId("prac"), createdAt: now, updatedAt: now };
...
export function listPractitioners(tenantId: string, opts?: { specialty?: string; status?: string; organizationId?: string }): DirectoryPractitioner[] {
  ...
  if (opts?.organizationId) results = results.filter((p) => p.organizationIds.includes(opts.organizationId!));
  return results;
}

export function updatePractitioner(id: string, patch: Partial<DirectoryPractitioner>): DirectoryPractitioner | undefined {
...
export function createOrganization(input: Omit<DirectoryOrganization, "id" | "createdAt" | "updatedAt">): DirectoryOrganization {
...
export function listOrganizations(tenantId: string, opts?: { type?: string; active?: boolean }): DirectoryOrganization[] {
...
export function updateOrganization(id: string, patch: Partial<DirectoryOrganization>): DirectoryOrganization | undefined {
...
export function createLocation(input: Omit<DirectoryLocation, "id" | "createdAt" | "updatedAt">): DirectoryLocation {
...
export function listLocations(tenantId: string, opts?: { organizationId?: string; status?: string }): DirectoryLocation[] {
  let results = Array.from(locationStore.values()).filter((l) => l.tenantId === tenantId);
  if (opts?.organizationId) results = results.filter((l) => l.organizationId === opts.organizationId);
  if (opts?.status) results = results.filter((l) => l.status === opts.status);
  return results;
}

export function updateLocation(id: string, patch: Partial<DirectoryLocation>): DirectoryLocation | undefined {
