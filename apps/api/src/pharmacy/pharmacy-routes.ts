...
export async function pharmacyRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/pharmacy/orders", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const {
      patientDfn, drugName, dose, route, schedule,
      orderType = "inpatient", ndc = null, drugClass = null,
      durationDays = null, instructions = "",
    } = body;
    if (!patientDfn || !drugName || !dose || !route || !schedule) {
      return reply.code(400).send({ ok: false, error: "patientDfn, drugName, dose, route, schedule required" });
    }
  ...
  server.post("/pharmacy/orders/:id/transition", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
    ...
    const result = transitionPharmOrder(
      id,
      status as PharmOrderStatus,
      { duz: session.duz || "", name: session.userName || "" },
    );
    if ("error" in result) return reply.code(400).send({ ok: false, error: result.error });
    return { ok: true, order: result };
  });

  server.post("/pharmacy/orders/:id/override", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
    const { id } = request.params as { id: string };
    const body = (request.body as any) || {};
  ...
  server.post("/pharmacy/dispense", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const body = (request.body as any) || {};
    const {
      pharmOrderId, patientDfn, quantity = 1,
      dispensedNdc = null, lotNumber = null, expirationDate = null,
  ...
  server.post("/pharmacy/admin-records", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const body = (request.body as any) || {};
    const {
      pharmOrderId, patientDfn, status = "given",
      givenDose = null, givenRoute = null,
      bcmaSessionId = null, barcodeVerified = false,
      right6Passed = null, site = null, patientResponse = null,
      holdReason = null,
    } = body;
    if (!pharmOrderId || !patientDfn) {
      return reply.code(400).send({ ok: false, error: "pharmOrderId and patientDfn required" });
  ...
  server.get("/pharmacy/writeback-posture", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    if (!session) return;
