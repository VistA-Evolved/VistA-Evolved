...
const HIGH_ALERT_CLASSES = new Set([
  "anticoagulant", "insulin", "opioid", "neuromuscular_blocker",
  "chemotherapy", "concentrated_electrolyte", "thrombolytic",
  "vasopressor", "sedative", "paralytic",
]);
...
export function createPharmOrder(
  tenantId: string,
  input: Omit<PharmOrder, "id" | "tenantId" | "createdAt" | "updatedAt" | "clinicalChecks" | "requiresStepUp" | "stepUpCompleted" | "verifyingPharmacistDuz" | "verifyingPharmacistName" | "verifiedAt">,
): PharmOrder {
...
export function transitionPharmOrder(
  ...
  const validTransitions: Record<PharmOrderStatus, PharmOrderStatus[]> = {
    ...
    dispensed: ["ready_for_admin", "returned" as any],
...
export function overrideClinicalCheck(
  ...
  const updated: PharmOrder = { ...order, clinicalChecks: checks, updatedAt: new Date().toISOString() };
...
export function updateDispenseStatus(
  ...
  patch?: Partial<Pick<DispenseEvent, "dispensedByDuz" | "dispensedByName" | "checkedByDuz" | "checkedByName" | "deliveryLocation">>,
...
export function listAdminRecords(tenantId: string, patientDfn?: string, pharmOrderId?: string): AdminRecord[] {
