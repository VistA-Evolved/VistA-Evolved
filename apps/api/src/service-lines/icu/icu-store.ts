...
function seedBeds() {
  ...
  for (const u of units) {
    for (let i = 1; i <= u.count; i++) {
      const id = `${u.prefix}${i}`;
      beds.set(id, {
        id, unit: u.unit, bedNumber: `${u.prefix}-${i}`,
        status: "available", monitors: u.monitors,
      });
    }
  }
}
...
export function listAdmissions(opts?: { unit?: string; status?: IcuAdmissionStatus }): IcuAdmission[] {
...
export function addFlowsheetEntry(data: {
  ...
  const entry: FlowsheetEntry = {
    id, admissionId: data.admissionId, category: data.category,
    timestamp: new Date().toISOString(), recordedBy: data.recordedBy,
