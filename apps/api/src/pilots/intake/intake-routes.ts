...
export default async function intakeRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get("/pilots/intakes", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return { ok: true, intakes: listIntakes(session.tenantId, { status: qs.status, partnerType: qs.partnerType }) };
  });
  ...
  server.post("/pilots/intakes/:id/transition", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body || {}) as Record<string, any>;
  ...
  server.post("/pilots/intakes/:id/generate-config", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
