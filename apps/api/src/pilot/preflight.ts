...
function makeCheck(
  ...
  return async (site) => {
    try {
      ...
      return { id, name, severity, status: "fail" as const, message: err.message || "Check threw an error" };
    }
  };
}
...
export async function runPreflightChecks(site: SiteConfig): Promise<PreflightResult> {
  ...
  const criticalFailed = checks.filter((c) => c.severity === "critical" && c.status === "fail").length;
  const totalCritical = checks.filter((c) => c.severity === "critical").length;
  const criticalScore = totalCritical > 0 ? ((totalCritical - criticalFailed) / totalCritical) * 60 : 60;
