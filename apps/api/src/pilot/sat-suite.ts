...
export function startSatRun(
  ...
  const run: SatRun = {
    ...
    summary: { totalScenarios: 0, passed: 0, failed: 0, skipped: 0, manualPending: 0, criticalPassed: 0, criticalFailed: 0, overallScore: 0, verdict: "reject" },
  };
...
export function reportDegradation(
  ...
  const mitigation = mitigationRegistry.find(
    (m) => m.source === source && !m.isActive,
  );
  if (mitigation) {
...
export function resolveDegradation(eventId: string): DegradationEvent | undefined {
  ...
  const mitigation = mitigationRegistry.find(
    (m) => m.source === event.source && m.isActive,
  );
...
function computeSummary(
  ...
  const criticalIds = new Set(
    scenarios.filter((s) => s.severity === "critical").map((s) => s.id),
  );
