...
export function checkBackupPosture(): BackupPosture {
  ...
  gates.push({
    ...
    detail: pgActive ? `Postgres configured (PLATFORM_PG_URL set)` : 'Postgres not configured',
  });
  ...
  gates.push({
    ...
    pass: true,
    detail:
      'VistA, Keycloak, Orthanc, Analytics use Docker volumes (survive down/up, destroyed by -v)',
  });
