...
export function checkCertificationPosture(): CertificationPosture {
  ...
  const composeFiles = ['services/vista/docker-compose.yml', 'docker-compose.prod.yml'];
  ...
  const moduleConfigs = ['config/modules.json', 'config/skus.json', 'config/capabilities.json'];
