...
export async function checkTenantIsolationPosture(): Promise<TenantIsolationPosture> {
  ...
  gates.push({
    ...
    pass: true,
    detail:
      'All tenant queries use try/finally with client.release() -- no pooled leakage [attestation]',
  });
  ...
  if (pgActive) {
    ...
    try {
      ...
      const tableMap = new Map(result.rows.map((r: any) => [r.tablename, r.rowsecurity]));
    ...
    try {
      ...
      const defaultVal =
        res.rows[0]?.current_setting || res.rows[0]?.app?.current_tenant_id || 'unknown';
      gates.push({
