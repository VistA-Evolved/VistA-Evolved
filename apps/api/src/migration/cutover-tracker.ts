...
function createDefaultGates(): CutoverGate[] {
  return [
    ...
    { id: "g1", phase: "pre-validation", name: "recon-clean", description: "All recon discrepancies resolved or accepted", required: true, passed: false },
    { id: "g2", phase: "pre-validation", name: "dual-run-stable", description: "Dual-run match rate >99% for 24h", required: true, passed: false },
    { id: "g3", phase: "pre-validation", name: "backup-verified", description: "Full VistA backup verified and tested", required: true, passed: false },
    { id: "g4", phase: "pre-validation", name: "rollback-tested", description: "Rollback procedure tested successfully", required: true, passed: false },
    // Data-freeze gates
    { id: "g5", phase: "data-freeze", name: "write-freeze", description: "VistA writes paused (read-only mode)", required: true, passed: false },
    { id: "g6", phase: "data-freeze", name: "queue-drained", description: "All pending HL7 messages processed", required: false, passed: false },
    // Final-sync gates
    { id: "g7", phase: "final-sync", name: "delta-sync", description: "Final delta sync completed", required: true, passed: false },
    { id: "g8", phase: "final-sync", name: "count-match", description: "Record counts match within tolerance", required: true, passed: false },
    // Post-validation gates
    { id: "g9", phase: "post-validation", name: "smoke-test", description: "Core workflow smoke tests pass", required: true, passed: false },
    { id: "g10", phase: "post-validation", name: "user-acceptance", description: "Key users validated core functions", required: false, passed: false },
  ];
}
...
export class CutoverTracker {
   ...
   */
  advancePhase(planId: string, targetPhase: CutoverPhase, userId: string): { ok: boolean; error?: string } {
   ...
   */
  updateGate(planId: string, gateId: string, passed: boolean, userId: string, notes?: string): boolean {
    const plan = cutoverPlans.get(planId);
