...
const FHIR_PATIENT: MappingTemplate = {
  ...
  fields: [
    ...
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
    ...
    { source: "address.state", target: "state", required: false, transforms: [{ fn: "uppercase" }] },
...
const FHIR_ALLERGY: MappingTemplate = {
  ...
  fields: [
    ...
    { source: "category", target: "allergyType", required: false, transforms: [{ fn: "lowercase" }] },
    { source: "criticality", target: "severity", required: false, transforms: [{ fn: "uppercase" }] },
    { source: "reaction", target: "reaction", required: false },
    { source: "onsetDateTime", target: "onsetDate", required: false, transforms: [{ fn: "date-iso8601" }] },
  ],
};
...
const FHIR_CONDITION: MappingTemplate = {
  ...
  fields: [
    ...
    { source: "clinicalStatus", target: "status", required: false },
    { source: "onsetDateTime", target: "onsetDate", required: false, transforms: [{ fn: "date-iso8601" }] },
    { source: "abatementDateTime", target: "resolvedDate", required: false, transforms: [{ fn: "date-iso8601" }] },
...
const FHIR_OBSERVATION: MappingTemplate = {
  ...
  entityType: "problem",
  version: "2.0.0",
  description: "Maps FHIR R4 Observation (vital signs, lab results) to the problem entity until a dedicated vitals entity is added",
  fields: [
    { source: "patientRef", target: "patientId", required: true },
    ...
    { source: "effectiveDateTime", target: "onsetDate", required: false, transforms: [{ fn: "date-iso8601" }] },
...
const FHIR_MEDICATION_REQUEST: MappingTemplate = {
  ...
  fields: [
    { source: "patientRef", target: "patientId", required: true },
    { source: "medicationDisplay", target: "drugName", required: true, transforms: [{ fn: "trim" }] },
    { source: "status", target: "status", required: false },
    { source: "intent", target: "orderType", required: false },
    { source: "authoredOn", target: "startDate", required: false, transforms: [{ fn: "date-iso8601" }] },
...
const FHIR_ENCOUNTER: MappingTemplate = {
  ...
  fields: [
    ...
    { source: "status", target: "status", required: false },
    { source: "periodStart", target: "appointmentDate", required: false, transforms: [{ fn: "date-iso8601" }] },
    { source: "periodEnd", target: "endDate", required: false, transforms: [{ fn: "date-iso8601" }] },
...
const FHIR_APPOINTMENT: MappingTemplate = {
  ...
  fields: [
    ...
    { source: "start", target: "appointmentDate", required: true, transforms: [{ fn: "date-iso8601" }] },
...
const EPIC_CCDA_PATIENT: MappingTemplate = {
  ...
  entityType: "patient",
  version: "1.0.0",
  description: "Epic CCD exports using templateId 2.16.840.1.113883.10.20.22.1.2. Uses recordTarget/patientRole. Epic embeds MRN under id with root 1.2.840.114350.1.13.*.",
  fields: [
    { source: "lastName", target: "lastName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "firstName", target: "firstName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
    ...
    { source: "race", target: "race", required: false, description: "Epic includes raceCode in sdtc:raceCode extension" },
...
const EPIC_CCDA_PROBLEM: MappingTemplate = {
  ...
  entityType: "problem",
  version: "1.0.0",
  description: "Epic Problem List section (LOINC 11450-4). Uses SNOMED CT codes in value/@code. Epic may include ICD-10 translations.",
  fields: [
    { source: "patientId", target: "patientId", required: true },
    { source: "icdCode", target: "icdCode", required: false, description: "SNOMED CT or ICD-10 from translation" },
    { source: "problemText", target: "problemText", required: true },
    { source: "status", target: "status", required: false },
    { source: "onsetDate", target: "onsetDate", required: false, transforms: [{ fn: "date-iso8601" }] },
    { source: "resolvedDate", target: "resolvedDate", required: false, transforms: [{ fn: "date-iso8601" }] },
  ],
};
...
const CERNER_CCDA_PATIENT: MappingTemplate = {
  ...
  entityType: "patient",
  version: "1.0.0",
  description: "Cerner Millennium CCD using templateId 2.16.840.1.113883.10.20.22.1.2. Cerner embeds FIN under id with root 2.16.840.1.113883.3.787.0.0.",
  fields: [
    { source: "lastName", target: "lastName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "firstName", target: "firstName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
...
const CERNER_CCDA_ALLERGY: MappingTemplate = {
  ...
  entityType: "allergy",
  version: "1.0.0",
  description: "Cerner Allergy section (LOINC 48765-2). Uses RxNorm codes for drug allergies, UNII for substance allergies.",
  fields: [
    { source: "patientId", target: "patientId", required: true },
    { source: "allergen", target: "allergen", required: true, transforms: [{ fn: "trim" }] },
    { source: "reaction", target: "reaction", required: false },
...
const ATHENA_CCDA_PATIENT: MappingTemplate = {
  ...
  entityType: "patient",
  version: "1.0.0",
  description: "athenahealth CCD export. Uses standard C-CDA demographics. Enterprise ID in id with root 2.16.840.1.113883.3.564.*.",
  fields: [
    { source: "lastName", target: "lastName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "firstName", target: "firstName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
...
const ECW_CCDA_PATIENT: MappingTemplate = {
  ...
  entityType: "patient",
  version: "1.0.0",
  description: "eClinicalWorks CCD export. Non-standard: may omit telecom, uses custom OIDs for facility IDs.",
  fields: [
    { source: "lastName", target: "lastName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "firstName", target: "firstName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
...
const PRACTICEFUSION_CCDA_PATIENT: MappingTemplate = {
  ...
  entityType: "patient",
  version: "1.0.0",
  description: "Practice Fusion CCD export. Standard C-CDA demographics. Practice Fusion uses root OID 2.16.840.1.113883.3.3761.*.",
  fields: [
    { source: "lastName", target: "lastName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "firstName", target: "firstName", required: true, transforms: [{ fn: "uppercase" }] },
    { source: "dateOfBirth", target: "dateOfBirth", required: true, transforms: [{ fn: "date-iso8601" }] },
