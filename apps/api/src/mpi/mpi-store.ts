...
export function listIdentities(
  tenantId: string,
  opts?: { goldenRecordId?: string }
): MpiPatientIdentity[] {
  let results = Array.from(identityStore.values()).filter((i) => i.tenantId === tenantId);
  if (opts?.goldenRecordId)
    results = results.filter((i) => i.goldenRecordId === opts.goldenRecordId);
  return results;
}

export function updateIdentity(
  id: string,
  patch: Partial<MpiPatientIdentity>
): MpiPatientIdentity | undefined {
...
export function findMatches(
  ...
  for (const c of candidates) {
    ...
    if (c.familyName.toLowerCase() === query.familyName.toLowerCase()) {
      matchedFields.push('familyName');
      score += 15;
    }
    if (c.givenName.toLowerCase() === query.givenName.toLowerCase()) {
      matchedFields.push('givenName');
      score += 10;
    }
    if (c.dateOfBirth === query.dateOfBirth) {
...
export function mergeIdentities(
  ...
  if (action === 'merge' || action === 'link') {
    ...
    updateIdentity(survivorId, {
      identifiers: mergedIds,
      goldenRecordId: survivor.goldenRecordId || survivorId,
    });
