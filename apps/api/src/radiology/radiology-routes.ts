...
export async function radiologyRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post("/radiology/orders/:id/transition", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
  ...
  server.post("/radiology/orders/:id/protocol", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.protocolName) return reply.code(400).send({ ok: false, error: "protocolName required" });
    const result = assignProtocol(id, String(body.protocolName), session.duz);
    if (!result.ok) return reply.code(400).send(result);
    return { ok: true, order: result.order };
  });

  server.post("/radiology/orders/:id/link-mwl", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.mwlWorklistItemId) return reply.code(400).send({ ok: false, error: "mwlWorklistItemId required" });
    const result = linkMwlToRadOrder(id, String(body.mwlWorklistItemId));
    if (!result.ok) return reply.code(400).send(result);
    return { ok: true };
  });

  server.post("/radiology/orders/:id/link-mpps", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.mppsRecordId) return reply.code(400).send({ ok: false, error: "mppsRecordId required" });
  ...
  server.get("/radiology/reading-worklist", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const q = (request.query ?? {}) as Record<string, string>;
  ...
  server.post("/radiology/reading-worklist", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.radOrderId || !body.studyInstanceUid || !body.accessionNumber || !body.modality || !body.procedureName) {
      return reply.code(400).send({
        ok: false,
        error: "radOrderId, studyInstanceUid, accessionNumber, modality, and procedureName required",
      });
    }
  ...
  server.post("/radiology/reading-worklist/:id/assign", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
  ...
  server.post("/radiology/reading-worklist/:id/transition", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.status) return reply.code(400).send({ ok: false, error: "status required" });
  ...
  server.post("/radiology/reports", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.radOrderId || !body.readingWorklistItemId || !body.studyInstanceUid || !body.accessionNumber || !body.findings || !body.impression) {
      return reply.code(400).send({
        ok: false,
        error: "radOrderId, readingWorklistItemId, studyInstanceUid, accessionNumber, findings, and impression required",
      });
    }
  ...
  server.post("/radiology/reports/:id/transition", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
    if (!body.status) return reply.code(400).send({ ok: false, error: "status required" });
  ...
  server.get("/radiology/dose-registry/patient-cumulative", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const q = (request.query ?? {}) as Record<string, string>;
  ...
  server.post("/radiology/critical-alerts", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const body = (request.body as Record<string, unknown>) ?? {};
    ...
    const alert = createRadCriticalAlert({
      ...
      notifyProviderName: body.notifyProviderName ? String(body.notifyProviderName) : (session.userName ?? session.duz),
    });
    return reply.code(201).send({ ok: true, alert });
  });

  server.post("/radiology/critical-alerts/:id/communicate", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
    const body = (request.body as Record<string, unknown>) ?? {};
  ...
  server.post("/radiology/critical-alerts/:id/ack", async (request: FastifyRequest, reply: FastifyReply) => {
    const session = await requireSession(request, reply);
    const { id } = request.params as { id: string };
  ...
  server.post("/radiology/critical-alerts/:id/resolve", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
    const { id } = request.params as { id: string };
  ...
  server.get("/radiology/writeback-posture", async (request: FastifyRequest, reply: FastifyReply) => {
    await requireSession(request, reply);
