...
export class VistaClinicalAdapter implements ClinicalEngineAdapter {
  ...
  async getNotes(dfn: string): Promise<AdapterResult<NoteRecord[]>> {
    try {
      const { safeCallRpc } = await import('../../lib/rpc-resilience.js');
      const rawLines = await safeCallRpc('TIU DOCUMENTS BY CONTEXT', [
        dfn,
        '1',
        '0',
        '0',
  ...
  async getEncounters(dfn: string): Promise<AdapterResult<EncounterRecord[]>> {
    try {
      ...
      try {
        ...
        /* ignore scheduling fallback errors */
      }
      return {
        ok: false,
        pending: true,
  ...
  async getReportText(dfn: string, reportId: string): Promise<AdapterResult<string>> {
    try {
      const { safeCallRpc } = await import('../../lib/rpc-resilience.js');
      const rawLines = await safeCallRpc('ORWRP REPORT TEXT', [
        dfn,
        reportId,
        '',
        '',
    ...
    try {
      ...
      return {
        ...
        data: {
          success: !failed,
          ien: failed ? undefined : ien,
          message: failed ? result : 'Allergy saved',
        },
    ...
    try {
      ...
      return {
        ...
        data: {
          success: !failed,
          ien: failed ? undefined : ien,
          message: failed ? result : 'Problem added',
        },
  ...
  async admitPatient(_request: AdmitRequest): Promise<AdapterResult<WriteResult>> {
    return {
      ...
      vistaGrounding: {
        ...
        vistaPackage: 'DG',
        vistaFiles: ['405', '2'],
        sandboxNote: 'Not available in WorldVistA Docker',
  ...
  async getMAR(
    ...
    return {
      ...
      vistaGrounding: {
        ...
        sandboxNote:
          'PSB package not installed in WorldVistA Docker. eMAR schedule endpoint uses heuristic due-time derivation from ORWPS ACTIVE sig text as interim.',
        migrationPath:
          'Install PSB (BCMA) package -> wire PSB MED LOG for real MAR data with actual admin times, nurse attestation, and witness tracking',
      },
    };
  }
  ...
  async recordAdministration(_request: MedAdminRequest): Promise<AdapterResult<WriteResult>> {
    return {
      ...
      vistaGrounding: {
        ...
        vistaFiles: ['53.79'],
        sandboxNote:
          'PSB MED LOG is a bidirectional RPC: reads admin history and writes new admin events. Write mode requires action code + order IEN + datetime + administered-by DUZ.',
        migrationPath:
          'Install PSB package -> wire PSB MED LOG write mode -> add witness validation -> integrate 5-rights barcode verification',
      },
    };
  }
  ...
  async scanBarcode(
    _barcode: string,
    _patientDfn?: string
  ): Promise<AdapterResult<BarcodeScanResult>> {
    return {
      ok: false,
      pending: true,
      target: 'PSJBCMA',
      ...
      vistaGrounding: {
        ...
        vistaFiles: ['53.45', '50'],
        sandboxNote:
          'PSJBCMA resolves NDC/UPC barcodes to drug IEN + order IEN. Also validates medication-patient match. Required for BCMA 5-rights verification.',
        migrationPath:
          'Install PSJ + PSB packages -> wire PSJBCMA -> integrate with formulary (File 50) -> add NDC cross-reference (File 50.67)',
      },
    };
  }
  ...
  async getAdminHistory(_dfn: string, _orderIen?: string): Promise<AdapterResult<MAREntry[]>> {
    return {
      ...
      vistaGrounding: {
        ...
        vistaFiles: ['53.79'],
        sandboxNote:
          'PSB MED LOG read mode returns chronological admin events: who gave what, when, witnessed by whom, refused/held reasons.',
        migrationPath:
          'Install PSB package -> wire PSB MED LOG read mode -> parse multi-line grouped records (similar to ORWPS ACTIVE ~prefix pattern)',
      },
    };
  }
  ...
  async verifyOrder(_request: PharmacyVerifyRequest): Promise<AdapterResult<PharmacyVerifyResult>> {
    return {
      ...
      vistaGrounding: {
        ...
        vistaFiles: ['53.1', '55'],
        sandboxNote:
          'PSJ VERIFY marks an inpatient pharmacy order as pharmacist-verified. Until verified, unit dose orders cannot be dispensed. IV orders require separate IV room verification.',
        migrationPath:
          'Install PSJ package -> wire PSJ VERIFY -> add pharmacy action profile (File 53.1) -> integrate with dispensing workflow',
      },
    };
  }
}
