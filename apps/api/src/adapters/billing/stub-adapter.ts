...
export class StubBillingAdapter implements BillingAdapter {
  ...
  async getClaims(): Promise<AdapterResult<Claim[]>> { return STUB; }
  async submitClaim(): Promise<AdapterResult<Claim>> { return STUB; }
  async getEOB(): Promise<AdapterResult<EOB>> { return STUB; }
  async getEligibility(): Promise<AdapterResult<EligibilityResult>> { return STUB; }
}
