...
async function testReportViewer() {
  ...
  if (hasPending) {
    ...
    assert(
      "pendingTarget references File #74",
      body.pendingTarget.vistaFile.includes("74"),
    );
  }

  if (hasReport) {
    assert("reportText is a non-empty string", typeof body.reportText === "string" && body.reportText.length > 0);
...
async function testViewerLink() {
  ...
  assert("viewer response has message", typeof body?.message === "string" && body.message.length > 0);
  ...
  if (body?.viewerAvailable) {
    ...
    assert("viewerType is ohif or basic", body.viewerType === "ohif" || body.viewerType === "basic");
  } else {
...
async function testInvalidStudyId() {
  ...
  const { status: reportStatus, body: reportBody } = await fetchJson("/imaging/report/NONEXISTENT-999");
  assert("non-existent report returns 200 (graceful)", reportStatus === 200);
  assert("non-existent report has pendingTarget", reportBody?.pendingTarget != null || reportBody?.available === false);

  // Viewer link for non-existent study (should still return instructions)
  const { status: viewerStatus, body: viewerBody } = await fetchJson("/imaging/viewer-link/NONEXISTENT-999");
...
async function testViewerLinkConfigured() {
  ...
  if (body?.viewerAvailable && body?.url) {
    assert("configured viewer URL contains StudyInstanceUIDs", body.url.includes("StudyInstanceUIDs=") || body.url.includes(uid));
