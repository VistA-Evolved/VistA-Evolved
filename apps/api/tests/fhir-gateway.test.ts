...
describe('FHIR R4 Gateway — Phase 178', () => {
  ...
  describe('MedicationRequest mapper', () => {
    ...
    it('maps status values', () => {
      ...
      expect(toFhirMedicationRequest({ ...MED, status: 'DISCONTINUED' }, '3').status).toBe(
        'stopped'
      );
  ...
  describe('Encounter mapper', () => {
    ...
    it('maps various encounter statuses', () => {
      expect(
        (toFhirEncounter({ ...ENCOUNTER, status: 'CHECKED OUT' }) as FhirEncounter).status
      ).toBe('finished');
      expect(
        (toFhirEncounter({ ...ENCOUNTER, status: 'CHECKED IN' }) as FhirEncounter).status
      ).toBe('in-progress');
      expect((toFhirEncounter({ ...ENCOUNTER, status: 'SCHEDULED' }) as FhirEncounter).status).toBe(
        'planned'
      );
      expect((toFhirEncounter({ ...ENCOUNTER, status: 'CANCELLED' }) as FhirEncounter).status).toBe(
        'cancelled'
      );
      expect((toFhirEncounter({ ...ENCOUNTER, status: '' }) as FhirEncounter).status).toBe(
        'unknown'
      );
    });

    it('maps encounter class codes', () => {
      expect((toFhirEncounter({ ...ENCOUNTER, class: 'IMP' }) as FhirEncounter).class.code).toBe(
        'IMP'
      );
      expect((toFhirEncounter({ ...ENCOUNTER, class: 'EMER' }) as FhirEncounter).class.code).toBe(
        'EMER'
      );
      expect(
        (toFhirEncounter({ ...ENCOUNTER, class: 'INPATIENT' }) as FhirEncounter).class.code
      ).toBe('IMP');
      expect(
        (toFhirEncounter({ ...ENCOUNTER, class: 'EMERGENCY' }) as FhirEncounter).class.code
      ).toBe('EMER');
    });
