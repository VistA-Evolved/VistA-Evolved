...
describe("Onboarding UX v2 -- Phase 262", () => {
  describe("Integration Steps Store", () => {
    const storePath = path.join(
      API_SRC,
      "config",
      "onboarding-integration-steps.ts",
    );
  ...
  describe("Integration Routes", () => {
    const routePath = path.join(
      API_SRC,
      "routes",
      "onboarding-integration-routes.ts",
    );
  ...
  describe("Base Onboarding Untouched", () => {
    it("onboarding-store.ts preserved", () => {
      expect(
        fs.existsSync(
          path.join(API_SRC, "config", "onboarding-store.ts"),
        ),
      ).toBe(true);
    });
    ...
    it("onboarding-routes.ts preserved", () => {
      expect(
        fs.existsSync(
          path.join(API_SRC, "routes", "onboarding-routes.ts"),
        ),
      ).toBe(true);
    });
  });
