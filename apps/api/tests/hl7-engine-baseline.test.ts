...
describe("Tenant Endpoint Configuration", () => {
  ...
  it("exports resolveInboundEndpoint", () => {
    expect(fileContains("apps/api/src/hl7/tenant-endpoints.ts", "resolveInboundEndpoint")).toBe(true);
  });
...
describe("PHI Safety", () => {
  it("no raw HL7 in application log calls", () => {
    ...
    const content = fs.readFileSync(
      path.join(ROOT, "apps/api/src/hl7/mllp-server.ts"),
      "utf-8",
    );
  ...
  it("tenant endpoints default PHI logging to false", () => {
    expect(fileContains(
      "apps/api/src/hl7/tenant-endpoints.ts",
