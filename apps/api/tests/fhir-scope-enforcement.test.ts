...
describe("checkScopeAccess", () => {
  it("grants access with exact resource scope", () => {
    const result = checkScopeAccess(
      ["openid", "profile", "patient/Patient.read"],
      "Patient",
    );
  ...
  it("prefers exact resource match over wildcard", () => {
    const result = checkScopeAccess(
      ["user/*.read", "patient/Patient.read"],
      "Patient",
    );
    expect(result.granted).toBe(true);
...
describe("enforceFhirScope", () => {
  function makeMockReply() {
    ...
    const reply: any = {
      ...
      header() { return reply; },
      send(body: any) { sent.body = body; reply.sent = true; return reply; },
    };
    return { reply, sent };
  }
  ...
  it("allows session-authenticated users unconditionally", () => {
    const request: any = {
      fhirPrincipal: {
        authMethod: "session",
        sub: "87", duz: "87", userName: "PROVIDER,CLYDE",
        roles: ["admin"], scopes: ["user/*.read"], tenantId: "default",
      },
    };
  ...
  it("allows bearer with matching patient scope", () => {
    const request: any = {
      fhirPrincipal: {
        authMethod: "bearer",
        sub: "u-1", duz: "87", userName: "PROVIDER",
        roles: ["provider"],
