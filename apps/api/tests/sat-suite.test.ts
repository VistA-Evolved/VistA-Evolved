...
describe("Pilot Hospital Hardening Pack -- Phase 265", () => {
  ...
  describe("Existing Pilot Infrastructure Preserved", () => {
    it("site-config.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "pilot", "site-config.ts")),
      ).toBe(true);
    });

    it("preflight.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "pilot", "preflight.ts")),
      ).toBe(true);
    });

    it("pilot-routes.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "routes", "pilot-routes.ts")),
      ).toBe(true);
    });
  });
});
