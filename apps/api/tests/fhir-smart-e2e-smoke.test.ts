  ...
  describe("Full pipeline: discovery -> metadata -> auth -> scope -> search -> page", () => {
    it("all Wave 5 modules are importable and functional", () => {
      ...
      const principal = principalFromSession({
        id: "s1", duz: "87", userName: "PROVIDER,CLYDE WV",
        roles: ["provider"], displayName: "C", tenantId: "default",
        loggedInAt: Date.now(),
      } as any);
      expect(principal.authMethod).toBe("session");
      ...
      const encounters = [{ id: "1", patientDfn: "3", dateTime: "3240601", status: "CHECKED OUT" }] as any[];
      const filtered = filterEncounters(encounters, {});
      expect(filtered.length).toBe(1);

      // Step 6: Paging
      const patient: PatientRecord = {
        dfn: "3", name: "CARTER,DAVID JR", ssn: "", dob: "", sex: "M", veteran: true,
      };
      const bundle = toPagedSearchBundle(
