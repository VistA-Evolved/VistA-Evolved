...
describe('HL7v2 Message Pipeline -- Phase 259', () => {
  ...
  describe('Enhanced Dead-Letter Queue', () => {
    ...
    it('stores raw messages for replay', () => {
      const content = fs.readFileSync(path.join(HL7_DIR, 'dead-letter-enhanced.ts'), 'utf-8');
  ...
  describe('Pipeline Routes', () => {
    it('hl7-pipeline.ts routes exist', () => {
      expect(fs.existsSync(path.join(API_SRC, 'routes', 'hl7-pipeline.ts'))).toBe(true);
    });

    it('has event query endpoint', () => {
      const content = fs.readFileSync(path.join(API_SRC, 'routes', 'hl7-pipeline.ts'), 'utf-8');
  ...
  describe('Store Policy Registration', () => {
    it('store-policy.ts registers HL7 stores', () => {
      const content = fs.readFileSync(path.join(API_SRC, 'platform', 'store-policy.ts'), 'utf-8');
