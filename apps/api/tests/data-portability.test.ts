...
describe('Data Portability Exports -- Phase 264', () => {
  ...
  describe('Data Portability Routes', () => {
    const routePath = path.join(API_SRC, 'routes', 'data-portability-routes.ts');
  ...
  describe('Existing Export Infrastructure Untouched', () => {
    it('export-engine.ts preserved', () => {
      expect(fs.existsSync(path.join(API_SRC, 'exports', 'export-engine.ts'))).toBe(true);
    });

    it('export-formats.ts preserved', () => {
      expect(fs.existsSync(path.join(API_SRC, 'exports', 'export-formats.ts'))).toBe(true);
    });

    it('fhir/types.ts preserved', () => {
      expect(fs.existsSync(path.join(API_SRC, 'fhir', 'types.ts'))).toBe(true);
    });
  });
});
