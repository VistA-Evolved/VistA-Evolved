...
describe('API Contract Verification', () => {
  ...
  describe('FHIR R4 Contracts', () => {
    ...
    it('includes SMART discovery', () => {
      const smart = fhirContracts.find((c) => c.path.includes('smart-configuration'));
    ...
    for (const rt of fhirResourceTypes) {
      it(`has contract for /fhir/${rt}`, () => {
        const contract = fhirContracts.find((c) => c.path === `/fhir/${rt}`);
    ...
    it('all FHIR search endpoints return Bundle shape', () => {
      const searches = fhirContracts.filter(
        (c) =>
          c.path.startsWith('/fhir/') && c.path !== '/fhir/metadata' && !c.path.includes('smart')
      );
  ...
  describe('Auth Flow', () => {
    it('has login endpoint', () => {
      const login = ROUTE_CONTRACTS.find((c) => c.path === '/auth/login' && c.method === 'POST');
    ...
    it('has session endpoint', () => {
      const session = ROUTE_CONTRACTS.find((c) => c.path === '/auth/session' && c.method === 'GET');
      expect(session).toBeDefined();
      expect(session!.auth).toBe('session');
    });

    it('has logout endpoint', () => {
      const logout = ROUTE_CONTRACTS.find((c) => c.path === '/auth/logout' && c.method === 'POST');
