...
describe("Support Tooling v2 -- Phase 263", () => {
  ...
  describe("Base Support Module Untouched", () => {
    it("diagnostics.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "support", "diagnostics.ts")),
      ).toBe(true);
    });

    it("ticket-store.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "support", "ticket-store.ts")),
      ).toBe(true);
    });

    it("support-routes.ts preserved", () => {
      expect(
        fs.existsSync(path.join(API_SRC, "routes", "support-routes.ts")),
      ).toBe(true);
    });
  });
});
