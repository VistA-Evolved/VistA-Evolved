...
describe("Resilience Certification — Phase 254", () => {
  ...
  describe("Health vs Readiness Endpoints", () => {
    ...
    it("/ready checks circuit breaker or VistA state", () => {
      expect(combined).toMatch(
        /circuitBreaker|cbState|probeConnect|vista.*reachable/i
      );
    });
  });
  ...
  describe("Posture Endpoints", () => {
    ...
    it("has observability posture", () => {
      expect(
        existsSync(join(postureDir, "observability-posture.ts"))
      ).toBe(true);
    });
    ...
    it("has data-plane posture", () => {
      expect(
        existsSync(join(postureDir, "data-plane-posture.ts"))
      ).toBe(true);
    });
  });
  ...
  describe("Backup & Recovery", () => {
    ...
    it("DR backup script exists", () => {
      ...
      const drBackup =
        fileExists("scripts/dr/backup.mjs") ||
  ...
  describe("Drill Infrastructure", () => {
    ...
    it("circuit-breaker drill script exists", () => {
      expect(
        fileExists("ops/drills/run-circuit-breaker-drill.ps1")
      ).toBe(true);
    });

    it("health-readiness drill script exists", () => {
      expect(
        fileExists("ops/drills/run-health-readiness-drill.ps1")
      ).toBe(true);
    });

    it("posture audit drill script exists", () => {
      expect(
        fileExists("ops/drills/run-posture-audit-drill.ps1")
      ).toBe(true);
    });

    it("defines 5+ drill scenarios", () => {
      const src = readFileSync(
        join(ROOT, "ops/drills/resilience-drills.ts"),
        "utf-8"
      );
      const drillCount = (src.match(/ResilienceDrill\s*=\s*{/g) || [])
        .length;
