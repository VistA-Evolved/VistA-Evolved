    ...
    },
    {
      ...
      fn: async () => {
        ...
        return {
          ...
          detail: `${d.reports?.length ?? 0} report type(s)`,
        };
      },
    },
...
export default function CPRSVerifyPage() {
  ...
  const runChecks = useCallback(async () => {
    setRunning(true);
    const updated: Check[] = CHECKS.map((c) => ({
      id: c.id,
      label: c.label,
          ...
          <tbody>
            {checks.map((c, i) => (
              <tr key={c.id}>
                ...
                <td>
                  {c.status === 'pass' && (
                    <span className={`${styles.badge} ${styles.active}`}>PASS</span>
                  )}
                  {c.status === 'fail' && (
                    <span className={`${styles.badge} ${styles.inactive}`}>FAIL</span>
                  )}
                  {c.status === 'pending' && (
                    <span className={`${styles.badge} ${styles.draft}`}>...</span>
                  )}
                  {c.status === 'warn' && (
                    <span className={`${styles.badge} ${styles.unsigned}`}>WARN</span>
                  )}
                </td>
