...
const colors = {
  ...
  textMuted: '#718096',
  pendingBg: '#fffbeb',
  pendingBorder: '#f6e05e',
  draftBg: '#ebf8ff',
  draftBorder: '#90cdf4',
  acceptedBg: '#f0fff4',
  acceptedBorder: '#9ae6b4',
};
...
function RiskFlagBadge({ flag }: { flag: RiskFlag }) {
  ...
  const colorMap: Record<string, string> = {
    falls: '#e53e3e',
    isolation: '#d69e2e',
    'critical-labs': '#e53e3e',
    'code-status': '#805ad5',
    restraints: '#dd6b20',
    'suicide-precautions': '#e53e3e',
    other: '#718096',
  };
  return (
    <span
      style={{
        background: colorMap[flag.type] || '#718096',
        color: '#fff',
        borderRadius: 4,
        padding: '2px 6px',
        fontSize: 11,
...
function ActiveTab({ ward }: { ward: string }) {
  ...
  const loadReports = useCallback(() => {
    ...
    apiFetch(`/handoff/reports?ward=${encodeURIComponent(ward)}`)
      .then((data) => {
        if (data.ok)
          setReports((data.reports || []).filter((r: HandoffSummary) => r.status !== 'archived'));
  ...
  const handleAction = useCallback(
    async (id: string, action: 'submit' | 'archive') => {
      setActionMsg(null);
      try {
        ...
        if (result.ok) {
          setActionMsg({
            ok: true,
            msg: `Report ${action === 'submit' ? 'submitted' : 'archived'} successfully`,
          });
          loadReports();
        } else {
  ...
  if (loading)
    return <div style={{ padding: 20, color: colors.textMuted }}>Loading handoff reports...</div>;
          ...
          {reports.map((r) => (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: colors.textMuted }}>
                Created by: {r.createdBy.name} | Patients: {r.patientCount} |{' '}
...
function CreateTab({ ward }: { ward: string }) {
    ...
    setPatientEntries((prev) =>
      prev.map((p, i) => {
        ...
        return {
          ...
          todos: [
            ...p.todos,
            { id: `new-${Date.now()}`, text: '', completed: false, priority: 'normal' as const },
          ],
        };
      })
    );
  ...
  const updateTodo = useCallback(
    (patientIdx: number, todoIdx: number, updates: Partial<TodoItem>) => {
      setPatientEntries((prev) =>
        prev.map((p, i) => {
          if (i !== patientIdx) return p;
  ...
  const toggleRiskFlag = useCallback((patientIdx: number, flagType: string) => {
    setPatientEntries((prev) =>
      prev.map((p, i) => {
        ...
        return {
          ...p,
          riskFlags: p.riskFlags.map((f) =>
            f.type === flagType ? { ...f, active: !f.active } : f
          ),
        };
      })
    );
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          <div>
            <label htmlFor="shift-label" style={labelStyle}>
              Shift Label:
            </label>
            <select
              id="shift-label"
          ...
          <div>
            <label htmlFor="shift-start" style={labelStyle}>
              Shift Start:
            </label>
            <input
              id="shift-start"
              type="datetime-local"
              value={shiftStart.replace(':00Z', '').replace('Z', '')}
              onChange={(e) => setShiftStart(e.target.value)}
              style={inputStyle}
            />
          </div>
          <div>
            <label htmlFor="shift-end" style={labelStyle}>
              Shift End:
            </label>
            <input
              id="shift-end"
              type="datetime-local"
              value={shiftEnd.replace(':00Z', '').replace('Z', '')}
        ...
        <div style={{ marginTop: 12 }}>
          <label htmlFor="shift-notes" style={labelStyle}>
            Global Shift Notes:
          </label>
          <textarea
            id="shift-notes"
            ...
            rows={2}
            placeholder="Shift-wide notes (staffing, equipment issues, etc.)"
            style={textareaStyle}
          />
        </div>
      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {(['situation', 'background', 'assessment', 'recommendation'] as const).map(
                  (field) => {
                    ...
                    return (
                      <div key={field}>
                        <label htmlFor={`sbar-${field}`} style={labelStyle}>
                          {fieldLabels[field]}
                        </label>
                        <textarea
                          id={`sbar-${field}`}
                ...
                {/* Nursing notes */}
                <div>
                  <label htmlFor="nursing-notes" style={labelStyle}>
                    Nursing Notes:
                  </label>
                  <textarea
                    id="nursing-notes"
                    value={selected.nursingNotes}
                    onChange={(e) =>
                      updatePatientEntry(selectedPatientIdx!, { nursingNotes: e.target.value })
                    }
                    rows={2}
                    style={textareaStyle}
                    placeholder="Free-text nursing notes for handoff..."
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <button
                      onClick={() => addTodo(selectedPatientIdx!)}
                      ...
                      {selected.todos.map((todo, ti) => (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) =>
                              updateTodo(selectedPatientIdx!, ti, { completed: e.target.checked })
                            }
                          ...
                          <input
                            ...
                            value={todo.text}
                            onChange={(e) =>
                              updateTodo(selectedPatientIdx!, ti, { text: e.target.value })
                            }
                          ...
                          <select
                            value={todo.priority}
                            onChange={(e) =>
                              updateTodo(selectedPatientIdx!, ti, {
                          ...
                          <button
                            onClick={() => removeTodo(selectedPatientIdx!, ti)}
                            style={{
...
function AcceptTab({ ward }: { ward: string }) {
  ...
  if (loading)
    return (
      <div style={{ padding: 20, color: colors.textMuted }}>Loading submitted handoffs...</div>
    );
  if (error) return <div style={{ padding: 20, color: colors.danger }}>{error}</div>;
            ...
            {!selectedReport ? (
              ...
              <div>
                ...
                >
                  <div>
                    <h4 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>
                      {selectedReport.shiftLabel}
                    </h4>
                    <div style={{ fontSize: 12, color: colors.textMuted }}>
                      From: {selectedReport.createdBy.name} | {selectedReport.patients.length}{' '}
                      patients
                    </div>
                  </div>
                  ...
                  {selectedReport.status === 'accepted' && <StatusBadge status="accepted" />}
                </div>
                          ...
                          <div key={field} style={{ marginBottom: 6 }}>
                            ...
                            <div
                              style={{
                                ...
                                borderLeft: `2px solid ${colors.draftBorder}`,
                              }}
                            >
                              {val}
                            </div>
...
function ArchiveTab({ ward }: { ward: string }) {
  ...
  if (loading)
    return <div style={{ padding: 20, color: colors.textMuted }}>Loading archive...</div>;
  if (error) return <div style={{ padding: 20, color: colors.danger }}>{error}</div>;
...
function HandoffPageContent() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg }}>
      ...
      >
        <label htmlFor="ward-select" style={{ fontWeight: 600 }}>
          Ward/Service:
        </label>
        <input
          id="ward-select"
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') handleWardChange();
          }}
...
export default function HandoffPage() {
  return (
    <Suspense
      fallback={<div style={{ padding: 40, textAlign: 'center' }}>Loading Shift Handoff...</div>}
    >
      <HandoffPageContent />
    </Suspense>
  );
