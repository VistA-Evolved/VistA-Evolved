...
export default function SchedulingPage() {
  ...
  const loadSchedule = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch(
        `/scheduling/appointments/range?startDate=${start}&endDate=${end}`
      );
    ...
    async (id: string, action: 'approve' | 'reject') => {
      ...
      try {
        await apiFetch(`/scheduling/requests/${id}/${action}`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            reason: action === 'reject' ? 'Rejected by scheduling staff' : undefined,
          }),
        });
        await loadRequests();
      } catch (err: any) {
  ...
  const quickCheckin = useCallback(
    async (apptRef: string, dfn: string, clinic: string, clinicIen?: string) => {
      setError('');
      try {
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 1200, margin: '0 auto' }}>
                ...
                {encounters.map((enc: any, i: number) => (
                  <tr key={enc.id || i} style={{ borderBottom: '1px solid #e9ecef' }}>
                    <td style={{ padding: '0.5rem' }}>
                      {enc.dateTime ? new Date(enc.dateTime).toLocaleTimeString() : '-'}
                    </td>
        ...
        <div>
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '0.375rem 0.75rem',
                background: patientDfn ? '#0d6efd' : '#6c757d',
                color: '#fff',
                border: 'none',
                borderRadius: 4,
                cursor: 'pointer',
            ...
            <button
              ...
              style={{
                padding: '0.375rem 0.75rem',
                background: patientDfn ? '#198754' : '#6c757d',
                color: '#fff',
                border: 'none',
                borderRadius: 4,
                cursor: 'pointer',
          ...
          {showReqForm && patientDfn && (
            ...
            >
              <h4 style={{ margin: '0 0 0.5rem', fontSize: '0.875rem' }}>
                New Appointment Request for DFN {patientDfn}
              </h4>
              <div
                style={{
                  display: 'grid',
              ...
              <button
                ...
                disabled={submitting || !reqClinic || !reqDate || !reqReason}
                style={{
                  padding: '0.375rem 0.75rem',
                  background:
                    submitting || !reqClinic || !reqDate || !reqReason ? '#6c757d' : '#198754',
                  color: '#fff',
                  border: 'none',
                  borderRadius: 4,
                  cursor: 'pointer',
          ...
          {patientAppts.length === 0 ? (
            <p style={{ color: '#6c757d', fontStyle: 'italic' }}>
              No appointments found. Enter a patient DFN and search.
            </p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
              ...
              <tbody>
                {patientAppts.map((a: any, i: number) => (
                  <tr key={a.id || i} style={{ borderBottom: '1px solid #e9ecef' }}>
                    ...
                    <td style={{ padding: '0.5rem' }}>
                      <span
                        style={{
                          ...
                          fontSize: '0.75rem',
                          background:
                            a.status?.includes('pending') || a.status?.includes('request')
                              ? '#fff3cd'
                              : '#d1e7dd',
                          color:
                            a.status?.includes('pending') || a.status?.includes('request')
                    ...
                    <td style={{ padding: '0.5rem', fontSize: '0.75rem', color: '#6c757d' }}>
                      {a.source || 'local'}
                    </td>
                  </tr>
                ))}
                    ...
                    <td style={{ padding: '0.5rem' }}>
                      {r.status === 'pending' && (
                        <span style={{ display: 'flex', gap: '0.25rem' }}>
                          <button
                            onClick={() => triageRequest(r.id, 'approve')}
                            style={{
                          ...
                          <button
                            onClick={() => triageRequest(r.id, 'reject')}
                            style={{
                              padding: '0.2rem 0.5rem',
                              background: '#dc3545',
                              color: '#fff',
      ...
      {tab === 'lifecycle' && !loading && (
        <div>
          <p style={{ fontSize: '0.875rem', color: '#6c757d', marginBottom: '1rem' }}>
            Appointment lifecycle state transitions tracked in Postgres. States: requested &rarr;
            waitlisted &rarr; booked &rarr; checked_in &rarr; completed | cancelled | no_show
          </p>
            ...
            >
              ...
              {lifecycleStats.byState && Object.keys(lifecycleStats.byState).length > 0 && (
                <span>
                  {' '}
                  |{' '}
                  {Object.entries(lifecycleStats.byState)
                    .map(([s, c]) => `${s}: ${c}`)
                    .join(', ')}
                </span>
              )}
          ...
          {lifecycleEntries.length === 0 ? (
            <p style={{ fontStyle: 'italic', color: '#6c757d' }}>
              {lifecycleDfn
                ? 'No lifecycle entries for this patient.'
                : 'No lifecycle entries recorded yet. Use POST /scheduling/lifecycle/transition to record state changes.'}
            </p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
              <thead>
              ...
              <tbody>
                {lifecycleEntries.map((e: any) => (
                  <tr key={e.id} style={{ borderBottom: '1px solid #e9ecef' }}>
                    <td style={{ padding: '0.375rem', fontSize: '0.75rem' }}>
                      {e.createdAt ? new Date(e.createdAt).toLocaleString() : '-'}
                    </td>
                    <td
                      style={{ padding: '0.375rem', fontFamily: 'monospace', fontSize: '0.7rem' }}
                    >
                      {e.appointmentRef?.slice(0, 12) || '-'}
                    </td>
                    <td style={{ padding: '0.375rem' }}>{e.clinicName || '-'}</td>
                    <td style={{ padding: '0.375rem' }}>
                      {e.previousState && (
                        <span style={{ color: '#6c757d' }}>{e.previousState} &rarr; </span>
                      )}
                      <span
                        style={{
                    ...
                    <td style={{ padding: '0.375rem', fontSize: '0.7rem', color: '#6c757d' }}>
                      {e.rpcUsed || '-'}
                    </td>
                    <td style={{ padding: '0.375rem', fontSize: '0.75rem' }}>
                      {e.transitionNote || '-'}
                    ...
                    <td style={{ padding: '0.375rem' }}>
                      {e.state === 'booked' && (
                        <button
                          onClick={() =>
                            quickCheckin(e.appointmentRef, e.patientDfn, e.clinicName, e.clinicIen)
                          }
                          style={{
                      ...
                      {e.state === 'checked_in' && (
                        <button
                          onClick={() =>
                            quickCheckout(e.appointmentRef, e.patientDfn, e.clinicName, e.clinicIen)
                          }
                          style={{
                            padding: '0.15rem 0.4rem',
                            background: '#198754',
      ...
      {tab === 'posture' && !loading && (
        <div>
          <p style={{ fontSize: '0.875rem', color: '#6c757d', marginBottom: '1rem' }}>
            VistA scheduling RPC availability matrix -- which RPCs exist and are callable in this
            sandbox.
          </p>
              ...
              <tbody>
                {postureEntries.map((p: any, i: number) => (
                  <tr key={i} style={{ borderBottom: '1px solid #e9ecef' }}>
                    <td
                      style={{ padding: '0.375rem', fontFamily: 'monospace', fontSize: '0.75rem' }}
                    >
                      {p.rpc}
                    </td>
                    ...
                    <td style={{ padding: '0.375rem', fontSize: '0.75rem', color: '#6c757d' }}>
                      {p.sandboxNote}
                    </td>
                  </tr>
                ))}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                <div>
                  <h4 style={{ fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                    Priorities ({referenceData.priorities?.length || 0})
                  </h4>
                  {(referenceData.priorities || []).length === 0 ? (
                    <p style={{ fontStyle: 'italic', color: '#6c757d', fontSize: '0.8rem' }}>
                      Empty (sandbox)
                    </p>
                  ) : (
                    <ul style={{ margin: 0, padding: '0 0 0 1rem', fontSize: '0.8rem' }}>
                      {referenceData.priorities.map((p: any) => (
                        <li key={p.ien}>
                          {p.ien}: {p.name}
                        </li>
                      ))}
                ...
                <div>
                  <h4 style={{ fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                    Types ({referenceData.types?.length || 0})
                  </h4>
                ...
                <div>
                  <h4 style={{ fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                    Statuses ({referenceData.statuses?.length || 0})
                  </h4>
      ...
      {tab === 'waitlist' && !loading && (
        <div>
          ...
          {waitlistEntries.length === 0 ? (
            <p style={{ fontStyle: 'italic', color: '#6c757d' }}>
              No wait-list entries found (sandbox may be empty).
            </p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
              <thead>
          ...
          {recallEntries.length === 0 ? (
            ...
            <ul style={{ fontSize: '0.85rem' }}>
              {recallEntries.map((r: any, i: number) => (
                <li key={r.ien || i}>
                  {r.ien}: {r.description || r.recallType || 'Recall entry'}
                </li>
              ))}
            </ul>
          )}
          {recallGrounding && (
            ...
            >
              ...
              <p style={{ margin: '0.25rem 0' }}>
                <strong>Files:</strong> {recallGrounding.vistaFiles?.join(', ')}
              </p>
              <p style={{ margin: '0.25rem 0' }}>
                <strong>RPCs:</strong> {recallGrounding.targetRpcs?.join(', ')}
              </p>
              <p style={{ margin: '0.25rem 0' }}>
                <strong>Migration:</strong> {recallGrounding.migrationPath}
              </p>
      ...
      {tab === 'parity' && !loading && (
        <div>
          <h2 style={{ fontSize: '1.125rem', marginBottom: '0.5rem' }}>
            VSE / VS GUI Parity Matrix
          </h2>
          {parityData ? (
            <>
              <div
                style={{ display: 'flex', gap: '1rem', marginBottom: '1rem', fontSize: '0.8rem' }}
              >
                <span>
                  <strong>Overall Coverage:</strong> {parityData.overallCoveragePct}%
                </span>
                <span>
                  <strong>Endpoints:</strong> {parityData.endpointCount}
                </span>
                <span>
                  <strong>RPCs:</strong> {parityData.rpcCount}
                </span>
                <span>
                  <strong>Capabilities:</strong> {parityData.capabilityCount}
                </span>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                <thead>
                ...
                <tbody>
                  {(parityData.surfaces || []).map((s: any) => (
                    <tr key={s.id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '0.5rem', fontSize: '0.7rem', color: '#6c757d' }}>
                        {s.rpcs?.join(', ')}
                      </td>
                      <td style={{ padding: '0.5rem', fontSize: '0.7rem', color: '#6c757d' }}>
                        {s.gaps?.join('; ')}
