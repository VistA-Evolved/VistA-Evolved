...
const S = {
  page: { maxWidth: '1200px', margin: '0 auto', padding: '16px', fontFamily: "'Segoe UI', Tahoma, sans-serif" } as React.CSSProperties,
  header: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' } as React.CSSProperties,
  title: { margin: 0, fontSize: '18px', fontWeight: 700, color: '#1a365d' } as React.CSSProperties,
  sourceTag: { fontSize: '10px', padding: '2px 6px', background: '#c6f6d5', color: '#276749', borderRadius: '4px', fontWeight: 600 } as React.CSSProperties,
  tabBar: { display: 'flex', gap: 0, borderBottom: '2px solid #e2e8f0', marginBottom: '16px' } as React.CSSProperties,
  tab: (active: boolean): React.CSSProperties => ({
    padding: '8px 20px', fontSize: '13px', fontWeight: active ? 700 : 500,
    background: active ? '#ebf8ff' : 'transparent', border: 'none', cursor: 'pointer',
    borderBottom: active ? '3px solid #2b6cb0' : '3px solid transparent',
    color: active ? '#2b6cb0' : '#4a5568', transition: 'all .15s',
  }),
  card: { background: '#fff', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '16px', marginBottom: '12px' } as React.CSSProperties,
  label: { fontSize: '12px', fontWeight: 600, color: '#4a5568', marginBottom: '4px' } as React.CSSProperties,
  error: { padding: '10px 14px', background: '#fed7d7', border: '1px solid #fc8181', borderRadius: '6px', color: '#c53030', fontSize: '13px', marginBottom: '12px' } as React.CSSProperties,
  loading: { padding: '24px', textAlign: 'center' as const, color: '#a0aec0', fontSize: '13px' },
  pendingBanner: { background: '#fffbeb', border: '1px solid #f6e05e', borderRadius: '6px', padding: '12px 16px', marginBottom: '12px' } as React.CSSProperties,
  pendingTitle: { fontWeight: 700, fontSize: '13px', color: '#975a16', marginBottom: '4px' } as React.CSSProperties,
  pendingText: { fontSize: '12px', color: '#744210', lineHeight: 1.5 } as React.CSSProperties,
  codeBlock: { background: '#f7fafc', border: '1px solid #e2e8f0', borderRadius: '4px', padding: '8px 12px', fontSize: '11px', fontFamily: 'Consolas, monospace', marginTop: '8px', lineHeight: 1.6, color: '#2d3748' } as React.CSSProperties,
  table: { width: '100%', borderCollapse: 'collapse' as const, fontSize: '12px' },
  th: { padding: '6px 10px', textAlign: 'left' as const, borderBottom: '2px solid #e2e8f0', fontWeight: 600, color: '#4a5568', background: '#f7fafc', fontSize: '11px', textTransform: 'uppercase' as const, letterSpacing: '0.5px' },
  ...
  badge: (color: string): React.CSSProperties => ({
    ...
    background: color === 'red' ? '#fed7d7' : color === 'green' ? '#c6f6d5' : color === 'yellow' ? '#fefcbf' : '#bee3f8',
    color: color === 'red' ? '#c53030' : color === 'green' ? '#276749' : color === 'yellow' ? '#975a16' : '#2b6cb0',
  }),
  actionBtn: (disabled: boolean): React.CSSProperties => ({
    padding: '6px 16px', fontSize: '12px', fontWeight: 600, borderRadius: '4px', border: 'none',
    cursor: disabled ? 'not-allowed' : 'pointer', color: '#fff',
    background: disabled ? '#cbd5e0' : '#2b6cb0', opacity: disabled ? 0.6 : 1,
  }),
  criticalRow: { background: '#fff5f5', borderLeft: '3px solid #e53e3e' } as React.CSSProperties,
  banner: { display: 'flex', alignItems: 'center', gap: '16px', padding: '10px 16px', background: '#ebf8ff', border: '1px solid #90cdf4', borderRadius: '6px', marginBottom: '16px' } as React.CSSProperties,
  ...
  modalContent: { background: '#fff', borderRadius: '8px', padding: '24px', minWidth: '500px', maxWidth: '700px', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 8px 30px rgba(0,0,0,0.15)' } as React.CSSProperties,
  textarea: { width: '100%', minHeight: '200px', padding: '10px', fontSize: '13px', fontFamily: "'Segoe UI', Tahoma, sans-serif", border: '1px solid #e2e8f0', borderRadius: '4px', resize: 'vertical' as const } as React.CSSProperties,
  selectInput: { padding: '6px 10px', fontSize: '13px', border: '1px solid #e2e8f0', borderRadius: '4px', background: '#fff', minWidth: '180px' } as React.CSSProperties,
  pendingTag: { display: 'inline-block', padding: '1px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 600, background: '#fefcbf', color: '#975a16' } as React.CSSProperties,
  dueIndicator: (overdue: boolean): React.CSSProperties => ({
    display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '3px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: 600,
    background: overdue ? '#fed7d7' : '#c6f6d5', color: overdue ? '#c53030' : '#276749',
  }),
  trendGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '12px' } as React.CSSProperties,
  trendCard: { background: '#f7fafc', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '12px' } as React.CSSProperties,
  trendTitle: { fontWeight: 700, fontSize: '13px', color: '#2d3748', marginBottom: '6px' } as React.CSSProperties,
  ...
  return (
    <div style={S.pendingBanner}>
      ...
      <div style={S.pendingText}>
        {targets.map((t, i) => (
          <div key={i}><code>{t.rpc}</code> ({t.package}) — {t.reason}</div>
        ))}
      </div>
      {grounding && (
        <div style={S.codeBlock}>
          <div><strong>VistA Files:</strong> {grounding.vistaFiles.join(', ')}</div>
          <div><strong>M Routines:</strong> {grounding.targetRoutines.join(', ')}</div>
          <div><strong>Migration Path:</strong> {grounding.migrationPath}</div>
          <div><strong>Sandbox Note:</strong> {grounding.sandboxNote}</div>
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      <div style={{ ...S.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          {notes.length > 0 && <span style={{ ...S.badge('blue'), marginLeft: '8px' }}>{notes.length}</span>}
        </div>
            ...
            <tbody>
              {notes.map((n) => (
                <tr key={n.ien}>
                  ...
                  <td style={S.td}><span style={S.badge(n.status === 'SIGNED' ? 'green' : 'yellow')}>{n.status || 'unsigned'}</span></td>
                  <td style={S.td}>
                    <button
      ...
      {showCreate && (
        <div style={S.modal} onClick={() => !creating && setShowCreate(false)}>
          <div style={S.modalContent} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ margin: '0 0 16px', fontSize: '16px', color: '#1a365d' }}>New Nursing Note</h3>

            <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
              <div>
                <div style={S.label}>Note Type</div>
                <select style={S.selectInput} value={noteTitle} onChange={(e) => setNoteTitle(e.target.value)}>
              ...
              <div>
                ...
                <select style={S.selectInput} value={noteShift} onChange={(e) => setNoteShift(e.target.value)}>
            ...
            {createResult && (
              <div style={createResult.ok
                ? { padding: '8px 12px', background: '#c6f6d5', borderRadius: '4px', marginBottom: '12px', fontSize: '12px', color: '#276749' }
                : { padding: '8px 12px', background: '#fed7d7', borderRadius: '4px', marginBottom: '12px', fontSize: '12px', color: '#c53030' }
              }>
                {createResult.ok
                ...
                {createResult.source === 'local-draft' && (
                  <div style={{ fontSize: '11px', marginTop: '4px', color: '#975a16' }}>
                    Draft ID: {createResult.draftId} — Will persist to VistA when TIU Nursing Note class is configured.
                  </div>
                )}
              </div>
            )}
        ...
        <div style={S.modal} onClick={() => setSelectedNote(null)}>
          <div style={S.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <h3 style={{ margin: 0, fontSize: '16px', color: '#1a365d' }}>{selectedNote.title}</h3>
              <button
            ...
            {loadingText ? (
              ...
              <pre style={{ ...S.codeBlock, whiteSpace: 'pre-wrap', fontSize: '13px', minHeight: '100px' }}>
                {selectedNoteText}
              </pre>
            )}
            <div style={{ marginTop: '12px', textAlign: 'right' }}>
              <button style={{ ...S.actionBtn(false), background: '#718096' }} onClick={() => setSelectedNote(null)}>
                Close
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
        {(['vitals', 'io', 'assessments'] as const).map((st) => (
          <button
            key={st}
            style={{
              padding: '4px 14px', fontSize: '12px', borderRadius: '12px', border: '1px solid #e2e8f0',
              background: subTab === st ? '#2b6cb0' : '#fff', color: subTab === st ? '#fff' : '#4a5568',
              cursor: 'pointer', fontWeight: subTab === st ? 600 : 400,
            }}
    ...
    <div>
      ...
      <div style={{ ...S.card, display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <div style={{ fontSize: '12px', color: '#4a5568' }}>
          Next vitals due: {data.nextVitalsDue !== 'Unknown'
        ...
        {Object.entries(trends).map(([type, values]) => {
          ...
          return (
            <div
              ...
              style={{
                ...S.trendCard,
                ...(isCritical ? { borderColor: '#e53e3e', borderWidth: '2px', background: '#fff5f5' } : {}),
              }}
            >
              ...
              <div style={S.trendValue}>
                {latest?.value || '—'}
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: 400, marginLeft: '4px' }}>
              ...
              {thresh && (
                <div style={{ fontSize: '10px', color: '#718096', marginTop: '2px' }}>
                  Range: {thresh.low !== undefined ? `${thresh.low}` : '—'} – {thresh.high !== undefined ? `${thresh.high}` : '—'} {thresh.unit}
                </div>
              )}
            </div>
          );
      ...
      {items.length > 0 && (
        <div style={{ ...S.card, marginTop: '12px' }}>
          <div style={{ fontWeight: 600, fontSize: '13px', color: '#2d3748', marginBottom: '8px' }}>All Vitals</div>
          <table style={S.table}>
            <thead>
              <tr>
            ...
            <tbody>
              {items.map((v, i) => (
                <tr key={i} style={v.critical ? S.criticalRow : {}}>
                  ...
                  <td style={{ ...S.td, fontWeight: v.critical ? 700 : 400, color: v.critical ? '#c53030' : '#2d3748' }}>
                    {v.value}
                  </td>
                  <td style={S.td}>{v.units}</td>
                  <td style={S.td}>
                    {v.critical ? <span style={S.badge('red')}>CRITICAL</span> : <span style={S.badge('green')}>Normal</span>}
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {types.map((t) => (
            <div key={t} style={{
              ...
              background: '#f7fafc', fontSize: '12px', color: '#4a5568',
            }}>
              {t}
        ...
        <div style={{ fontSize: '11px', color: '#a0aec0', marginTop: '12px' }}>
          Assessments target GN(228) ASSESSMENT file or TIU-based nursing templates.
          Will be functional when ZVENAS custom RPCs are built (Phase 84B).
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
          ...
          <tbody>
            {tasks.map((t) => (
              <tr key={t.id} style={t.status === 'OVERDUE' ? S.criticalRow : {}}>
                ...
                <td style={S.td}>
                  <span style={S.badge(t.priority === 'high' ? 'red' : 'blue')}>
                    {t.priority}
                  </span>
                </td>
                <td style={S.td}>{t.due}</td>
                <td style={S.td}>
                  <span style={S.badge(t.status === 'OVERDUE' ? 'red' : t.status === 'Scheduled' ? 'green' : 'yellow')}>
                    {t.status}
                  </span>
                </td>
              </tr>
      ...
      {taskData?.pendingTargets?.length > 0 && (
        <IntegrationPendingSection
          title="Full Task Engine"
          targets={taskData.pendingTargets}
        />
      )}
      ...
      <div style={{ ...S.card, fontSize: '12px', color: '#718096' }}>
        ...
        <ul style={{ margin: '4px 0 0', paddingLeft: '18px' }}>
          <li>Critical vitals flagged with configurable thresholds (BP ≥180, HR &lt;50/&gt;130, Temp &lt;95/&gt;103°F, SpO2 ≤90%)</li>
...
function NursingDocumentationPageInner() {
  ...
  const loadPatientContext = useCallback(async (patientDfn: string) => {
    ...
    try {
      ...
      setPatient({ dfn: patientDfn, name: `Patient ${patientDfn}`, location: '', roomBed: '', attending: '' });
    }
    setLoadingCtx(false);
  }, []);
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        <button
          style={{ marginLeft: 'auto', ...S.actionBtn(false), background: '#718096', fontSize: '11px', padding: '4px 12px' }}
      ...
      {!dfn && (
        <div style={S.card}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            />
            <button style={S.actionBtn(!dfnInput.trim())} onClick={handleSelectPatient} disabled={!dfnInput.trim()}>
              Load Patient
            </button>
          </div>
        </div>
      ...
      {dfn && !loadingCtx && (
        <>
          ...
          <div style={{ marginBottom: '8px' }}>
            <button
              style={{ fontSize: '11px', color: '#2b6cb0', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
              onClick={() => { setDfn(''); setPatient(null); setDfnInput(''); }}
            >
              Change Patient
            </button>
          </div>
...
export default function NursingDocumentationPage() {
  return (
    <Suspense fallback={<div style={{ padding: '32px', color: '#718096' }}>Loading nursing module...</div>}>
