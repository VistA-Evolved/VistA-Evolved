...
function RegistryTab() {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: '#9ca3af', padding: 40 }}>Loading registry...</div>;
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <input
          ...
          style={{ padding: '6px 10px', fontSize: 13, border: '1px solid #d1d5db', borderRadius: 4, width: 220 }}
        />
        <select
          value={statusFilter}
        ...
        <tbody>
          {hmos.map(hmo => (
            <tr key={hmo.payerId} style={{ borderBottom: '1px solid #f3f4f6' }}>
              ...
              <td style={{ padding: '8px 6px' }}>
                <Badge text={hmo.integrationMode} color={MODE_COLORS[hmo.integrationMode] || '#6b7280'} />
              </td>
              <td style={{ padding: '8px 6px' }}>
                <Badge text={hmo.status.replace(/_/g, ' ')} color={STATUS_COLORS[hmo.status] || '#6b7280'} />
              </td>
              <td style={{ padding: '8px 6px' }}>
              ...
              <td style={{ padding: '8px 6px' }}>{hmo.evidence.length} source{hmo.evidence.length !== 1 ? 's' : ''}</td>
              <td style={{ padding: '8px 6px' }}>
                <button
      ...
      {hmos.length === 0 && (
        <div style={{ textAlign: 'center', color: '#9ca3af', padding: 24 }}>No HMOs match the current filter</div>
      )}
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 16 }}>
        {hmo.payerId} | {hmo.brandNames.join(', ')} | <Badge text={hmo.status.replace(/_/g, ' ')} color={STATUS_COLORS[hmo.status] || '#6b7280'} />
      </div>

      {/* Capabilities */}
      <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, marginTop: 16 }}>Capabilities</h3>
          ...
          {Object.entries(hmo.capabilities).map(([key, val]) => (
            <tr key={key} style={{ borderBottom: '1px solid #f3f4f6' }}>
              ...
              <td style={{ padding: '6px 4px' }}>
                <Badge text={String(val).replace(/_/g, ' ')} color={CAPABILITY_COLORS[val] || '#6b7280'} />
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {/* Evidence */}
      <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, marginTop: 16 }}>Evidence Sources</h3>
        ...
        <ul style={{ fontSize: 12, paddingLeft: 16, margin: 0 }}>
          {hmo.evidence.map((ev, i) => (
            <li key={i} style={{ marginBottom: 8 }}>
              <a href={ev.url} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb' }}>{ev.title}</a>
              <span style={{ color: '#9ca3af' }}> ({ev.kind})</span>
              {ev.notes && <div style={{ color: '#6b7280', fontSize: 11, marginTop: 2 }}>{ev.notes}</div>}
            </li>
          ))}
        </ul>
      )}
      ...
      {hmo.contractingTasks && hmo.contractingTasks.length > 0 && (
        <>
          <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, marginTop: 16 }}>Contracting Tasks</h3>
          <ul style={{ fontSize: 12, paddingLeft: 16, margin: 0 }}>
            {hmo.contractingTasks.map((task, i) => (
              <li key={i} style={{ marginBottom: 4, color: '#374151' }}>{task}</li>
            ))}
          </ul>
        </>
      )}
      ...
      {/* Canonical Source */}
      <h3 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, marginTop: 16 }}>Canonical Source</h3>
      <div style={{ fontSize: 12 }}>
        <a href={hmo.canonicalSource.url} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb' }}>
...
function CapabilitiesTab() {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: '#9ca3af', padding: 40 }}>Loading capabilities...</div>;

  const capKeys: (keyof HmoCapabilities)[] = ['loa', 'eligibility', 'claimsSubmission', 'claimStatus', 'remittance', 'memberPortal', 'providerPortal'];
    ...
    <div style={{ overflowX: 'auto' }}>
      ...
      <table style={{ borderCollapse: 'collapse', fontSize: 11, minWidth: 900 }}>
        ...
        <tbody>
          {hmos.map(hmo => (
            <tr key={hmo.payerId} style={{ borderBottom: '1px solid #f3f4f6' }}>
              <td style={{ padding: '6px 8px', whiteSpace: 'nowrap', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
...
function PacketsTab() {
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
        Generate LOA request or claim submission packets for any HMO. Packets include
        required fields, portal links (if available), and step-by-step instructions.
      </p>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <select
          value={selectedPayerId}
          onChange={e => { setSelectedPayerId(e.target.value); setPacket(null); }}
          style={{ padding: '6px 10px', fontSize: 13, border: '1px solid #d1d5db', borderRadius: 4, flex: 1, maxWidth: 400 }}
        >
          <option value="">Select HMO...</option>
          {hmos.map(h => (
            <option key={h.payerId} value={h.payerId}>{h.brandNames[0] || h.legalName} ({h.payerId})</option>
          ))}
        </select>

        <select
          value={packetType}
          onChange={e => { setPacketType(e.target.value as 'loa' | 'claim'); setPacket(null); }}
          style={{ padding: '6px 10px', fontSize: 13, border: '1px solid #d1d5db', borderRadius: 4 }}
        >
        ...
        <button
          ...
          style={{
            padding: '6px 16px', fontSize: 13, background: selectedPayerId ? '#2563eb' : '#d1d5db',
            color: '#fff', border: 'none', borderRadius: 4, cursor: selectedPayerId ? 'pointer' : 'default',
          }}
        >
          Generate
        </button>
      ...
      {packet && (
        <div style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: 16, background: '#f9fafb' }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>
            {packetType === 'loa' ? 'LOA Request' : 'Claim Submission'} Packet: {packet.payerName}
          </h3>
          <div style={{ fontSize: 12, marginBottom: 8 }}>
            <Badge text={packet.integrationMode} color={MODE_COLORS[packet.integrationMode] || '#6b7280'} />
            {packet.portalUrl && (
              <span style={{ marginLeft: 8 }}>
                <a href={packet.portalUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontSize: 12 }}>
                  Open Portal
                </a>
              </span>
            )}
          ...
          <h4 style={{ fontSize: 12, fontWeight: 600, marginTop: 12, marginBottom: 4 }}>Instructions</h4>
          <ol style={{ fontSize: 12, paddingLeft: 16, margin: 0 }}>
            {packet.instructions.map((inst, i) => (
              <li key={i} style={{ marginBottom: 4, color: '#374151' }}>{inst}</li>
            ))}
          </ol>

          <h4 style={{ fontSize: 12, fontWeight: 600, marginTop: 12, marginBottom: 4 }}>Required Fields</h4>
          <ul style={{ fontSize: 12, paddingLeft: 16, margin: 0, columns: 2 }}>
            {packet.requiredFields.map((f, i) => (
              <li key={i} style={{ marginBottom: 2, color: '#374151' }}>{f}</li>
            ))}
          </ul>
...
function ValidationTab() {
  ...
  if (loading) return <div style={{ textAlign: 'center', color: '#9ca3af', padding: 40 }}>Validating...</div>;
  if (!validation) return <div style={{ textAlign: 'center', color: '#9ca3af', padding: 40 }}>Validation data unavailable</div>;

  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        <StatCard label="Registry Valid" value={validation.valid ? 'YES' : 'NO'} color={validation.valid ? '#16a34a' : '#dc2626'} />
        <StatCard label="HMO Count" value={validation.count} />
        <StatCard label="Errors" value={validation.errors.length} color={validation.errors.length > 0 ? '#dc2626' : '#16a34a'} />
        <StatCard label="Warnings" value={validation.warnings.length} color={validation.warnings.length > 0 ? '#d97706' : '#16a34a'} />
      </div>

      {validation.errors.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <h3 style={{ fontSize: 13, fontWeight: 600, color: '#dc2626', marginBottom: 8 }}>Errors</h3>
          <ul style={{ fontSize: 12, color: '#dc2626', paddingLeft: 16 }}>
            {validation.errors.map((e, i) => <li key={i} style={{ marginBottom: 4 }}>{e}</li>)}
          </ul>
        </div>
      )}
      ...
      {validation.warnings.length > 0 && (
        <div>
          <h3 style={{ fontSize: 13, fontWeight: 600, color: '#d97706', marginBottom: 8 }}>Warnings</h3>
          <ul style={{ fontSize: 12, color: '#d97706', paddingLeft: 16 }}>
            {validation.warnings.map((w, i) => <li key={i} style={{ marginBottom: 4 }}>{w}</li>)}
          </ul>
        </div>
      )}
