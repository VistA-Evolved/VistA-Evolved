...
function ProgressBar({ pct, color = "#3b82f6" }: { pct: number; color?: string }) {
  return (
    <div style={{ background: "#334155", borderRadius: 4, height: 8, width: "100%" }}>
      <div style={{ background: color, borderRadius: 4, height: 8, width: `${Math.min(pct, 100)}%`, transition: "width 0.3s" }} />
    </div>
  );
}
...
function adapterStatusBadge(status: string) {
  switch (status) {
    case "portal_adapter_available": return <Badge text="Portal Adapter" color="#059669" />;
    case "generic_manual_adapter": return <Badge text="Generic Manual" color="#d97706" />;
    case "manual_only": return <Badge text="Manual Only" color="#64748b" />;
    default: return <Badge text={status} color="#475569" />;
  }
}

function payerTypeBadge(type: string) {
  switch (type) {
    case "hmo_l1": return <Badge text="L1" color="#3b82f6" />;
    case "hmo_l3": return <Badge text="L3" color="#8b5cf6" />;
    default: return <Badge text={type} color="#475569" />;
  }
}
...
export default function PhMarketPage() {
  ...
  if (loading) return <div style={{ padding: 32, color: "#94a3b8" }}>Loading PH market data...</div>;
  ...
  return (
    <div style={{ padding: "24px 32px", color: "#e2e8f0", maxWidth: 1200 }}>
      ...
      <p style={{ color: "#94a3b8", marginBottom: 24, fontSize: 13 }}>
        All 27 Insurance Commission-licensed HMOs -- integration status, capabilities, and contracting progress.
      </p>
      ...
      {tab === "overview" && summary && (
        <div>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 32 }}>
            ...
            <StatCard label="Portal Adapters" value={summary.integration.portalAdapterAvailable} detail={`${summary.integration.adapterCoveragePct}% coverage`} />
            <StatCard label="Capability Coverage" value={`${summary.capabilities.coveragePct}%`} detail={`${summary.capabilities.knownSlots}/${summary.capabilities.totalCapabilitySlots} slots`} />
            <StatCard label="LOA Templates" value={summary.loaTemplates.total} detail={`${summary.loaTemplates.withPortalSubmission} portal`} />
            <StatCard label="Claim Configs" value={summary.claimPackets.total} detail={`${summary.claimPackets.withPortalUpload} portal upload`} />
            <StatCard label="Contracting" value={`${summary.contracting.progressPct}%`} detail={`${summary.contracting.completedTasks}/${summary.contracting.totalTasks} tasks`} />
          </div>

          {/* Integration progress */}
          <div style={{ background: "#1e293b", borderRadius: 8, padding: 20, marginBottom: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Integration Progress</h3>
            <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 60px", gap: 8, alignItems: "center" }}>
              ...
              <span style={{ fontSize: 12, color: "#94a3b8", textAlign: "right" }}>{summary.integration.adapterCoveragePct}%</span>

              <span style={{ fontSize: 12, color: "#94a3b8" }}>Capability Matrix</span>
              <ProgressBar pct={summary.capabilities.coveragePct} color="#3b82f6" />
              <span style={{ fontSize: 12, color: "#94a3b8", textAlign: "right" }}>{summary.capabilities.coveragePct}%</span>

              <span style={{ fontSize: 12, color: "#94a3b8" }}>Contracting</span>
              <ProgressBar pct={summary.contracting.progressPct} color="#d97706" />
              <span style={{ fontSize: 12, color: "#94a3b8", textAlign: "right" }}>{summary.contracting.progressPct}%</span>
            </div>
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #334155" }}>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>HMO</th>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>Type</th>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>CA #</th>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>Adapter</th>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>Capabilities</th>
                <th style={{ padding: "8px 12px", textAlign: "left", color: "#94a3b8", fontWeight: 600 }}>Status</th>
              </tr>
            </thead>
            <tbody>
              {manifest.map((entry) => (
                <tr key={entry.payerId} style={{ borderBottom: "1px solid #1e293b" }}>
                  ...
                  <td style={{ padding: "8px 12px", fontSize: 11, color: "#94a3b8" }}>{entry.caNumber ?? "--"}</td>
                  <td style={{ padding: "8px 12px" }}>{adapterStatusBadge(entry.adapterStatus)}</td>
                  <td style={{ padding: "8px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <ProgressBar pct={entry.capabilityCoverage.pct} />
                      <span style={{ fontSize: 11, color: "#94a3b8", whiteSpace: "nowrap" }}>{entry.capabilityCoverage.pct}%</span>
                    </div>
                  </td>
                  <td style={{ padding: "8px 12px", fontSize: 11, color: "#94a3b8" }}>{entry.contractingStatus}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === "contracting" && (
        <div style={{ padding: 8 }}>
          <p style={{ color: "#94a3b8", marginBottom: 16, fontSize: 13 }}>
            Contracting tasks are managed via the Contracting Hub.
            Initialize tasks for a payer to begin the onboarding workflow.
          </p>
          <div style={{ background: "#1e293b", borderRadius: 8, padding: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Contracting Task Summary</h3>
            {summary ? (
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                ...
                <StatCard label="In Progress" value={summary.contracting.byStatus.in_progress ?? 0} />
