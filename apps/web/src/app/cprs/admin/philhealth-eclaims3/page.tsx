...
interface Submission {
  ...
  staffNotes: string[];
  timeline: Array<{ timestamp: string; fromStatus: string; toStatus: string; actor: string; detail?: string }>;
  createdAt: string;
  updatedAt: string;
}
  ...
  return (
    <div style={{ padding: 24, fontFamily: "system-ui, sans-serif", maxWidth: 1100 }}>
      ...
      <p style={{ color: "#6b7280", fontSize: 13, marginBottom: 16 }}>
        Deadline: April 1, 2026 &middot; Status: Manual submission workflow active &middot;
        XML spec: {adapterStatus ? (adapterStatus.specAvailable ? "Available" : "Pending") : "Loading..."}
      </p>
      ...
      <div style={{
        ...
        official eClaims 3.0 schema is acquired and certification completed. Export bundles
        and upload to the PhilHealth portal manually.
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {(["build", "submissions", "denials", "spec"] as Tab[]).map((t) => (
          <button key={t} style={btnStyle(tab === t)} onClick={() => setTab(t)}>
            {t === "build" ? "Build & Export" : t === "submissions" ? "Submissions" : t === "denials" ? "Denials" : "Spec Gates"}
          </button>
        ))}
      </div>
      ...
      {tab === "build" && (
        <div>
          <div style={cardStyle}>
            <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>1. Select Claim Draft</h3>
                  ...
                  <tbody>
                    {drafts.map((d) => (
                      ...
                      >
                        ...
                        <td style={{ padding: "4px 8px" }}>{d.patientLastName}, {d.patientFirstName}</td>
                        <td style={{ padding: "4px 8px", fontFamily: "monospace" }}>{d.philhealthPin}</td>
                        <td style={{ padding: "4px 8px" }}>{d.patientType === "I" ? "Inpatient" : "Outpatient"}</td>
                        <td style={{ padding: "4px 8px" }}>{d.admissionDate}</td>
                        <td style={{ padding: "4px 8px" }}><StatusBadge status={d.status} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {buildError && (
              <div style={{ marginTop: 8, padding: "6px 10px", background: "#fee2e2", borderRadius: 4, color: "#991b1b", fontSize: 13 }}>
                {buildError}
              </div>
            )}
          </div>
          ...
          {builtPacket && (
            <div style={cardStyle}>
              ...
              <div style={{ fontSize: 13 }}>
                <div><strong>Packet ID:</strong> <code style={{ fontSize: 12 }}>{builtPacket.packetId}</code></div>
                <div><strong>Patient:</strong> {builtPacket.patient.lastName}, {builtPacket.patient.firstName}</div>
                <div><strong>Facility:</strong> {builtPacket.facility.facilityName} ({builtPacket.facility.facilityCode})</div>
                <div><strong>Type:</strong> {builtPacket.patientType === "I" ? "Inpatient" : "Outpatient"}</div>
                <div><strong>Date:</strong> {builtPacket.admissionDate}</div>
                <div><strong>Total Charges:</strong> PHP {builtPacket.totals.totalCharges.toFixed(2)}</div>
                <div><strong>Content Hash:</strong> <code style={{ fontSize: 11 }}>{builtPacket.contentHash.slice(0, 24)}...</code></div>
              </div>

              <h3 style={{ fontSize: 15, fontWeight: 600, marginTop: 16, marginBottom: 8 }}>3. Export Bundle</h3>
              <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>
              ...
              {exportResult && (
                <div style={{ marginTop: 12, background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 6, padding: 12 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#166534" }}>Export Generated</div>
                  <div style={{ fontSize: 12, marginTop: 4 }}>
                    <div>Bundle ID: <code>{exportResult.bundleId}</code></div>
                    <div>XML Spec: {exportResult.xmlSpecAvailable ? "Schema-based" : "Placeholder (spec pending)"}</div>
                  </div>
                    ...
                    <tbody>
                      {exportResult.artifacts.map((a, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #f0fdf4" }}>
                          <td style={{ padding: "2px 6px" }}>{a.format.replace(/_/g, " ").toUpperCase()}</td>
                          <td style={{ padding: "2px 6px", fontFamily: "monospace", fontSize: 11 }}>{a.filename}</td>
                          <td style={{ padding: "2px 6px" }}>{(a.sizeBytes / 1024).toFixed(1)} KB</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {tab === "submissions" && (
        <div>
          <div style={cardStyle}>
            <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>Submission Status Board</h3>
            {submissions.length === 0 ? (
              <p style={{ fontSize: 13, color: "#9ca3af" }}>No submissions yet. Build and export a packet first.</p>
            ) : (
              <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid #e5e7eb", textAlign: "left" }}>
                ...
                <tbody>
                  {submissions.map((s) => (
                    <tr key={s.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                      <td style={{ padding: "4px 8px", fontFamily: "monospace", fontSize: 11 }}>{s.id.slice(0, 16)}</td>
                      <td style={{ padding: "4px 8px" }}><StatusBadge status={s.status} /></td>
                      ...
                      <td style={{ padding: "4px 8px", fontSize: 11 }}>{s.updatedAt.slice(0, 19).replace("T", " ")}</td>
                      <td style={{ padding: "4px 8px" }}>
                        <button
          ...
          {selectedSubmission && (
            <div style={cardStyle}>
              <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>
                Submission Detail: <code style={{ fontSize: 12 }}>{selectedSubmission.id.slice(0, 20)}</code>
              </h3>
              <div style={{ fontSize: 13, marginBottom: 12 }}>
                <div><strong>Status:</strong> <StatusBadge status={selectedSubmission.status} /></div>
                <div><strong>Packet ID:</strong> <code style={{ fontSize: 11 }}>{selectedSubmission.packetId}</code></div>
                <div><strong>TCN:</strong> {selectedSubmission.transmittalControlNumber ?? "Not assigned"}</div>
              </div>

              {/* Status transition buttons */}
              <div style={{ marginBottom: 12 }}>
                <span style={{ fontSize: 12, fontWeight: 600, marginRight: 8 }}>Transition to:</span>
                {selectedSubmission.status === "draft" && (
                  <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4 }} onClick={() => handleTransition(selectedSubmission.id, "reviewed")}>Reviewed</button>
                )}
                {selectedSubmission.status === "reviewed" && (
                  <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4 }} onClick={() => handleTransition(selectedSubmission.id, "exported")}>Exported</button>
                )}
                {selectedSubmission.status === "exported" && (
                  <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4 }} onClick={() => handleTransition(selectedSubmission.id, "submitted_manual")}>
                    Submitted (Manual)
                  </button>
                )}
                {selectedSubmission.status === "submitted_manual" && (
                  <>
                    <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4, borderColor: "#16a34a", color: "#16a34a" }} onClick={() => handleTransition(selectedSubmission.id, "accepted")}>
                      Accepted
                    </button>
                    <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4, borderColor: "#dc2626", color: "#dc2626" }} onClick={() => handleTransition(selectedSubmission.id, "denied")}>
                      Denied
                    </button>
                  </>
                )}
                {selectedSubmission.status === "denied" && (
                  <button style={{ ...btnStyle(), fontSize: 11, marginRight: 4 }} onClick={() => handleTransition(selectedSubmission.id, "appealed")}>Appealed</button>
                )}
              </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ fontSize: 12, marginTop: 4 }}>
                    {selectedSubmission.timeline.map((t, i) => (
                      <div key={i} style={{ padding: "2px 0", borderLeft: "2px solid #e5e7eb", paddingLeft: 8, marginBottom: 4 }}>
              ...
              {selectedSubmission.staffNotes.length > 0 && (
                <div>
                  ...
                  <ul style={{ fontSize: 12, margin: "4px 0", paddingLeft: 16 }}>
                    {selectedSubmission.staffNotes.map((n, i) => <li key={i}>{n}</li>)}
                  </ul>
                </div>
              )}
            </div>
      ...
      {tab === "denials" && (
        <div>
          <div style={cardStyle}>
            ...
            <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>
              When PhilHealth denies a claim, select the submission and enter the denial details below.
            </p>
            ...
            {(() => {
              ...
              if (denied.length === 0) {
                return <p style={{ fontSize: 13, color: "#9ca3af" }}>No denied submissions to process.</p>;
              }
              return denied.map((s) => (
                <div key={s.id} style={{ border: "1px solid #fca5a5", borderRadius: 6, padding: 12, marginBottom: 8 }}>
                  <div style={{ fontSize: 13 }}>
                    <strong>Submission:</strong> <code style={{ fontSize: 11 }}>{s.id.slice(0, 20)}</code>
                    ...
                    <div style={{ fontSize: 12, marginTop: 4 }}>
                      ...
                      <ul style={{ margin: "2px 0", paddingLeft: 16 }}>
                        {s.denialReasons.map((r, i) => (
                          <li key={i}>{r.text} {r.code ? `(${r.code})` : ""} [{r.category ?? "other"}]</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap" }}>
                    <div>
                      ...
                      <input
                        style={{ border: "1px solid #d1d5db", borderRadius: 4, padding: "4px 8px", fontSize: 12, width: 260 }}
                        value={denialText}
      ...
      {tab === "spec" && (
        <div>
          <div style={cardStyle}>
            ...
            <div style={{ fontSize: 13, marginBottom: 12 }}>
              <strong>{specProgress.completed}</strong> of <strong>{specProgress.total}</strong> gates completed
            </div>
            <div style={{
              background: "#f3f4f6",
              borderRadius: 4,
              ...
              <div style={{
                ...
                height: "100%",
                width: specProgress.total > 0 ? `${(specProgress.completed / specProgress.total) * 100}%` : "0%",
                borderRadius: 4,
          ...
          <div style={cardStyle}>
            ...
            <ul style={{ fontSize: 13, paddingLeft: 16, margin: 0 }}>
              <li>PhilHealth eClaims Portal: <a href="https://eclaims.philhealth.gov.ph" target="_blank" rel="noreferrer">eclaims.philhealth.gov.ph</a></li>
              <li>Spec Status Doc: <code>docs/runbooks/philhealth-eclaims3-spec-status.md</code></li>
