...
export default function OnboardingPage() {
  ...
  // Probe result
  const [probeResult, setProbeResult] = useState<{ ok: boolean; vista?: string; error?: string } | null>(null);

  // Module selection
  const [availableModules, setAvailableModules] = useState<Array<{ moduleId: string; name: string; enabled: boolean }>>([]);
      ...
      case 'tenant':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <label>
              Station Number
              <input type="text" value={facilityStation} onChange={(e) => setFacilityStation(e.target.value)}
                style={{ width: '100%', padding: 8, marginTop: 4, border: '1px solid #ccc', borderRadius: 4 }} />
            </label>
            <label>
              VistA Host
              <input type="text" value={vistaHost} onChange={(e) => setVistaHost(e.target.value)}
                placeholder="localhost" style={{ width: '100%', padding: 8, marginTop: 4, border: '1px solid #ccc', borderRadius: 4 }} />
            </label>
            <label>
              VistA Port
              <input type="text" value={vistaPort} onChange={(e) => setVistaPort(e.target.value)}
                placeholder="9430" style={{ width: '100%', padding: 8, marginTop: 4, border: '1px solid #ccc', borderRadius: 4 }} />
            </label>
            <button onClick={() => advance({ facilityName, facilityStation, vistaHost, vistaPort: Number(vistaPort) })}
              disabled={loading || !facilityName}
              style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button onClick={runProbe} disabled={loading}
              style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', alignSelf: 'flex-start' }}>
              {loading ? 'Probing...' : 'Test Connection'}
            </button>
            {probeResult && (
              <div style={{
                padding: 12, borderRadius: 4, border: '1px solid',
            ...
            <button onClick={() => advance({ probeResult })} disabled={loading}
              style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
      ...
      case 'modules':
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <p style={{ color: '#666', margin: 0 }}>Choose which modules to enable for this facility.</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8 }}>
              {availableModules.map((mod) => (
                <label key={mod.moduleId} style={{
                  display: 'flex', alignItems: 'center', gap: 8, padding: 8,
                  border: '1px solid #ddd', borderRadius: 4, cursor: 'pointer',
                  background: selectedModules.includes(mod.moduleId) ? '#e3f2fd' : '#fff',
                }}>
                  <input type="checkbox" checked={selectedModules.includes(mod.moduleId)}
                    onChange={() => toggleModule(mod.moduleId)} />
                  {mod.name || mod.moduleId}
                </label>
              ))}
            </div>
            <button onClick={() => advance({ modules: selectedModules })} disabled={loading}
              style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
      ...
      case 'users':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <p style={{ color: '#666', margin: 0 }}>Invite initial users. Requires OIDC/Keycloak integration for production use.</p>
            <div style={{ padding: 12, background: '#fff3e0', borderRadius: 4, border: '1px solid #ff9800' }}>
              User provisioning is pending OIDC integration. You can skip this step and configure users later.
            </div>
            <button onClick={() => advance({ skipped: true })} disabled={loading}
              style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
      ...
      case 'complete':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <p style={{ color: '#666', margin: 0 }}>Review your configuration and finalize the onboarding.</p>
            <div style={{ padding: 16, background: '#f5f5f5', borderRadius: 4 }}>
              <h4 style={{ margin: '0 0 8px 0' }}>Configuration Summary</h4>
              {active.steps.map((s) => (
                <div key={s.step} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0' }}>
                  <span>{STEP_LABELS[s.step] || s.step}</span>
                  <span style={{
                    color: s.status === 'completed' ? '#4caf50' : s.status === 'in-progress' ? '#ff9800' : '#999',
                    fontWeight: 600,
                  }}>
                    {s.status === 'completed' ? 'Done' : s.status === 'in-progress' ? 'Current' : 'Pending'}
                  </span>
                </div>
              ))}
            </div>
            <button onClick={() => advance({ finalized: true })} disabled={loading}
              style={{ padding: '10px 24px', background: '#4caf50', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>
  ...
  return (
    <div className={styles.cprsPage} style={{ padding: 24 }}>
      ...
      {error && (
        <div style={{ padding: 12, background: '#ffebee', border: '1px solid #f44336', borderRadius: 4, marginBottom: 16 }}>
          {error}
        </div>
      )}
      ...
      {!active ? (
        <div>
          ...
          <button onClick={startNew} disabled={loading}
            style={{ padding: '10px 24px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', marginBottom: 24 }}>
                ...
                <tbody>
                  {sessions.map((s) => (
                    <tr key={s.id} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={{ padding: 8, fontFamily: 'monospace', fontSize: 12 }}>{s.id.slice(0, 16)}</td>
                      <td style={{ padding: 8 }}>{s.tenantId}</td>
                      <td style={{ padding: 8 }}>{STEP_LABELS[s.currentStep] || s.currentStep}</td>
                      <td style={{ padding: 8, fontSize: 12 }}>{new Date(s.createdAt).toLocaleDateString()}</td>
                      <td style={{ padding: 8 }}>
                        <span style={{
                      ...
                      <td style={{ padding: 8 }}>
                        {!s.completedAt && (
                          <button onClick={() => loadSession(s.id)}
                            style={{ padding: '4px 12px', background: '#0066cc', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 12 }}>
                            Resume
                          </button>
                        )}
                      </td>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 24 }}>
            {active.steps.map((s) => (
              <div key={s.step} style={{
                flex: 1, padding: 8, textAlign: 'center', borderRadius: 4,
                background: s.status === 'completed' ? '#e8f5e9' : s.status === 'in-progress' ? '#e3f2fd' : '#f5f5f5',
                ...
                {s.status === 'completed' && <div style={{ color: '#4caf50', fontSize: 11 }}>Done</div>}
              </div>
            ))}
          </div>

          {active.completedAt ? (
            <div style={{ padding: 24, background: '#e8f5e9', borderRadius: 8, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 24 }}>
            <button onClick={() => setActive(null)}
              style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer' }}>
              Back to Sessions
            </button>
          </div>
        </div>
