...
type ClaimStatus = 'draft' | 'ready_for_submission' | 'exported' | 'test_uploaded' | 'submitted_pending' | 'returned_to_hospital' | 'paid' | 'denied';
...
interface ClaimDraft {
  ...
  charges: Array<{ category: string; description: string; netAmount: number; quantity: number; unitCharge: number; discount: number; phicCoverage: number; patientShare: number }>;
  procedures: Array<{ code: string; description?: string }>;
  professionalFees: Array<{ physicianName: string; physicianLicense: string; feeAmount: number; serviceDate: string }>;
  soaElectronic?: Record<string, unknown>;
  lastExportManifest?: Record<string, unknown>;
  testUploadResult?: { simulated: boolean; transmittalControlNumber: string; validationPassed: boolean; validationErrors: string[]; nextSteps: string[] };
...
export default function PhilHealthClaimsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setShowCreate(false);
        setFLastName(''); setFFirstName(''); setFPin(''); setFAdmission('');
        fetchClaims();
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100 }}>
      ...
      <div style={{
        ...
        for review. It does NOT submit claims to PhilHealth. All test uploads are <strong>SIMULATED</strong>.
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700 }}>PhilHealth Claim Drafts</h2>
        <button onClick={() => setShowCreate(!showCreate)}
          style={{ padding: '6px 16px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 4, fontSize: 13, cursor: 'pointer' }}>
          + New Claim
        </button>
      </div>
      ...
      {error && (
        <div style={{ background: '#fee2e2', border: '1px solid #ef4444', borderRadius: 4, padding: '8px 12px', marginBottom: 12, fontSize: 13 }}>
          {error}
          <button onClick={() => setError(null)} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>x</button>
        </div>
      )}
      ...
      {showCreate && (
        <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 6, padding: 16, marginBottom: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>New PhilHealth Claim Draft</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <input placeholder="Last Name *" value={fLastName} onChange={e => setFLastName(e.target.value)}
              style={{ padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: 4, fontSize: 12 }} />
            <input placeholder="First Name *" value={fFirstName} onChange={e => setFFirstName(e.target.value)}
              style={{ padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: 4, fontSize: 12 }} />
            <input placeholder="PhilHealth PIN *" value={fPin} onChange={e => setFPin(e.target.value)}
              style={{ padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: 4, fontSize: 12 }} />
            <input type="date" value={fAdmission} onChange={e => setFAdmission(e.target.value)}
              style={{ padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: 4, fontSize: 12 }} />
            <select value={fType} onChange={e => setFType(e.target.value as 'O' | 'I')}
              style={{ padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: 4, fontSize: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        ...
        <div style={{ flex: '0 0 380px', maxHeight: 600, overflowY: 'auto' }}>
          {claims.length === 0 ? (
            <p style={{ fontSize: 13, color: '#6b7280' }}>No claim drafts yet. Create one to get started.</p>
          ) : claims.map(c => (
            <div key={c.id}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 13, fontWeight: 600 }}>{c.patientLastName}, {c.patientFirstName}</span>
                <span style={{
                  fontSize: 10, padding: '2px 6px', borderRadius: 3, fontWeight: 600,
                  color: '#fff', background: STATUS_COLORS[c.status] || '#6b7280',
                }}>{c.status.replace(/_/g, ' ')}</span>
              </div>
              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                PIN: {c.philhealthPin} | {c.patientType === 'I' ? 'Inpatient' : 'Outpatient'} | Adm: {c.admissionDate}
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Detail Panel */}
        {detail && (
          <div style={{ flex: 1, background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 6, padding: 16, maxHeight: 600, overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
              <button onClick={handleValidate}
                style={{ padding: '4px 10px', background: '#7c3aed', color: '#fff', border: 'none', borderRadius: 3, fontSize: 11, cursor: 'pointer' }}>
                Validate
              </button>
              {detail.status === 'draft' && (
                <button onClick={() => handleTransition('ready_for_submission')}
                  style={{ padding: '4px 10px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 3, fontSize: 11, cursor: 'pointer' }}>
                  Mark Ready
                </button>
              )}
              {(detail.status === 'ready_for_submission' || detail.status === 'exported') && (
                <button onClick={handleExport}
                  style={{ padding: '4px 10px', background: '#059669', color: '#fff', border: 'none', borderRadius: 3, fontSize: 11, cursor: 'pointer' }}>
                  Export Package
                </button>
              )}
              {detail.status === 'exported' && (
                <button onClick={handleTestUpload}
                  style={{ padding: '4px 10px', background: '#d97706', color: '#fff', border: 'none', borderRadius: 3, fontSize: 11, cursor: 'pointer' }}>
            ...
            {validation && (
              <div style={{ marginBottom: 12 }}>
                ...
                {validation.eclaims3Compliance.admissionDateRequiresEsoa && (
                  <div style={{ marginTop: 4, fontSize: 11, padding: '4px 8px', background: '#fef3c7', borderRadius: 3 }}>
                    eClaims 3.0: Admission date requires electronic SOA.
                    {validation.eclaims3Compliance.scannedPdfDetected && ' Scanned PDF REJECTED.'}
                    {validation.eclaims3Compliance.electronicSoaPresent ? ' eSOA present.' : ' eSOA will be generated on export.'}
                  </div>
                )}
            ...
            {detail.testUploadResult && (
              <div style={{
                ...
                <div><strong>Test Upload Result</strong> (SIMULATED)</div>
            ...
            {detail.lastExportManifest && (
              <div style={{ marginBottom: 12, padding: '8px 12px', background: '#f3e8ff', border: '1px solid #c4b5fd', borderRadius: 4, fontSize: 12 }}>
                <strong>Export Package</strong>
                <div style={{ fontSize: 11 }}>
                  Files: {(detail.lastExportManifest as any).files?.length ?? 0}
                  {' | '}Total charges: PHP {((detail.lastExportManifest as any).claimSummary?.totalCharges ?? 0).toLocaleString()}
              ...
              {detail.diagnoses.length === 0 ? (
                ...
                <ul style={{ margin: '4px 0 0 16px', padding: 0, fontSize: 11 }}>
                  {detail.diagnoses.map((d, i) => (
                    <li key={i}>{d.icdCode} ({d.type}){d.description ? ` -- ${d.description}` : ''}</li>
                  ))}
                </ul>
              )}
            </div>
                  ...
                  <tbody>
                    {detail.charges.map((ch, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <td style={{ textAlign: 'right', padding: '2px 4px' }}>{ch.netAmount.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div>
              ...
              <div style={{ marginTop: 4 }}>
                {detail.timeline.map((t, i) => (
                  <div key={i} style={{ fontSize: 11, padding: '3px 0', borderBottom: '1px solid #f3f4f6' }}>
                    <span style={{ color: '#6b7280' }}>{new Date(t.timestamp).toLocaleString()}</span>
