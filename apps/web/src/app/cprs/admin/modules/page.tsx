  ...
  return (
    <div className={styles.cprsPage}>
      ...
      <div style={{ padding: '16px 24px', overflow: 'auto', flex: 1 }}>
        ...
        {tab === 'connectors' && (
          <ConnectorsTab config={marketplaceConfig} onToggle={toggleConnector} saving={saving} />
        )}
        {tab === 'jurisdiction' && (
          <JurisdictionTab
        ...
        {tab === 'status' && (
          <StatusTab summary={summary} config={marketplaceConfig} manifests={manifests} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#6c757d', marginBottom: 16 }}>
        Toggle modules to enable/disable them for this tenant. Modules with unmet dependencies
        cannot be enabled. Click a module row to see its full manifest.
      </p>
              ...
              >
                ...
                <td style={{ padding: '8px 12px' }}>
                  {m.manifest.alwaysEnabled ? (
                    <span style={{ color: '#198754', fontWeight: 600, fontSize: 11 }}>
                      ALWAYS ON
                    </span>
                  ) : m.enabled ? (
                    <span style={{ color: '#0d6efd', fontWeight: 600, fontSize: 11 }}>ENABLED</span>
                  ) : (
                    <span style={{ color: '#dc3545', fontWeight: 600, fontSize: 11 }}>
                      DISABLED
                    </span>
                  )}
                </td>
                ...
                <td style={{ padding: '8px 12px' }}>
                  {m.manifest.alwaysEnabled ? (
                    ...
                    <button
                      ...
                      title={
                        !m.dependenciesMet
                          ? `Missing deps: ${m.missingDependencies.join(', ')}`
                          : ''
                      }
                    >
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, fontSize: 12 }}>
      <div>
        <h4 style={{ margin: '0 0 8px', fontSize: 13 }}>
          {m.manifest.name} v{m.manifest.version}
        </h4>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <code
            style={{ background: '#e2e3e5', padding: '1px 6px', borderRadius: 3, fontSize: 11 }}
          >
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 4 }}>
            {m.manifest.routePatterns.map((r, i) => (
              <code
                key={i}
                style={{ display: 'block', fontSize: 10, color: '#495057', padding: '1px 0' }}
              >
                {r}
              </code>
            ))}
      ...
      <div>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          {m.manifest.dataStores.map((ds) => (
            ...
            >
              <strong>{ds.id}</strong>{' '}
              <span style={{ color: '#6c757d', fontSize: 10 }}>({ds.type})</span>
...
function ConnectorsTab({
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#6c757d', marginBottom: 16 }}>
        Connector settings for tenant <strong>{config.tenantId}</strong> ({config.jurisdiction}{' '}
        jurisdiction). Toggle connectors on/off. Secrets are managed via environment variables, not
        stored here.
      </p>
          ...
          {config.connectors.map((c, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 6,
                padding: '12px 16px',
                background: c.enabled ? '#fff' : '#f8f9fa',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    cursor: 'pointer',
                    background: c.enabled ? '#dc3545' : '#198754',
                    color: '#fff',
                    fontWeight: 600,
                  }}
                >
              ...
              <div style={{ fontSize: 12 }}>
                {Object.entries(c.settings).map(([k, v]) => (
                  <div
                    key={k}
                    style={{ display: 'inline-block', marginRight: 12, marginBottom: 4 }}
                  >
...
function JurisdictionTab({
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#6c757d', marginBottom: 16 }}>
        Select a jurisdiction pack for tenant <strong>{config.tenantId}</strong>. Changing
        jurisdiction resets connector settings and regulatory defaults.
      </p>
      <div style={{ display: 'grid', gap: 12, maxWidth: 600 }}>
        {jurisdictions.map((j) => (
          <div
            ...
            onClick={() => {
              if (j.id !== config.jurisdiction && !saving) onChange(j.id);
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: 11, color: '#6c757d' }}>
                {j.defaultConnectorCount} default connector
                {j.defaultConnectorCount !== 1 ? 's' : ''}
              </span>
            </div>
      ...
      {config && (
        <div style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 16 }}>
          ...
          <div style={{ fontSize: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div>
              <strong>Facility:</strong> {config.facilityName}
            </div>
            <div>
              <strong>Jurisdiction:</strong> {config.jurisdiction}
            </div>
            <div>
              <strong>Modules:</strong> {config.enabledModules.length} enabled
            </div>
            <div>
              <strong>Connectors:</strong> {config.connectors.filter((c) => c.enabled).length}{' '}
              active
            </div>
            <div>
      ...
      >
        <div>
          <p style={{ fontSize: 13, color: '#6c757d', margin: 0 }}>
            DB-backed module entitlements per tenant. Toggle modules to enable/disable them.
            ...
            return (
              <tr key={mod.moduleId} style={{ borderBottom: '1px solid #dee2e6' }}>
                ...
                <td style={{ padding: '8px 12px', fontSize: 11, color: '#6c757d' }}>
                  {ent?.enabledAt
                    ? new Date(ent.enabledAt).toLocaleString()
                    : ent?.disabledAt
                      ? new Date(ent.disabledAt).toLocaleString()
                      : '-'}
                </td>
                <td style={{ padding: '8px 12px' }}>
                  {!mod.alwaysEnabled && (
...
function FeatureFlagsTab() {
  ...
  const createFlag = async () => {
    if (!newFlag.flagKey || !newFlag.flagValue) {
      setError('flagKey and flagValue are required');
      return;
    }
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#6c757d', marginBottom: 16 }}>
        Manage per-tenant feature flags with gradual rollout support. Flags are key-value pairs that
        control fine-grained behavior within modules.
      </p>
      {error && (
        <div
          style={{
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <input
            ...
            onChange={(e) => setNewFlag((f) => ({ ...f, flagKey: e.target.value }))}
            style={{
              padding: '4px 8px',
              fontSize: 12,
          ...
          <input
            ...
            onChange={(e) => setNewFlag((f) => ({ ...f, flagValue: e.target.value }))}
            style={{
              padding: '4px 8px',
              fontSize: 12,
              border: '1px solid #ced4da',
          ...
          <input
            ...
            value={newFlag.moduleId}
            onChange={(e) => setNewFlag((f) => ({ ...f, moduleId: e.target.value }))}
            style={{
              padding: '4px 8px',
              fontSize: 12,
              border: '1px solid #ced4da',
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        ...
        <tbody>
          {flags.length === 0 && (
            <tr>
              <td colSpan={6} style={{ padding: 16, color: '#6c757d', textAlign: 'center' }}>
                No feature flags set for this tenant.
              </td>
            </tr>
          )}
          {flags.map((f) => (
              ...
              <td style={{ padding: '8px 12px', fontSize: 12 }}>
                <span
                  style={{
                    ...
                    fontWeight: 600,
                    background:
                      (f.rolloutPercentage ?? 100) === 100
                        ? '#d1e7dd'
                        : (f.rolloutPercentage ?? 100) === 0
                          ? '#f8d7da'
                          : '#fff3cd',
                    color:
                      (f.rolloutPercentage ?? 100) === 100
...
function ModuleAuditTab() {
  ...
  return (
    <div>
      ...
      >
        <p style={{ fontSize: 13, color: '#6c757d', margin: 0 }}>
          Append-only audit trail of all module entitlement and feature flag changes. {total} total
          entries.
        </p>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, lineHeight: '28px', color: '#6c757d' }}>
        ...
        <tbody>
          ...
          {entries.map((e) => (
            <tr key={e.id} style={{ borderBottom: '1px solid #dee2e6' }}>
              <td style={{ padding: '6px 10px', whiteSpace: 'nowrap', color: '#6c757d' }}>
                {new Date(e.createdAt).toLocaleString()}
              </td>
              <td style={{ padding: '6px 10px' }}>
                {e.actorId} <span style={{ fontSize: 10, color: '#6c757d' }}>({e.actorType})</span>
              </td>
              ...
              <td style={{ padding: '6px 10px' }}>
                <span
                  style={{
                    ...
                    fontWeight: 600,
                    background:
                      e.action === 'enable' || e.action === 'create'
                        ? '#d1e7dd'
                        : e.action === 'disable' || e.action === 'delete'
                          ? '#f8d7da'
                          : '#fff3cd',
                    color:
                      e.action === 'enable' || e.action === 'create'
              ...
              >
                {e.beforeJson && (
                  <span title={e.beforeJson}>
                    B: {e.beforeJson.substring(0, 40)}
                    {e.beforeJson.length > 40 ? '...' : ''}
                  </span>
                )}
                {e.beforeJson && e.afterJson && ' | '}
                {e.afterJson && (
                  <span title={e.afterJson}>
                    A: {e.afterJson.substring(0, 40)}
                    {e.afterJson.length > 40 ? '...' : ''}
                  </span>
                )}
                {!e.beforeJson && !e.afterJson && '-'}
              </td>
