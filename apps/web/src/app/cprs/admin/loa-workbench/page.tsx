        ...
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
            ...
            >
              ...
              {['draft', 'submitted', 'pending', 'approved', 'denied', 'cancelled', 'expired'].map(
                (s) => (
            ...
            <button onClick={fetchLoas} style={BTN_PRIMARY}>
              Refresh
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div style={CARD}>
              <p style={{ color: '#888' }}>
                No LOA requests found. Create one from the Create LOA tab.
              </p>
            </div>
          ) : (
            <table style={TABLE}>
              ...
              <tbody>
                {loas.map((loa) => (
                  ...
                  >
                    <td style={TD}>
                      <code style={{ fontSize: 11 }}>{loa.id.slice(0, 8)}</code>
                    </td>
                    ...
                    <td style={TD}>
                      <span
                        style={{
                          ...BADGE,
                          background: STATUS_COLORS[loa.status] ?? '#666',
                          color: '#fff',
                        }}
                      >
                        {loa.status}
                    ...
                    <td style={TD}>
                      <div style={{ display: 'flex', gap: 4 }} onClick={(e) => e.stopPropagation()}>
                        {loa.status === 'draft' && (
                          <button
                            onClick={() => handleSubmitLoa(loa.id)}
                            style={{
                              ...BTN,
                              background: '#2563eb',
                              color: '#fff',
                        ...
                        {['submitted', 'pending'].includes(loa.status) && (
                          <>
                            <button
                              onClick={() => handleApproveLoa(loa.id)}
                              style={{
                                ...BTN,
                                background: '#16a34a',
                                color: '#fff',
            ...
            <div style={{ ...CARD, marginTop: 16 }}>
              ...
              >
                <h3 style={{ fontSize: 15, fontWeight: 600 }}>
                  LOA Detail: {selectedLoa.id.slice(0, 8)}
                </h3>
                <button onClick={() => setSelectedLoa(null)} style={BTN_SECONDARY}>
                  Close
                </button>
              </div>
              <div
              ...
              >
                <div>
                  <strong>Patient:</strong> {selectedLoa.patientName ?? selectedLoa.patientDfn}
                </div>
                <div>
                  <strong>Payer:</strong> {selectedLoa.payerId}
                </div>
                <div>
                  <strong>Status:</strong> {selectedLoa.status}
                </div>
                <div>
                  <strong>Mode:</strong> {selectedLoa.submissionMode ?? 'N/A'}
                </div>
                <div>
                  <strong>Member ID:</strong> {selectedLoa.memberId ?? 'N/A'}
                </div>
                <div>
                  <strong>Encounter:</strong> {selectedLoa.encounterDate ?? 'N/A'}
                </div>
                <div>
                  <strong>Diagnosis:</strong>{' '}
                  {selectedLoa.diagnosisCodes
                    ?.map((c) => (typeof c === 'string' ? c : c.code))
                    .join(', ') || 'None'}
                </div>
                <div>
                  <strong>Procedures:</strong>{' '}
                  {selectedLoa.procedureCodes
                    ?.map((c) => (typeof c === 'string' ? c : c.code))
                    .join(', ') || 'None'}
                </div>
              </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              <div>
                ...
                <input
                  style={INPUT}
                  value={formPatientDfn}
                  onChange={(e) => setFormPatientDfn(e.target.value)}
                  placeholder="e.g. 3"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. WORLDVISTA HOSPITAL"
                />
              </div>
