  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100 }}>
          ...
          {showForm && (
            <div style={{ padding: 16, border: "1px solid #e5e7eb", borderRadius: 8, marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                <label>
                  ...
                  <input value={newName} onChange={(e) => setNewName(e.target.value)}
                    style={{ padding: "6px 10px", border: "1px solid #d1d5db", borderRadius: 4, width: 240 }}
                    placeholder="Manila General Hospital" />
                </label>
                <label>
                  <span style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Code</span>
                  <input value={newCode} onChange={(e) => setNewCode(e.target.value)}
                    style={{ padding: "6px 10px", border: "1px solid #d1d5db", borderRadius: 4, width: 120 }}
                    placeholder="MGH" />
                </label>
                <label>
                  <span style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Environment</span>
              ...
              <button
                ...
                style={{ padding: "8px 20px", background: "#2563eb", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 500 }}
              >
                Create Site
              </button>
            </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {sites.map((s) => (
                <tr key={s.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  ...
                  <td style={{ padding: 8 }}>
                    <span style={{
                      padding: "2px 8px", borderRadius: 4, fontSize: 12,
                      background: s.status === "ready" ? "#dcfce7" : s.status === "active" ? "#dbeafe" : "#f3f4f6",
                      color: s.status === "ready" ? "#166534" : s.status === "active" ? "#1e40af" : "#374151",
                    }}>
                      {s.status}
                    </span>
                  </td>
              ...
              {sites.length === 0 && !loading && (
                <tr><td colSpan={6} style={{ padding: 16, textAlign: "center", color: "#9ca3af" }}>No pilot sites configured</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {tab === "preflight" && (
        <div>
          {!preflight && !loading && (
            <p style={{ color: "#6b7280" }}>Select a site and run preflight checks from the Sites tab.</p>
          )}
          ...
          {preflight && (
            <div>
              <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
                <div style={{
                  padding: 20, border: "1px solid #e5e7eb", borderRadius: 8, textAlign: "center", minWidth: 120,
                  background: preflight.readiness === "ready" ? "#dcfce7" : preflight.readiness === "partial" ? "#fef3c7" : "#fef2f2",
                }}>
                  <div style={{ fontSize: 32, fontWeight: 700 }}>{preflight.score}%</div>
                  <div style={{ fontSize: 13, color: "#6b7280" }}>Readiness Score</div>
                </div>
                <div style={{ padding: 20, border: "1px solid #e5e7eb", borderRadius: 8, textAlign: "center", minWidth: 100 }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: "#166534" }}>{preflight.passed}</div>
                  <div style={{ fontSize: 13, color: "#6b7280" }}>Passed</div>
                </div>
                <div style={{ padding: 20, border: "1px solid #e5e7eb", borderRadius: 8, textAlign: "center", minWidth: 100 }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: "#991b1b" }}>{preflight.failed}</div>
                ...
                <div style={{
                  ...
                  <div style={{
                    fontSize: 16, fontWeight: 600, textTransform: "uppercase",
                    color: preflight.readiness === "ready" ? "#166534" : preflight.readiness === "partial" ? "#92400e" : "#991b1b",
                  }}>
                    {preflight.readiness}
                  </div>
                  ...
                  {preflight.checks.map((c) => (
                    <tr key={c.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                      ...
                      <td style={{ padding: 8 }}>
                        <span style={{
                          padding: "2px 8px", borderRadius: 4, fontSize: 12,
                          background: c.severity === "critical" ? "#fef2f2" : c.severity === "warning" ? "#fef3c7" : "#eff6ff",
                          color: c.severity === "critical" ? "#991b1b" : c.severity === "warning" ? "#92400e" : "#1d4ed8",
                        }}>
                          {c.severity}
                        </span>
                      </td>
