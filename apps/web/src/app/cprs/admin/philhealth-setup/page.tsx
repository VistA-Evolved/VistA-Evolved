  ...
  const handleToggleReadiness = async (itemId: string, completed: boolean) => {
    try {
      const res = await fetch(
        `${API}/rcm/philhealth/setup/readiness/${encodeURIComponent(itemId)}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', ...csrfHeaders() },
          credentials: 'include',
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900 }}>
      ...
      >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, color: '#6b7280' }}>
                PRC: {p.prcLicenseNumber}
                {p.specialty ? ` | ${p.specialty}` : ''}
              </div>
            </div>
            <button
              onClick={() => handleRemoveProvider(p.prcLicenseNumber)}
              style={{
                padding: '4px 8px',
                background: '#fee2e2',
                color: '#dc2626',
        ...
        {showProviderForm ? (
          <div
            style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}
          >
            <input
            ...
            <input
              ...
              value={newPrcLicense}
              onChange={(e) => setNewPrcLicense(e.target.value)}
              style={{
                padding: '6px 8px',
                border: '1px solid #d1d5db',
                borderRadius: 4,
      ...
      {/* Readiness Checklist */}
      <section
        style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 6, padding: 16 }}
      >
