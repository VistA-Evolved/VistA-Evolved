...
const S = {
  ...
  tab: { padding: '10px 20px', cursor: 'pointer', fontSize: 14, borderTop: 'none', borderRight: 'none', borderBottom: 'none', borderLeft: 'none', background: 'none', color: '#555' } as React.CSSProperties,
  tabActive: { padding: '10px 20px', cursor: 'pointer', fontSize: 14, borderTop: 'none', borderRight: 'none', borderBottom: '3px solid #1a56db', borderLeft: 'none', background: 'none', color: '#1a56db', fontWeight: 700, marginBottom: -2 } as React.CSSProperties,
      ...
      {!loading && tab === 'insurance' && (
        <table style={S.table}>
          <thead><tr>
            ...
            <th style={S.th}>Reimburse Flag</th><th style={S.th}>City</th>
          </tr></thead>
          <tbody>
            {data.map((c: any, i: number) => (
              <tr key={c.ien ?? i} style={{ ...S.trHover, background: rowBg(i, hov === i) }}
                ...
                <td style={S.td}>{c.reimburseType ?? '-'}</td>
