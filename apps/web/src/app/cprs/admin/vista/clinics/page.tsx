...
const tabDefs: { id: Tab; label: string }[] = [
  ...
  { id: 'apptTypes', label: 'Appointment Types' },
];
...
export default function VistaClinicsPage() {
  ...
  function load(q?: string) {
    ...
    const url = tab === 'clinics'
      ? `/admin/vista/clinics?search=${encodeURIComponent(s)}&count=200`
      : '/admin/vista/appointment-types';
    apiFetch(url)
  ...
  function showDetail(ien: string) {
    apiFetch(`/admin/vista/clinics/${ien}`)
  ...
  return (
    <div style={S.page}>
      <h1 style={S.h1}>VistA Clinic Management</h1>
      <div style={S.tabBar}>
        {tabDefs.map(t => (
      ...
      {tab === 'clinics' && (
        <form onSubmit={onSearch}>
          <input style={S.search} placeholder="Search clinics..." value={search} onChange={e => setSearch(e.target.value)} />
        </form>
      )}
      ...
      {!loading && tab === 'clinics' && (
        <table style={S.table}>
          <thead><tr>
            <th style={S.th}>IEN</th><th style={S.th}>Name</th><th style={S.th}>Abbreviation</th>
            <th style={S.th}>Stop Code</th><th style={S.th}>Provider</th>
          </tr></thead>
          <tbody>
            {data.map((c: any, i: number) => (
              <tr key={c.ien ?? i} style={{ ...S.trHover, background: rowBg(i, hov === i) }}
                ...
                <td style={S.td}>{c.abbreviation ?? '-'}</td>
                <td style={S.td}>{c.stopCode ?? '-'}</td>
      ...
      {!loading && tab === 'apptTypes' && (
        <table style={S.table}>
          <thead><tr><th style={S.th}>IEN</th><th style={S.th}>Name</th><th style={S.th}>Duration</th></tr></thead>
          <tbody>
            {data.map((a: any, i: number) => (
              <tr key={a.ien ?? i} style={{ background: rowBg(i, hov === i) }}
                onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(-1)}>
                <td style={S.td}>{a.ien}</td><td style={S.td}>{a.name}</td><td style={S.td}>{a.duration ?? '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {detail && (
        <div style={S.detail}>
          <div style={S.detailTitle}>Clinic Detail: {detail.name ?? detail.ien}</div>
