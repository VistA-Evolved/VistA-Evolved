...
function ScoreTab() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await apiFetch<{ ok: boolean; score: AlignmentScore }>("/admin/alignment/score");
        setScore(data.score);
      } catch { setScore(null); }
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={item.label} style={{ background: "#1e293b", padding: 12, borderRadius: 6, border: "1px solid #334155", textAlign: "center" }}>
        ...
        <tbody>
          {score.panels.map((p) => (
            <tr key={p.panel} style={{ borderBottom: "1px solid #1e293b" }}>
              ...
              <td style={{ padding: 8, textAlign: "center", color: scoreColor(p.score), fontWeight: 700 }}>{p.score}</td>
              <td style={{ padding: 8, textAlign: "center" }}>{p.wiredRpcs}/{p.totalRpcs}</td>
...
function GatesTab() {
  ...
  return (
    <div>
      ...
      {report.gates.map((g) => (
        <div key={g.id} style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, padding: 16, marginBottom: 12 }}>
          ...
          <p style={{ color: "#cbd5e1", fontSize: 12, margin: 0, fontFamily: "monospace" }}>{g.detail}</p>
        </div>
      ))}
    </div>
  );
...
function TripwiresTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ ok: boolean; tripwires: Tripwire[] }>("/admin/alignment/tripwires");
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <button onClick={seed} style={{ padding: "8px 16px", background: "#8b5cf6", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
          Seed Defaults
        </button>
      </div>

      {loading ? <p style={{ color: "#94a3b8" }}>Loading...</p> : tripwires.length === 0 ? (
          ...
          <tbody>
            {tripwires.map((tw) => (
              <tr key={tw.id} style={{ borderBottom: "1px solid #1e293b" }}>
                ...
                <td style={{ padding: 8, textAlign: "center", fontWeight: 700, color: tw.fireCount > 0 ? "#ef4444" : "#22c55e" }}>{tw.fireCount}</td>
                <td style={{ padding: 8, textAlign: "center" }}>
                  <span style={{ color: tw.enabled ? "#22c55e" : "#94a3b8" }}>{tw.enabled ? "active" : "paused"}</span>
                </td>
...
function SnapshotsTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<{ ok: boolean; snapshots: Snapshot[] }>("/admin/alignment/snapshots");
  ...
  const capture = async () => {
    await apiFetch("/admin/alignment/snapshots", {
      method: "POST",
      body: JSON.stringify({ name: `Manual capture ${new Date().toISOString()}`, capturedBy: "admin" }),
    });
    await load();
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <button onClick={capture} style={{ padding: "8px 16px", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
          Capture Now
        </button>
      </div>

      {loading ? <p style={{ color: "#94a3b8" }}>Loading...</p> : snapshots.length === 0 ? (
          ...
          <tbody>
            {snapshots.map((s) => (
              <tr key={s.id} style={{ borderBottom: "1px solid #1e293b" }}>
                ...
                <td style={{ padding: 8, textAlign: "center", color: "#22c55e", fontWeight: 700 }}>{s.passRate}%</td>
                <td style={{ padding: 8, fontSize: 12, color: "#94a3b8" }}>{new Date(s.capturedAt).toLocaleString()}</td>
