...
function MetricCard({ label, value }: { label: string; value: string | number }) {
  return (
    <div style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: "12px 16px", minWidth: 140, textAlign: "center" }}>
    ...
    <div>
      ...
      {metrics.byAcuity && Object.keys(metrics.byAcuity).length > 0 && (
        <div>
          ...
          {Object.entries(metrics.byAcuity).map(([k, v]) => `${k}: ${v}`).join(", ")}
        </div>
      )}
    </div>
  );
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
            {Object.entries(metrics.byRoom).map(([roomId, info]: any) => (
              <span key={roomId} style={{
                padding: "4px 8px", borderRadius: 4, fontSize: 12,
                background: info.status === "in-use" ? "#fef3c7" : info.status === "available" ? "#d1fae5" : "#e2e8f0",
              }}>
                {roomId}: {info.status}
              </span>
            ))}
    ...
    <div>
      ...
      {metrics.byUnit && (
        <div style={{ marginBottom: 8 }}>
          ...
          {Object.entries(metrics.byUnit).map(([unit, info]: any) =>
            `${unit}: ${info.occupied}/${info.total}`
          ).join(", ")}
        </div>
      )}
      {metrics.byCodeStatus && Object.keys(metrics.byCodeStatus).length > 0 && (
        <div>
          ...
          {Object.entries(metrics.byCodeStatus).map(([k, v]) => `${k}: ${v}`).join(", ")}
        </div>
      )}
    </div>
  );
