...
export default function RemittanceIntakePage() {
  ...
  const formatCents = (c: number) =>
    `PHP ${(c / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}`;
    ...
    <div style={PAGE}>
      ...
      {message && (
        ...
        >
          ...
          <button
            onClick={() => setMessage('')}
            style={{ ...BTN_SECONDARY, marginLeft: 12, padding: '4px 8px', fontSize: 11 }}
          >
            dismiss
          </button>
        </div>
      ...
      {tab === 'documents' && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
            ...
            <button onClick={fetchDocs} style={BTN_PRIMARY}>
              Refresh
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div style={CARD}>
              <p style={{ color: '#888' }}>
                No remittance documents. Upload one from the Upload tab.
              </p>
            </div>
          ) : (
            <table style={TABLE}>
              ...
              <tbody>
                {docs.map((doc) => (
                  ...
                  >
                    <td style={TD}>
                      <code style={{ fontSize: 11 }}>{doc.id.slice(0, 8)}</code>
                    </td>
                    ...
                    <td style={TD}>
                      ...
                      {doc.underpaymentFlagged && (
                        <span
                          style={{ ...BADGE, background: '#dc2626', color: '#fff', marginLeft: 4 }}
                        >
                          UNDERPAID
                        </span>
                      )}
            ...
            <div style={{ ...CARD, marginTop: 16 }}>
              ...
              >
                <h3 style={{ fontSize: 15, fontWeight: 600 }}>
                  Document: {selectedDoc.id.slice(0, 8)}
                </h3>
                <button onClick={() => setSelectedDoc(null)} style={BTN_SECONDARY}>
                  Close
                </button>
              </div>
              <div
              ...
              >
                ...
                <div>
                  <strong>Total Paid:</strong> {formatCents(selectedDoc.totalPaid)}
                </div>
                <div>
                ...
                <div>
                  ...
                  {selectedDoc.underpaymentFlagged
                    ? formatCents(selectedDoc.underpaymentAmount ?? 0)
                    : 'None'}
                </div>
              </div>
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  {selectedDoc.auditTrail.map((e, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#888', padding: '1px 0' }}>
                      [{new Date(e.timestamp).toLocaleString()}] {e.action} by {e.actor}
                      {e.detail ? ` -- ${e.detail}` : ''}
                    </div>
                  ))}
                </div>
        ...
        <div style={{ maxWidth: 600 }}>
          <div style={CARD}>
            ...
            <p style={{ fontSize: 12, color: '#888', marginBottom: 12 }}>
              Upload metadata only. The actual file blob should be stored in a secure external
              store. Provide the storage reference below.
            </p>
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. MaxiCare Healthcare Corp."
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. maxicare-remit-2025-01.pdf"
                />
              </div>
              <div>
                ...
                <input
                  style={INPUT}
                  value={formStorageRef}
                  onChange={(e) => setFormStorageRef(e.target.value)}
                  placeholder="e.g. s3://bucket/remittances/abc123.pdf"
                />
              </div>
              <button onClick={handleUpload} style={BTN_PRIMARY}>
              ...
              <div style={CARD}>
                ...
                <div
                  style={{
                    fontSize: 28,
                    fontWeight: 700,
                    color: stats.underpaymentCount > 0 ? '#ef4444' : '#888',
                  }}
                >
                ...
                {stats.underpaymentTotal > 0 && (
                  <div style={{ fontSize: 11, color: '#ef4444' }}>
                    {formatCents(stats.underpaymentTotal)}
                  </div>
                )}
              </div>
