    ...
    <div style={{ padding: 24, fontFamily: "system-ui, sans-serif", maxWidth: 1200 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, borderBottom: "1px solid #ddd", paddingBottom: 8 }}>
        {(["templates", "quick-text", "specialty-packs", "stats"] as Tab[]).map((t) => (
          ...
          >
            {t === "templates" ? "Templates" : t === "quick-text" ? "Quick Text" : t === "specialty-packs" ? "Specialty Packs" : "Stats"}
          </button>
        ))}
      </div>
      ...
      {tab === "templates" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            />
            <button onClick={loadTemplates} style={{ padding: "6px 12px" }}>Refresh</button>
          </div>
          {loading ? (
            <p>Loading...</p>
          ) : templates.length === 0 ? (
            <p style={{ color: "#999" }}>No templates found. Seed specialty packs to get started.</p>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f5f5f5" }}>
                  <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #ddd" }}>Name</th>
                  <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #ddd" }}>Specialty</th>
                  <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #ddd" }}>Setting</th>
                  <th style={{ textAlign: "center", padding: 8, borderBottom: "1px solid #ddd" }}>Version</th>
              ...
              <tbody>
                {templates.map((t) => (
                  <tr key={t.id} style={{ borderBottom: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: 8 }}><span style={{ background: "#e3f2fd", padding: "2px 8px", borderRadius: 12, fontSize: 12 }}>{t.specialty}</span></td>
                    <td style={{ padding: 8 }}>{t.setting}</td>
                    <td style={{ textAlign: "center", padding: 8 }}>v{t.version}</td>
                    <td style={{ textAlign: "center", padding: 8 }}>
                      <span style={{
                        background: t.status === "published" ? "#e8f5e9" : t.status === "archived" ? "#fce4ec" : "#fff3e0",
                    ...
                    <td style={{ textAlign: "right", padding: 8 }}>
                      {t.status === "draft" && (
                        <button onClick={() => handlePublish(t.id)} style={{ marginRight: 4, fontSize: 12 }}>Publish</button>
                      )}
                      {t.status !== "archived" && (
                        <button onClick={() => handleArchive(t.id)} style={{ fontSize: 12, color: "#c62828" }}>Archive</button>
                      )}
                    </td>
                  </tr>
                ))}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f5f5f5" }}>
                  ...
                  <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #ddd" }}>Text (preview)</th>
              ...
              <tbody>
                {quickTexts.map((q) => (
                  <tr key={q.id}>
                    ...
                    <td style={{ padding: 8, maxWidth: 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{q.text}</td>
      ...
      {tab === "specialty-packs" && (
        <div>
          ...
          <button
            onClick={handleSeed}
            style={{ padding: "8px 20px", background: "#0066cc", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer" }}
          >
