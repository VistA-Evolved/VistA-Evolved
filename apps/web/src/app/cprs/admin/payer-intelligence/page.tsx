...
export default function PayerIntelligencePage() {
  ...
  const exportCsv = () => {
    ...
    const header =
      'Payer ID,Payer Name,Claims,Paid Claims,Denied,Underpaid,Avg Days,Denial Rate,Return Rate,Underpayment Rate';
    const rows = payers.map((p) =>
      [
        ...
        p.totalDenied,
        p.totalUnderpaid,
        (p.avgDaysToPayment ?? 0).toFixed(1),
        formatPercent(p.denialRate),
        formatPercent(p.returnRate),
        formatPercent(p.underpaymentRate),
      ].join(',')
    );
    ...
    a.href = url;
    a.download = `payer-intelligence-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Period Filter */}
      <div
        style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 16, fontSize: 12 }}
      >
        <label>
          From:
          <input
...
function KpiSection({
  ...
  if (payers.length === 0) {
    return (
      <div style={{ padding: 24, textAlign: 'center', color: '#6b7280', fontSize: 13 }}>
        No payer data in selected period.
      </div>
    );
  }
  ...
  const avgDays =
    payers.length > 0
      ? payers.reduce((s, p) => s + (p.avgDaysToPayment ?? 0), 0) / payers.length
      : 0;
    ...
    <div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
        ...
        <StatCard
          ...
          color={avgDays > 30 ? '#dc2626' : '#059669'}
        />
      </div>
        ...
        <thead>
          <tr style={{ borderBottom: '2px solid #e5e7eb', textAlign: 'left' }}>
            <SortTh
              ...
              field="payerName"
              onClick={onSort}
              arrow={sortArrow('payerName')}
            />
            <SortTh
              label="Claims"
              field="totalClaims"
              onClick={onSort}
              arrow={sortArrow('totalClaims')}
            />
            <SortTh
              label="Paid"
              field="totalPaid"
              onClick={onSort}
              arrow={sortArrow('totalPaid')}
            />
            <SortTh
              label="Denied"
              field="totalDenied"
              onClick={onSort}
              arrow={sortArrow('totalDenied')}
            />
            <SortTh
              label="Avg Days"
              field="avgDaysToPayment"
              onClick={onSort}
              arrow={sortArrow('avgDaysToPayment')}
            />
            <SortTh
              label="Denial %"
              field="denialRate"
              onClick={onSort}
              arrow={sortArrow('denialRate')}
            />
            <SortTh
              label="Underpay %"
              field="underpaymentRate"
              onClick={onSort}
              arrow={sortArrow('underpaymentRate')}
            />
          </tr>
        </thead>
        <tbody>
          {payers.map((p) => (
            <tr key={p.payerId} style={{ borderBottom: '1px solid #f3f4f6' }}>
              ...
              <td
                style={{
                  padding: '6px 8px',
                  color: (p.avgDaysToPayment ?? 0) > 30 ? '#dc2626' : '#059669',
                  fontWeight: 600,
                }}
              >
...
function AgingSection({
  ...
  return (
    <div>
      ...
      {/* Bar chart */}
      <div
        style={{ display: 'flex', gap: 12, alignItems: 'flex-end', height: 160, marginBottom: 24 }}
      >
...
function SortTh({
  ...
  label: string;
  field: keyof PayerKPI;
  onClick: (f: keyof PayerKPI) => void;
  arrow: string;
}) {
  return (
    <th
