  ...
  return (
    <div className={styles.cprsPage}>
      ...
      {safetyStatus && !safetyStatus.enabled && (
        ...
        >
          <strong>EXPORT-ONLY MODE</strong> -- Claims will be exported as X12 artifacts, not
          submitted to payers. Set CLAIM_SUBMISSION_ENABLED=true to enable live submission.
        </div>
      )}
...
function ClaimsTab() {
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    Promise.all([apiFetch('/rcm/claims?limit=100'), apiFetch('/rcm/claims/stats')])
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          {Object.entries(stats.byStatus ?? {}).map(([status, count]) => (
            ...
            >
              <span
                style={{
                  color: statusColors[status] ?? '#495057',
                  fontWeight: 600,
      ...
      {loading ? (
        ...
        <p style={{ color: '#6c757d', fontSize: 13 }}>
          No claims yet. Create a draft claim via the API.
        </p>
      ) : (
            ...
            {claims.map((c) => (
              <tr key={c.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '6px 10px' }}>
                  ...
                  {c.isDemo && (
                    <span
                      style={{ marginLeft: 4, fontSize: 10, color: '#dc3545', fontWeight: 600 }}
                    >
                      DEMO
                    </span>
                  )}
                ...
                <td style={{ padding: '6px 10px' }}>
                  {c.exportArtifactPath ? (
                    <span
                      style={{ fontSize: 10, color: '#198754', fontWeight: 600 }}
                      title={c.exportArtifactPath}
                    >
                      EXPORTED
                    </span>
                  ) : (
...
function PayersTab() {
  ...
  const refresh = useCallback(() => {
    ...
    Promise.all([apiFetch(`/rcm/payers?${params.toString()}`), apiFetch('/rcm/payers/stats')])
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', fontSize: 12 }}>
          ...
          {Object.entries(stats.byCountry ?? {}).map(([c, n]) => (
            <span key={c}>
              <strong>{c}:</strong> {n as number}
            </span>
          ))}
          {Object.entries(stats.byMode ?? {}).map(([m, n]) => (
            <span key={m}>
              <strong>{modeLabels[m] ?? m}:</strong> {n as number}
            </span>
          ))}
        </div>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
              ...
              <th style={{ padding: '8px 10px', borderBottom: '1px solid #dee2e6' }}>
                Integration
              </th>
              <th style={{ padding: '8px 10px', borderBottom: '1px solid #dee2e6' }}>Status</th>
            ...
            {payers.map((p) => (
              <tr key={p.payerId} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '6px 10px' }}>
                  <span
                    style={{
                      color: p.status === 'active' ? '#198754' : '#dc3545',
                      fontWeight: 600,
                    }}
                  >
                    {p.status}
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#6c757d' }}>
                    ...
                    <div>
                      Transactions: {c.supportedTransactions?.slice(0, 5).join(', ')}
                      {c.supportedTransactions?.length > 5 ? '...' : ''}
                    </div>
                    {h?.details && (
                      <div style={{ marginTop: 4, fontStyle: 'italic' }}>{h.details}</div>
                    )}
                  </div>
                </div>
              );
          ...
          {pipelineStats && (
            <div>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontSize: 12 }}>
                <span>
                  <strong>Total entries:</strong> {pipelineStats.total}
                </span>
                <span>
                  <strong>Errors:</strong> {pipelineStats.errorCount}
                </span>
              </div>
              {pipelineStats.byStage && Object.keys(pipelineStats.byStage).length > 0 && (
                <div style={{ marginTop: 8, fontSize: 12 }}>
                  <strong>By Stage:</strong>{' '}
                  {Object.entries(pipelineStats.byStage)
                    .map(([s, n]) => `${s}: ${n}`)
                    .join(' | ')}
                </div>
              )}
              {pipelineStats.byTransaction &&
                Object.keys(pipelineStats.byTransaction).length > 0 && (
                  <div style={{ marginTop: 4, fontSize: 12 }}>
                    <strong>By Transaction:</strong>{' '}
                    {Object.entries(pipelineStats.byTransaction)
                      .map(([t, n]) => `${t}: ${n}`)
                      .join(' | ')}
                  </div>
                )}
          ...
          {jobStats && (
            <div style={{ marginTop: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontSize: 12 }}>
                <span>
                  <strong>Total:</strong> {jobStats.total ?? 0}
                </span>
                {Object.entries(jobStats.byStatus ?? {}).map(([s, n]) => (
                  <span key={s}>
                    <strong>{s}:</strong> {n as number}
                  </span>
                ))}
              </div>
              {jobStats.byType && Object.keys(jobStats.byType).length > 0 && (
                <div style={{ marginTop: 4, fontSize: 12 }}>
                  ...
                  {Object.entries(jobStats.byType)
                    .map(([t, n]) => `${t}: ${n}`)
                    .join(' | ')}
                </div>
              )}
    ...
    <div>
      ...
      {loading ? (
        ...
        <p style={{ color: '#dc3545', fontSize: 13 }}>
          Failed to load durable job data: {data?.error ?? 'unknown error'}
        </p>
      ) : (
        <>
          {/* Stats summary */}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #dee2e6' }}>
                    Attempts
                  </th>
                  <th style={{ padding: '8px 12px', borderBottom: '1px solid #dee2e6' }}>
                    Created
  ...
  const checkGate = async () => {
    ...
    try {
      const res = await apiFetch(
        `/rcm/ops/evidence-gate/check?payerId=${encodeURIComponent(payerId.trim())}`
      );
      setResult(res);
    } catch {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#6c757d', margin: '0 0 16px 0' }}>
        Check whether a payer has verified evidence for each integration method. Missing or stale
        evidence blocks live integration calls.
      </p>
      ...
      {result?.ok && result.overview && (
        <div>
          <h4 style={{ fontSize: 13, margin: '16px 0 8px 0' }}>
            Evidence Overview for {result.payerId}
          </h4>
          <p
            style={{
              fontSize: 12,
              color: result.overview.hasAnyVerified ? '#0f5132' : '#842029',
              margin: '0 0 12px 0',
            }}
          >
          ...
          {Array.isArray(result.overview.methods) && result.overview.methods.length > 0 ? (
              ...
              <tbody>
                {result.overview.methods.map((m: any, i: number) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '8px 12px', color: '#6c757d', fontSize: 11 }}>
                      {m.lastVerified ? new Date(m.lastVerified).toLocaleString() : 'Never'}
                    </td>
                    <td style={{ padding: '8px 12px' }}>
                      {m.stale && (
                        <span
            ...
            <p style={{ color: '#6c757d', fontSize: 12 }}>
              No evidence records registered for this payer.
            </p>
          )}
        </div>
      )}
      ...
      {result?.ok && !result.overview && result.allowed !== undefined && (
        <div
          style={{
            padding: '12px 16px',
            marginTop: 12,
            background: result.allowed ? '#d1e7dd' : '#f8d7da',
            border: `1px solid ${result.allowed ? '#badbcc' : '#f5c6cb'}`,
            borderRadius: 4,
            fontSize: 12,
          }}
        >
...
function DraftFromVistaTab() {
  ...
  const prerequisitesCheck = () => {
    ...
    const wrapperStatus: 'ok' | 'missing' | 'pending' = wrapperRpc
      ? wrapperRpc.available
        ? 'ok'
        : 'pending'
      : 'pending';
    const issues: Array<{ field: string; source: string; status: 'ok' | 'missing' | 'pending' }> = [
      {
        field: 'Encounters (PCE)',
        source: 'ORWPCE VISIT',
        status: encounters?.ok && encounters.count > 0 ? 'ok' : 'missing',
      },
      {
        field: 'Insurance Coverage',
        source: 'IBCN INSURANCE QUERY',
        status: coverage?.ok && coverage.count > 0 ? 'ok' : 'missing',
      },
  ...
  return (
    <div>
        ...
        {rpcCheck && (
          <div style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 10, marginTop: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>
              ...
              {rpcCheck.allRequired === true && (
                <span style={{ color: '#198754', marginLeft: 8 }}>All required RPCs available</span>
              )}
              {rpcCheck.allRequired === false && (
                <span style={{ color: '#dc3545', marginLeft: 8 }}>Some required RPCs missing</span>
              )}
            </div>
            {rpcCheck.rpcs?.map((r: any, i: number) => (
              ...
              >
                ...
                {r.required && (
                  <span style={{ fontSize: 9, fontWeight: 700, color: '#dc3545' }}>REQUIRED</span>
                )}
      ...
      >
        ...
        <input
          value={patientIen}
          onChange={(e) => setPatientIen(e.target.value)}
          style={{
            padding: '4px 8px',
      ...
      {encounters && (
        <div
          style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 12, marginBottom: 16 }}
        >
      ...
      {coverage?.ok && coverage.count > 0 && (
        ...
        >
          ...
          {coverage.results.map((p: any) => p.insuranceName || p.policyId).join(', ')}
        </div>
      )}
      ...
      {encounters?.ok && encounters.count > 0 && step === 'select' && (
        <div
          style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 14, marginBottom: 16 }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <strong style={{ fontSize: 13 }}>Select Encounter(s)</strong>
            <span style={{ fontSize: 11, color: '#6c757d' }}>
              {encounters.count} encounters found
            </span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
                <th
                  style={{ padding: '6px 8px', borderBottom: '1px solid #dee2e6', width: 40 }}
                ></th>
            ...
            <tbody>
              {encounters.results.slice(0, 25).map((e: any, i: number) => (
                <tr
                  ...
                  onClick={() =>
                    setSelectedEncounter(selectedEncounter === e.visitIen ? '' : e.visitIen)
                  }
                >
          ...
          >
            {loading
              ? 'Generating...'
              : `2. Generate Draft${selectedEncounter ? ` (Visit ${selectedEncounter})` : ' (All Encounters)'}`}
          </button>
        </div>
      )}
      ...
      {step === 'review' && draftResult && (
        <div style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            >
              {draftResult.ok ? `${draftResult.candidates?.length || 0} DRAFTS` : 'ERROR'}
            </span>
          </div>
          ...
          {draftResult.errors?.length > 0 && (
            ...
            >
              {draftResult.errors.map((e: string, i: number) => (
                <div key={i}>{e}</div>
              ))}
            </div>
          )}
          ...
          {draftResult.candidates?.map((c: any, i: number) => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11, color: '#6c757d' }}>
                  {c.encounter?.dateTime} - {c.encounter?.location}
                </span>
                <code
                  style={{
                    fontSize: 10,
              ...
              >
                <div>
                  <strong>Diagnoses:</strong> {c.claim?.diagnoses?.length || 0}
                </div>
                <div>
                  <strong>Procedures:</strong> {c.claim?.lines?.length || 0}
                </div>
                <div>
                  <strong>Payer:</strong> {c.claim?.payerName || c.claim?.payerId || 'None'}
                </div>
                <div>
                  <strong>Total Charge:</strong> ${((c.claim?.totalCharge || 0) / 100).toFixed(2)}
                </div>
              </div>
                  ...
                  <ul style={{ margin: '4px 0 0 16px', padding: 0 }}>
                    {c.sourceMissing?.map((m: any, j: number) => (
                      <li key={j} style={{ color: '#856404', marginBottom: 2 }}>
                        ...
                        <span style={{ color: '#adb5bd' }}>({m.vistaSource})</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: '#856404' }}>
          <p style={{ margin: '4px 0' }}>
            <strong>VistA Files:</strong> {g.vistaFiles.join(', ')}
          </p>
          <p style={{ margin: '4px 0' }}>
            <strong>Target Routines:</strong> {g.targetRoutines.join(', ')}
          </p>
          <p style={{ margin: '4px 0' }}>
            <strong>Migration Path:</strong> {g.migrationPath}
          </p>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: '#6c757d' }}>
          Phase 39 -- VistA-First Billing Grounding
        </span>
      </div>
      ...
      {encounters && encounters.ok && (
        <div
          style={{ border: '1px solid #dee2e6', borderRadius: 6, padding: 14, marginBottom: 14 }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          {encounters.count === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
                  <th style={{ padding: '6px 8px', borderBottom: '1px solid #dee2e6' }}>
                    Visit IEN
                  </th>
                  <th style={{ padding: '6px 8px', borderBottom: '1px solid #dee2e6' }}>
                    Date/Time
        ...
        {icdResults && icdResults.ok && (
          <div>
            <p style={{ fontSize: 11, color: '#6c757d', marginBottom: 6 }}>
              {icdResults.count} results for &quot;{icdResults.searchText}&quot; (RPC:{' '}
              {icdResults.rpcUsed})
            </p>
...
function WorkqueuesTab() {
  ...
  const typeLabels: Record<string, string> = {
    rejection: 'Rejection (999/277CA)',
    denial: 'Denial (835)',
    missing_info: 'Missing Info (277)',
  };
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                <td style={{ padding: '6px 10px' }}>
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      color: '#fff',
                      background: priorityColors[item.priority] ?? '#6c757d',
                    }}
                  >
                    {item.priority}
                  </span>
                ...
                <td style={{ padding: '6px 10px', color: '#6c757d', fontSize: 11 }}>
...
function RulesTab() {
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    Promise.all([apiFetch('/rcm/rules?limit=200'), apiFetch('/rcm/rules/stats')])
              ...
              <tr key={rule.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                >
                  ...
                  {rule.condition?.field ??
                    rule.condition?.maxDaysFromService ??
                    rule.condition?.minCents ??
                    ''}
                </td>
                <td
                  style={{
...
function DirectoryTab() {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      await apiFetch('/rcm/directory/refresh', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        {stats && (
          <span style={{ fontSize: 11, color: '#6c757d' }}>
            {stats.total ?? 0} payers across {Object.keys(stats.byCountry ?? {}).length}{' '}
            jurisdictions
          </span>
        )}
        <button
        ...
        {['', 'US', 'PH', 'AU', 'SG', 'NZ'].map((c) => (
          <button
            ...
            style={{
              ...
              border: '1px solid #dee2e6',
              borderRadius: 4,
              background: countryFilter === c ? '#0d6efd' : '#fff',
        ...
        {subTabs.map((st) => (
          <button
            ...
            style={{
              padding: '8px 16px',
              border: 'none',
              borderBottom: subTab === st.id ? '2px solid #0d6efd' : '2px solid transparent',
              background: 'transparent',
              cursor: 'pointer',
              fontSize: 12,
              ...
              <tbody>
                {payers.map((p: any) => (
                  <tr key={p.payerId} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      <span
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 600,
                          background: typeColors[p.payerType] ?? '#6c757d',
                          color: p.payerType === 'CLEARINGHOUSE' ? '#000' : '#fff',
                        }}
                      >
                        {p.payerType}
                      </span>
                    ...
                    <td style={{ padding: '6px 10px', fontSize: 10, color: '#6c757d' }}>
                      {typeof p.regulatorySource === 'object' && p.regulatorySource
                        ? (p.regulatorySource.authority ?? JSON.stringify(p.regulatorySource))
                        : (p.regulatorySource ?? '-')}
                    </td>
                  </tr>
                ))}
                {payers.length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ padding: 16, textAlign: 'center', color: '#6c757d' }}>
                      No directory payers found. Click Refresh Directory.
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                {enrollment.map((pkt: any) => (
                  <tr key={pkt.payerId} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      <span
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 600,
                          background: statusColors[pkt.enrollmentStatus] ?? '#6c757d',
                          color: pkt.enrollmentStatus === 'IN_PROGRESS' ? '#000' : '#fff',
                        }}
                      >
                    ...
                    <td style={{ padding: '6px 10px', fontSize: 11 }}>
                      {(pkt.certRequirements ?? []).length}
                    </td>
                    <td style={{ padding: '6px 10px', fontSize: 11 }}>
                      {(pkt.goLiveChecklist ?? []).length}
                    </td>
                    <td
                      style={{
                        padding: '6px 10px',
...
function TransactionsTab() {
  ...
  const [loading, setLoading] = useState(true);
  const [subTab, setSubTab] = useState<'list' | 'stats' | 'translators' | 'connectivity' | 'dlq'>(
    'list'
  );
  ...
  const stateColors: Record<string, string> = {
    ...
    ack_accepted: '#198754',
    ack_rejected: '#dc3545',
    response_pending: '#ffc107',
    response_received: '#198754',
    reconciled: '#198754',
    failed: '#dc3545',
    cancelled: '#6c757d',
    dlq: '#dc3545',
              ...
              <tbody>
                {txns.map((t: any) => (
                  <tr key={t.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      <span
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 600,
                          background: stateColors[t.state] ?? '#6c757d',
                          color: '#fff',
                        }}
                      >
                        {t.state}
                    ...
                    <td style={{ padding: '6px 10px', fontFamily: 'monospace', fontSize: 10 }}>
                      {t.envelope?.controlNumber}
                    </td>
                    <td style={{ padding: '6px 10px', fontSize: 11 }}>
                      {t.envelope?.senderId?.trim()}
                    </td>
                    <td style={{ padding: '6px 10px', fontSize: 11 }}>
                      {t.envelope?.receiverId?.trim()}
                    </td>
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      {t.retryCount ?? 0}
                    </td>
                ...
                {txns.length === 0 && (
                  <tr>
                    <td colSpan={8} style={{ padding: 16, textAlign: 'center', color: '#6c757d' }}>
                      No transactions yet.
                    </td>
                  </tr>
                )}
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {Object.entries(stats.byState).map(([state, count]) => (
                      <span
                        ...
                        style={{
                          padding: '4px 10px',
                          background: stateColors[state] ?? '#6c757d',
                          color: '#fff',
                          borderRadius: 10,
                          fontSize: 11,
                        }}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      <span
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 600,
                          background: tr.available ? '#198754' : '#dc3545',
                          color: '#fff',
                        }}
                      >
                        {tr.available ? 'YES' : 'NO'}
                ...
                {translators.length === 0 && (
                  <tr>
                    <td colSpan={3} style={{ padding: 16, textAlign: 'center', color: '#6c757d' }}>
                      No translators registered.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {subTab === 'dlq' && (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {dlq.map((t: any) => (
                  <tr key={t.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '6px 10px', fontSize: 11, color: '#dc3545' }}>
                      {(t.errors ?? []).length > 0
                        ? t.errors[t.errors.length - 1]?.description
                        : '-'}
                    </td>
                    <td style={{ padding: '6px 10px', fontFamily: 'monospace', fontSize: 10 }}>
                ...
                {dlq.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ padding: 16, textAlign: 'center', color: '#198754' }}>
...
function GatewaysTab() {
  ...
  const statusColor = (s: string) =>
    s === 'green' ? '#198754' : s === 'amber' ? '#fd7e14' : '#dc3545';
  const statusIcon = (s: string) =>
    s === 'green' ? '\u2705' : s === 'amber' ? '\u26A0\uFE0F' : '\u274C';

  if (loading)
    return <div style={{ padding: 20, color: '#6c757d' }}>Loading gateway readiness...</div>;

  return (
    <div>
      <h3 style={{ margin: '0 0 8px 0', fontSize: 16 }}>National Gateway Readiness</h3>
      <p style={{ margin: '0 0 16px 0', fontSize: 12, color: '#6c757d' }}>
        Unified readiness checklist for all national gateways. Click a gateway for conformance
        details.
      </p>
      ...
      {selectedGateway && conformance && (
        <div style={{ marginTop: 20, border: '1px solid #dee2e6', borderRadius: 8, padding: 16 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <table
              style={{ width: '100%', borderCollapse: 'collapse', marginTop: 6, fontSize: 12 }}
            >
              <thead>
                <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
...
function AdaptersTab() {
  ...
  useEffect(() => {
    Promise.all([apiFetch('/rcm/adapters'), apiFetch('/rcm/adapters/health')])
      .then(([a, h]) => {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#6c757d', marginBottom: 12 }}>
        Higher-level workflow adapters for eligibility, claim status, submission, and denial
        handling. Connectors handle transport; adapters handle business workflow.
      </p>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead>
          <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
...
function JobsTab() {
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    Promise.all([apiFetch('/rcm/jobs/scheduler'), apiFetch('/rcm/jobs/queue/stats')])
      .then(([s, q]) => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        ...
        <button onClick={refresh} style={{ fontSize: 11, padding: '4px 12px', cursor: 'pointer' }}>
          Refresh
        </button>
      </div>
      ...
      >
        ...
        {scheduler?.stats && (
          <span style={{ marginLeft: 16, color: '#6c757d' }}>
            Processed: {scheduler.stats.totalProcessed} | Failed: {scheduler.stats.totalFailed} |
            Rate-Limited: {scheduler.stats.totalRateLimited}
            {scheduler.stats.uptimeMs > 0 &&
              ` | Uptime: ${Math.floor(scheduler.stats.uptimeMs / 1000)}s`}
          </span>
        )}
      </div>
        ...
        <tbody>
          {scheduler?.jobConfigs?.map((j: any) => (
            <tr key={j.type}>
              ...
              <td style={{ padding: '6px 8px', borderBottom: '1px solid #f0f0f0' }}>
                {(j.intervalMs / 1000).toFixed(0)}s
              </td>
              <td style={{ padding: '6px 8px', borderBottom: '1px solid #f0f0f0' }}>
                {j.rateLimitPerHour}
              </td>
              <td style={{ padding: '6px 8px', borderBottom: '1px solid #f0f0f0' }}>
                {j.rateLimitRemaining}
              </td>
              <td style={{ padding: '6px 8px', borderBottom: '1px solid #f0f0f0' }}>
                <span
                  style={{
        ...
        <div>
          ...
          >
            {Object.entries(queueStats).map(([k, v]) => (
              <div
                key={k}
                style={{ padding: 8, background: '#f8f9fa', borderRadius: 4, fontSize: 11 }}
              >
      ...
      >
        <strong>Note:</strong> Polling jobs are disabled by default. Set{' '}
        <code>RCM_ELIGIBILITY_POLLING=true</code> and <code>RCM_CLAIM_STATUS_POLLING=true</code> in
        env to enable. Rate limits prevent payer throttling.
      </div>
    </div>
  );
}
...
function OpsDashboardTab() {
  ...
  if (loading)
    return <div style={{ color: '#6c757d', fontSize: 13 }}>Loading ops dashboard...</div>;
  ...
  return (
    <div>
          ...
          Object.entries(systemHealth).map(([key, val]) => (
            <div
              ...
              style={{
                ...
                background: '#f8f9fa',
                borderRadius: 6,
                borderLeft: `3px solid ${healthColor(val as string)}`,
              }}
            >
              <div style={{ fontSize: 11, color: '#6c757d', marginBottom: 4 }}>
                {key.replace(/([A-Z])/g, ' $1').toUpperCase()}
              </div>
              <div style={{ fontSize: 14, fontWeight: 600, color: healthColor(val as string) }}>
                {String(val).toUpperCase().replace(/-/g, ' ')}
              </div>
            </div>
          ))}
      </div>
      ...
      <h4 style={{ fontSize: 13, marginBottom: 8 }}>
        Connector States ({connectivity?.summary?.totalConnectors ?? 0} total)
      </h4>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {connectivity?.summary &&
          ['connected', 'degraded', 'disconnected', 'pending'].map((s) => {
            const count =
              s === 'connected'
                ? connectivity.summary.connected
                : s === 'degraded'
                  ? connectivity.summary.degraded
                  : s === 'disconnected'
                    ? connectivity.summary.disconnected
          ...
          {(connectivity?.connectors ?? []).map((c: any) => {
            ...
            return (
              <tr key={c.connectorId}>
                ...
                >
                  {c.pendingTarget ? (
                    <span title={c.pendingTarget.migrationPath}>
                      {c.pendingTarget.configRequired?.join(', ') ?? 'Config required'}
                    </span>
                  ) : (
                    '-'
                  )}
          ...
          {(connectivity?.adapters ?? []).map((a: any) => {
            ...
            return (
              <tr key={a.adapterId}>
                ...
                >
                  {a.pendingTarget ? (
                    <span title={a.pendingTarget.migrationPath}>
                      {a.pendingTarget.configRequired?.join(', ') ?? 'Config required'}
                    </span>
                  ) : (
                    '-'
                  )}
      ...
      >
        ...
        {workqueues?.byType &&
          Object.entries(workqueues.byType).map(([k, v]) => (
            <div
              key={k}
              style={{ padding: 8, background: '#f8f9fa', borderRadius: 4, fontSize: 11 }}
            >
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <button
          onClick={handleEnqueueEligibility}
          style={{
            padding: '6px 14px',
            fontSize: 12,
            cursor: 'pointer',
        ...
        <button
          onClick={handleEnqueueStatusPoll}
          style={{
            padding: '6px 14px',
            fontSize: 12,
            cursor: 'pointer',
      ...
      {enqueueResult && (
        <div
          style={{
            padding: 8,
            background: enqueueResult.ok ? '#d1e7dd' : '#f8d7da',
            borderRadius: 4,
            fontSize: 11,
            marginBottom: 12,
...
function EligibilityTab() {
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    Promise.all([apiFetch('/rcm/eligibility/history?limit=50'), apiFetch('/rcm/eligibility/stats')])
      .then(([hist, st]) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body: any = {
        patientDfn: formData.patientDfn,
      ...
      if (formData.provenance === 'MANUAL') {
        body.manualResult = {
          eligible: formData.manualEligible,
          notes: formData.manualNotes || undefined,
        };
      }
      await apiFetch('/rcm/eligibility/check', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  if (loading)
    return <div style={{ color: '#6c757d', fontSize: 13 }}>Loading eligibility checks...</div>;
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
          {[
            ...
            {
              label: 'Avg ms',
              value: stats.avgResponseMs != null ? Math.round(stats.avgResponseMs) : '-',
              color: '#6c757d',
            },
          ].map((s) => (
            <div
        ...
        >
          ...
          >
            <label style={{ fontSize: 12 }}>
              ...
              <br />
              <input
                value={formData.patientDfn}
                onChange={(e) => setFormData((p) => ({ ...p, patientDfn: e.target.value }))}
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                onChange={(e) => setFormData((p) => ({ ...p, payerId: e.target.value }))}
                style={{ width: '100%', padding: 6, fontSize: 13 }}
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <select
                ...
                onChange={(e) => setFormData((p) => ({ ...p, provenance: e.target.value }))}
                style={{ width: '100%', padding: 6, fontSize: 13 }}
              >
          ...
          {formData.provenance === 'MANUAL' && (
            <div
              style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginBottom: 12 }}
            >
              <label style={{ fontSize: 12 }}>
                Eligible?
                <br />
                <select
                  value={String(formData.manualEligible)}
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, manualEligible: e.target.value === 'true' }))
                  }
                  style={{ width: '100%', padding: 6, fontSize: 13 }}
                >
              ...
              <label style={{ fontSize: 12 }}>
                ...
                <input
                  ...
                  onChange={(e) => setFormData((p) => ({ ...p, manualNotes: e.target.value }))}
                  style={{ width: '100%', padding: 6, fontSize: 13 }}
          ...
          <button
            ...
            disabled={submitting || !formData.patientDfn || !formData.payerId}
            style={{
              padding: '6px 20px',
              fontSize: 13,
              background: '#198754',
            ...
            {results.map((r: any) => (
              <tr key={r.id}>
                ...
                <td style={{ padding: '4px 8px', borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  >
                    {r.eligible === null || r.eligible === undefined
                      ? 'UNKNOWN'
                      : r.eligible
                        ? 'YES'
                        : 'NO'}
...
function ClaimStatusTab() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body: any = {
        ...
        payerId: formData.payerId,
        payerClaimId: formData.payerClaimId || undefined,
        provenance: formData.provenance,
      };
      if (formData.provenance === 'MANUAL') {
        body.manualResult = {
          claimStatus: formData.manualClaimStatus,
          notes: formData.manualNotes || undefined,
        };
      }
      await apiFetch('/rcm/claim-status/check', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  const loadTimeline = async () => {
    ...
    const data = await apiFetch(
      `/rcm/claim-status/timeline?claimRef=${encodeURIComponent(timelineRef)}`
    );
    setTimeline(data?.timeline ?? []);
  };

  if (loading)
    return <div style={{ color: '#6c757d', fontSize: 13 }}>Loading claim status checks...</div>;
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          ...
          >
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                onChange={(e) => setFormData((p) => ({ ...p, claimRef: e.target.value }))}
                style={{ width: '100%', padding: 6, fontSize: 13 }}
          ...
          {formData.provenance === 'MANUAL' && (
            ...
            >
              <label style={{ fontSize: 12 }}>
                ...
                <br />
                <select
                  value={formData.manualClaimStatus}
                  onChange={(e) =>
                    setFormData((p) => ({ ...p, manualClaimStatus: e.target.value }))
                  }
                  style={{ width: '100%', padding: 6, fontSize: 13 }}
                >
          ...
          <button
            onClick={handleSubmit}
            disabled={submitting || !formData.claimRef || !formData.payerId}
            style={{
              padding: '6px 20px',
              fontSize: 13,
              background: '#198754',
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'end' }}>
        <label style={{ fontSize: 12 }}>
          Timeline for Claim Ref:
          <br />
          <input
            value={timelineRef}
            onChange={(e) => setTimelineRef(e.target.value)}
      ...
      {timeline && (
        <div style={{ marginBottom: 16, padding: 12, background: '#f0f0f0', borderRadius: 6 }}>
          <strong style={{ fontSize: 13 }}>
            Timeline: {timelineRef} ({timeline.length} entries)
          </strong>
          {timeline.length === 0 ? (
            <div style={{ fontSize: 12, color: '#6c757d', marginTop: 8 }}>
              No status checks found for this claim.
            </div>
          ) : (
            <div style={{ marginTop: 8 }}>
              {timeline.map((t: any, i: number) => (
                <div
                  ...
                  style={{
                    padding: '4px 0',
                    borderBottom: i < timeline.length - 1 ? '1px solid #dee2e6' : 'none',
                    fontSize: 12,
                  }}
                >
                  ...
                  {t.paidAmountCents != null && (
                    <span> -- Paid: ${(t.paidAmountCents / 100).toFixed(2)}</span>
                  )}
                </div>
              ))}
            </div>
    ...
    <div>
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
          <div
            style={{ padding: '8px 16px', background: '#e2e3e5', borderRadius: 6, fontSize: 12 }}
          >
            <strong>Total:</strong> {stats.total}
          </div>
          <div
            style={{
              padding: '8px 16px',
              background: expiring.length > 0 ? '#fff3cd' : '#d1e7dd',
              borderRadius: 6,
              fontSize: 12,
            }}
          >
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder="Credential Value / Number"
              required
              style={{ padding: '4px 8px', fontSize: 12 }}
            />
            <input
        ...
        >
          ...
          >
            <h4 style={{ margin: 0, fontSize: 14 }}>
              {selectedCred.entityName} - {selectedCred.credentialType.toUpperCase()}
            </h4>
            <button
          ...
          <div style={{ fontSize: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            <div>
              <strong>Value:</strong> {selectedCred.credentialValue}
            </div>
            <div>
              <strong>Status:</strong> {selectedCred.status}
            </div>
            <div>
              <strong>Entity:</strong> {selectedCred.entityType} / {selectedCred.entityId}
            </div>
            <div>
              <strong>Authority:</strong> {selectedCred.issuingAuthority || '-'}
            </div>
            <div>
              <strong>Issued:</strong> {selectedCred.issuedAt?.slice(0, 10) || '-'}
            </div>
            <div>
              <strong>Expires:</strong> {selectedCred.expiresAt?.slice(0, 10) || '-'}
            </div>
            <div>
              <strong>Verified:</strong>{' '}
              {selectedCred.verifiedAt
                ? `${selectedCred.verifiedAt.slice(0, 10)} by ${selectedCred.verifiedBy}`
                : 'Not verified'}
            </div>
          </div>
          {documents.length > 0 && (
            <div style={{ marginTop: 8 }}>
              ...
              <ul style={{ margin: '4px 0', paddingLeft: 16, fontSize: 11 }}>
                {documents.map((d: any) => (
                  <li key={d.id}>
                    {d.fileName} ({d.mimeType}) -- {d.storagePath}
                  </li>
                ))}
              </ul>
            </div>
      ...
      {!loading && credentials.length === 0 && (
        <div style={{ fontSize: 12, color: '#6c757d', padding: 16, textAlign: 'center' }}>
          No credentials yet. Click &quot;+ Add Credential&quot; to begin.
        </div>
      )}
      ...
      {expiring.length > 0 && (
        ...
        >
          <strong style={{ fontSize: 12, color: '#664d03' }}>
          ...
          <ul style={{ margin: '4px 0', paddingLeft: 16, fontSize: 11, color: '#664d03' }}>
            {expiring.slice(0, 5).map((c: any) => (
              <li key={c.id}>
                {c.entityName} -- {c.credentialType} -- expires {c.expiresAt?.slice(0, 10)}
              </li>
            ))}
...
function AccreditationTab() {
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    Promise.all([apiFetch('/rcm/accreditation'), apiFetch('/rcm/accreditation/stats')])
    ...
    <div>
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder="Provider NPI / Facility ID"
              required
              style={{ padding: '4px 8px', fontSize: 12 }}
            />
          </div>
      ...
      {selectedAccred && (
        ...
        >
          ...
          >
            <h4 style={{ margin: 0, fontSize: 14 }}>
              {selectedAccred.payerName} -- {selectedAccred.providerEntityId}
            </h4>
            <button
              onClick={() => setSelectedAccred(null)}
              style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 14 }}
            >
              x
            </button>
          ...
          >
            <div>
              <strong>Payer ID:</strong> {selectedAccred.payerId}
            </div>
            <div>
              <strong>Status:</strong> {selectedAccred.status}
            </div>
            <div>
              <strong>Effective:</strong> {selectedAccred.effectiveDate?.slice(0, 10) || '-'}
            </div>
            <div>
              <strong>Expiration:</strong> {selectedAccred.expirationDate?.slice(0, 10) || '-'}
            </div>
            <div>
              <strong>Last Verified:</strong>{' '}
              {selectedAccred.lastVerifiedAt
                ? `${selectedAccred.lastVerifiedAt.slice(0, 10)} by ${selectedAccred.lastVerifiedBy}`
                : 'Not verified'}
            </div>
          </div>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <ul style={{ margin: '4px 0', paddingLeft: 16, fontSize: 11 }}>
                {selectedAccred.notes.map((n: any, i: number) => (
                  <li key={i}>
                    {n.date?.slice(0, 10)} ({n.author}): {n.text}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div>
            ...
            >
              ...
              <button
                onClick={() => handleAddTask(selectedAccred.id)}
                style={{
                  fontSize: 11,
                  padding: '2px 8px',
                  cursor: 'pointer',
        ...
        >
          <thead>
            <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
              ...
              <th style={{ padding: '6px 8px', borderBottom: '2px solid #dee2e6' }}>
                Provider Entity
              </th>
              <th style={{ padding: '6px 8px', borderBottom: '2px solid #dee2e6' }}>Status</th>
      ...
      {!loading && accreditations.length === 0 && (
        <div style={{ fontSize: 12, color: '#6c757d', padding: 16, textAlign: 'center' }}>
          No accreditation records yet. Click &quot;+ Add Accreditation&quot; to track payer
          enrollment status.
        </div>
      )}
    </div>
...
function ClaimLifecycleTab() {
  ...
  const refresh = useCallback(() => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        setError(err?.message || 'Failed to load claim lifecycle data');
      })
      .finally(() => setLoading(false));
  }, []);
  ...
  const handleScrub = async (id: string) => {
    try {
      ...
      setError(err?.message || 'Failed to scrub draft');
    }
  };
  ...
  const handleTransition = async (id: string, toStatus: string) => {
    try {
      ...
      if (!res.ok) {
        setError(res.error || `Failed to transition to ${toStatus}`);
        return;
      }
      refresh();
    } catch (err: any) {
      setError(err?.message || 'Failed to transition draft');
    }
  };
  ...
  return (
    <div>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <input
              ...
              style={{ padding: 6, fontSize: 12, border: '1px solid #ced4da', borderRadius: 4 }}
            />
            <input
          ...
          <tbody>
            {drafts.map((d: any) => (
              <tr key={d.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '4px 8px' }}>
                  ${((d.totalChargeCents || 0) / 100).toFixed(2)}
                </td>
                <td style={{ padding: '4px 8px' }}>
                  <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    {d.status === 'ready' && (
                      <button
                        onClick={() => handleTransition(d.id, 'submitted')}
                        style={{
                          padding: '2px 8px',
                          fontSize: 10,
                          background: '#0dcaf0',
                    ...
                    {(d.status === 'denied' || d.status === 'rejected') && (
                      <button
                        onClick={() => handleTransition(d.id, 'appealed')}
                        style={{
                          padding: '2px 8px',
                          fontSize: 10,
                          background: '#6f42c1',
      ...
      {!loading && subTab === 'drafts' && drafts.length === 0 && (
        <div style={{ fontSize: 12, color: '#6c757d', padding: 16, textAlign: 'center' }}>
          No claim drafts yet. Click &quot;+ New Draft&quot; to create one.
        </div>
      )}
      ...
      {!loading && subTab === 'metrics' && metrics && (
        <div>
          ...
          >
            <div
              style={{ background: '#f8f9fa', borderRadius: 8, padding: 16, textAlign: 'center' }}
            >
            ...
            >
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    metrics.firstPassRate != null && metrics.firstPassRate >= 90
                      ? '#198754'
                      : '#dc3545',
                }}
              >
          ...
          >
            ...
            <div
              style={{ background: '#f8f9fa', borderRadius: 8, padding: 16, textAlign: 'center' }}
            >
              <div style={{ fontSize: 18, fontWeight: 700 }}>{metrics.avgScrubScore ?? '--'}</div>
          ...
          {scrubMetrics && (
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                ...
                >
                  <div style={{ fontSize: 18, fontWeight: 700 }}>
                    {scrubMetrics.passRate != null ? `${scrubMetrics.passRate}%` : '--'}
                  </div>
                ...
                >
                  <div
                    style={{
                      fontSize: 18,
                      fontWeight: 700,
                      color: scrubMetrics.contractingNeededCount > 0 ? '#dc3545' : '#198754',
                    }}
                  >
                    {scrubMetrics.contractingNeededCount}
                  </div>
      ...
      {!loading && subTab === 'rules' && (
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
            Active Scrub Rules ({rules.length})
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
                ...
                <tr key={r.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <td
                    style={{
                      padding: '4px 8px',
                      fontSize: 10,
                      color: r.evidenceSource === 'contracting_needed' ? '#dc3545' : '#6c757d',
                    }}
                  >
                    {r.evidenceSource}
                  </td>
        ...
        <div>
          ...
          {!cfoLoading && !cfoData?.ok && (
            <div style={{ fontSize: 12, color: '#dc3545', padding: 16, textAlign: 'center' }}>
              Unable to load revenue summary. Ensure PostgreSQL is running and claim_draft table has
              data.
            </div>
          )}
          {!cfoLoading && cfoData?.ok && (
            <div>
              ...
              >
                ...
                >
                  <div style={{ fontSize: 24, fontWeight: 700, color: '#198754' }}>
                    $
                    {((cfoData.netRevenue?.netRevenueCents || 0) / 100).toLocaleString(undefined, {
                ...
                >
                  <div style={{ fontSize: 24, fontWeight: 700 }}>
                    $
                    {((cfoData.netRevenue?.totalChargeCents || 0) / 100).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                  <div style={{ fontSize: 11, color: '#6c757d' }}>Total Charges</div>
                ...
                >
                  <div
                    style={{
                      fontSize: 24,
                      fontWeight: 700,
                      color: cfoData.collectionRate?.rate >= 90 ? '#198754' : '#dc3545',
                    }}
                  >
                    {cfoData.collectionRate?.rate != null
                      ? `${cfoData.collectionRate.rate}%`
                      : '--'}
                  </div>
                ...
                {[
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: '#6c757d' }}>
                      $
                      {((b.data?.totalCents || 0) / 100).toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </div>
                  </div>
                    ...
                    <tbody>
                      {cfoData.payerMix.map((p: any) => (
                        <tr key={p.payerId} style={{ borderBottom: '1px solid #f0f0f0' }}>
                          ...
                          <td style={{ padding: '4px 8px' }}>
                            ${((p.totalChargeCents || 0) / 100).toFixed(2)}
                          </td>
                          <td style={{ padding: '4px 8px', color: '#198754' }}>
                            ${((p.totalPaidCents || 0) / 100).toFixed(2)}
                          </td>
              ...
              {cfoData.collectionRate?.totalClaimCount === 0 && (
                ...
                >
                  No claim drafts found for this period. Create claims via the Claim Drafts tab or
                  API to see CFO metrics.
                </div>
              )}
            </div>
          )}
      ...
      {/* ---- Aging Sub-Tab ---- */}
      {!loading && subTab === 'aging' && <AgingSubTab />}
    </div>
  );
}
...
function AgingSubTab() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/rcm/claim-lifecycle/drafts/aging?olderThanDays=${days}`)
      ...
      .catch((err) => {
        setAgingError(err?.message || 'Failed to load aging data');
      })
      .finally(() => setLoading(false));
  }, [days]);
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        ...
        <select
          ...
          style={{ padding: 4, fontSize: 12, border: '1px solid #ced4da', borderRadius: 4 }}
        >
          ...
          <tbody>
            {aging.map((a: any) => (
              <tr key={a.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '4px 8px' }}>
                  ${((a.totalChargeCents || 0) / 100).toFixed(2)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {!loading && aging.length === 0 && (
        <div style={{ fontSize: 12, color: '#198754', padding: 16, textAlign: 'center' }}>
          No aging denials found for the selected period.
        </div>
      )}
    </div>
  );
...
function EvidenceRegistryTab() {
  ...
  const handleArchive = async (id: string) => {
    try {
      await fetch(`${API_BASE}/rcm/evidence/${id}`, {
        method: 'DELETE',
        credentials: 'include',
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 12 }}>
        ...
        <button onClick={refresh} style={{ fontSize: 11, padding: '2px 10px', cursor: 'pointer' }}>
          Refresh
        </button>
      </div>
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 11 }}>
          ...
          {coverage && (
            <span>
              Coverage: <b>{coverage.coveragePercent ?? 0}%</b>
            </span>
          )}
          {coverage && (
            <span>
              Gaps:{' '}
              <b style={{ color: coverage.missingEvidence > 0 ? '#dc3545' : '#198754' }}>
                {coverage.missingEvidence ?? 0}
              </b>
            </span>
          )}
          {stats.byStatus &&
        ...
        {subTabs.map((st) => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              borderRadius: 4,
              background: subTab === st.id ? '#0d6efd' : '#e9ecef',
              color: subTab === st.id ? '#fff' : '#212529',
              border: 'none',
            }}
          >
            {st.label}
      ...
      {!loading && subTab === 'list' && (
        <div style={{ overflowX: 'auto' }}>
          ...
          {evidence.length === 0 && (
            <div style={{ fontSize: 12, color: '#6c757d', padding: 16, textAlign: 'center' }}>
              No evidence entries yet. Use the "+ Add Evidence" tab to create one.
            </div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          >
            {[
              ...
              {
                label: 'Missing',
                value: coverage.missingEvidence,
                color: coverage.missingEvidence > 0 ? '#dc3545' : '#198754',
              },
              { label: 'Coverage %', value: `${coverage.coveragePercent}%` },
            ].map((m) => (
              <div
                key={m.label}
                style={{ background: '#f8f9fa', padding: 8, borderRadius: 4, textAlign: 'center' }}
              >
                <div style={{ fontSize: 10, color: '#6c757d' }}>{m.label}</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: m.color ?? '#212529' }}>
                  {m.value}
                </div>
              </div>
            ))}
      ...
      {!loading && subTab === 'gaps' && (
        <div>
          {gaps.length === 0 ? (
            <div style={{ fontSize: 12, color: '#198754', padding: 16, textAlign: 'center' }}>
              No evidence gaps. All payers with active integration modes have backing evidence.
            </div>
          ) : (
            <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
              <thead>
        ...
        <div style={{ maxWidth: 500, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <label style={{ fontSize: 11 }}>
            ...
            <input
              ...
              placeholder="https://payer.com/edi-guide"
              style={{ width: '100%', padding: 4, fontSize: 12 }}
            />
          </label>
          <label style={{ fontSize: 11 }}>
          ...
          <label style={{ fontSize: 11 }}>
            ...
            <input
              ...
              placeholder="sftp, https, soap, rest"
              style={{ width: '100%', padding: 4, fontSize: 12 }}
            />
          </label>
          <label style={{ fontSize: 11 }}>
            ...
            <input
              ...
              placeholder="EDI Support: 1-800-XXX-XXXX"
              style={{ width: '100%', padding: 4, fontSize: 12 }}
            />
          </label>
          <label style={{ fontSize: 11 }}>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Requires clearinghouse enrollment..."
              style={{ width: '100%', padding: 4, fontSize: 12 }}
            />
          </label>
          <label style={{ fontSize: 11 }}>
