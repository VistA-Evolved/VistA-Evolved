...
const colors = {
  ...
  heuristicBg: '#ebf8ff',
  heuristicBorder: '#90cdf4',
  allergyBg: '#fff5f5',
  allergyBorder: '#fc8181',
  prnBg: '#f0fff4',
  scheduledBg: '#ebf8ff',
};
...
function HeuristicDisclaimer({ text }: { text: string }) {
  return (
    <div
      style={{
        background: colors.heuristicBg,
        border: `1px solid ${colors.heuristicBorder}`,
        borderRadius: 6,
        padding: '10px 14px',
        margin: '12px 0',
        fontSize: 13,
  ...
  return (
    <div
      style={{
        ...
        border: `2px solid ${colors.allergyBorder}`,
        borderRadius: 6,
        padding: '12px 16px',
        margin: '12px 0',
      }}
...
function DuplicateTherapyBanner({ duplicates }: { duplicates: DuplicateAlert[] }) {
  ...
  return (
    <div
      style={{
        background: colors.heuristicBg,
        border: `2px solid ${colors.heuristicBorder}`,
        borderRadius: 6,
        padding: '12px 16px',
        margin: '12px 0',
      }}
...
function ScheduleTab({ dfn }: { dfn: string }) {
  ...
  if (loading)
    return (
      <div style={{ padding: 20, color: colors.textMuted }}>Loading medication schedule...</div>
    );
  ...
  return (
    <div>
      {heuristicWarning && <HeuristicDisclaimer text={heuristicWarning} />}
      <DuplicateTherapyBanner duplicates={duplicates} />
      {pendingTargets.length > 0 && (
        <IntegrationPendingBanner
          targets={pendingTargets}
          context="Real-time due times require BCMA"
          status={schedStatus}
        />
      )}
...
function AllergiesTab({ dfn }: { dfn: string }) {
  ...
  if (loading)
    return <div style={{ padding: 20, color: colors.textMuted }}>Loading allergy data...</div>;
  if (error) return <div style={{ padding: 20, color: colors.danger }}>{error}</div>;
...
function AdminTab({ dfn }: { dfn: string }) {
  ...
  if (loading)
    return <div style={{ padding: 20, color: colors.textMuted }}>Loading medications...</div>;
        ...
        targets={[
          {
            rpc: 'PSB MED LOG',
            package: 'PSB',
            reason: 'Write administration record to BCMA Medication Log (file 53.79)',
          },
        ]}
  ...
  return (
    <div>
        ...
        targets={[
          {
            rpc: 'PSB MED LOG',
            package: 'PSB',
            reason: 'BCMA medication verification against active orders',
          },
          {
            rpc: 'PSJBCMA',
            package: 'PSB',
            reason: 'Barcode-to-medication lookup via PSJ BCMA routines',
          },
        ]}
      ...
      >
        ...
        >
          {['Right Patient', 'Right Medication', 'Right Dose', 'Right Route', 'Right Time'].map(
            (right) => (
              <div
                key={right}
                style={{
      ...
      >
        ...
        <div style={{ fontSize: 13, color: '#4a5568', lineHeight: 1.6 }}>
          ...
          scanning to verify the 5 Rights of medication administration before each dose. The system
          relies on the PSB (BCMA) and PSJ (Inpatient Pharmacy) packages, which are not included in
          the WorldVistA Docker sandbox.
        </div>
...
function EmarPageContent() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg }}>
      ...
      >
        <div>
          <span style={{ fontWeight: 600 }}>Patient:</span> {patientName || `DFN ${dfn}`}
        </div>
        {patientLocation && (
          <div>
            <span style={{ fontWeight: 600 }}>Location:</span> {patientLocation}
          </div>
        )}
        {patientRoomBed && (
          <div>
            <span style={{ fontWeight: 600 }}>Room/Bed:</span> {patientRoomBed}
          </div>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') handleDfnChange();
            }}
            style={{
              width: 60,
              padding: '2px 6px',
