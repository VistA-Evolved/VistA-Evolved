...
const BY_ID = new Map<string, WebModuleDefinition>(MODULE_DEFINITIONS.map((m) => [m.id, m]));
 ...
 */
export function filterEnabledModules(allModuleIds: string[], enabledIds: Set<string>): string[] {
  return allModuleIds.filter((id) => enabledIds.has(id) || BY_ID.get(id)?.alwaysEnabled);
}
