...
export default function GenericCodeSheetPanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/gec/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for GEC.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/gec/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}