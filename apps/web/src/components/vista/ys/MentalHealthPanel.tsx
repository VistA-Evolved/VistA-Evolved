...
const RPCS: { name: string; slug: string }[] = [{"name":"YS GAF API","slug":"ys-gaf-api"},{"name":"YTAPI NEW GAF","slug":"ytapi-new-gaf"},{"name":"YSRP ASI NARRATIVE","slug":"ysrp-asi-narrative"},{"name":"YSRP ASI ITEM","slug":"ysrp-asi-item"},{"name":"YTAPI ASI FACTORS","slug":"ytapi-asi-factors"}];

interface RpcResult { ok: boolean; rpcUsed?: string[]; data?: string[]; error?: string; }

export default function MentalHealthPanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/ys/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for YS.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/ys/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}