...
const RPCS: { name: string; slug: string }[] = [{"name":"RAMAG EXAM ORDER","slug":"ramag-exam-order"},{"name":"RAMAG EXAM REGISTER","slug":"ramag-exam-register"},{"name":"RAMAG ORDER CANCEL","slug":"ramag-order-cancel"},{"name":"RAMAG EXAM CANCEL","slug":"ramag-exam-cancel"},{"name":"RAMAG EXAM COMPLETE","slug":"ramag-exam-complete"},{"name":"RAMAG EXAMINED","slug":"ramag-examined"},{"name":"RAMAG EXAM STATUS REQUIREMENTS","slug":"ramag-exam-status-requirements"}];

interface RpcResult { ok: boolean; rpcUsed?: string[]; data?: string[]; error?: string; }

export default function RadiologyNuclearMedicinePanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/ra/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-xs text-muted-foreground">7 RPCs available</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for RA.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/ra/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}