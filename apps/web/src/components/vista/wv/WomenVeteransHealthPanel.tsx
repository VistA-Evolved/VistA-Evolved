// Auto-generated by vista-module-gen for Women Veterans Health (WV)
...
const RPCS: { name: string; slug: string }[] = [{"name":"ORQQPXRM GET WH LETTER TEXT","slug":"orqqpxrm-get-wh-letter-text"},{"name":"ORQQPXRM GET WH LETTER TYPE","slug":"orqqpxrm-get-wh-letter-type"},{"name":"WVRPCOR COVER","slug":"wvrpcor-cover"},{"name":"WVRPCOR POSTREP","slug":"wvrpcor-postrep"},{"name":"WVRPCOR SAVEDATA","slug":"wvrpcor-savedata"},{"name":"WVRPCOR EIE","slug":"wvrpcor-eie"},{"name":"WVRPCOR REASONS","slug":"wvrpcor-reasons"},{"name":"WVRPCOR SITES","slug":"wvrpcor-sites"},{"name":"WVRPCOR CONSAVE","slug":"wvrpcor-consave"},{"name":"WVRPCOR DETAIL","slug":"wvrpcor-detail"}];

interface RpcResult { ok: boolean; rpcUsed?: string[]; data?: string[]; error?: string; }

export default function WomenVeteransHealthPanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/wv/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-xs text-muted-foreground">10 RPCs available</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for WV.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/wv/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}