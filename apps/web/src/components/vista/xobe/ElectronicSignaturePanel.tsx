...
const RPCS: { name: string; slug: string }[] = [{"name":"XOBE ESIG GET CODE","slug":"xobe-esig-get-code"},{"name":"XOBE ESIG SET CODE","slug":"xobe-esig-set-code"},{"name":"XOBE ESIG GET DATA","slug":"xobe-esig-get-data"},{"name":"XOBE ESIG SET DATA","slug":"xobe-esig-set-data"},{"name":"XOBE ESIG IS DEFINED","slug":"xobe-esig-is-defined"}];

interface RpcResult { ok: boolean; rpcUsed?: string[]; data?: string[]; error?: string; }

export default function ElectronicSignaturePanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/xobe/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-xs text-muted-foreground">5 RPCs available</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for XOBE.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/xobe/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}