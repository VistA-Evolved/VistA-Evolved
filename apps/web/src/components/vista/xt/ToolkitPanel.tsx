...
const RPCS: { name: string; slug: string }[] = [{"name":"XTMUNIT-TEST LOAD","slug":"xtmunit-test-load"},{"name":"XTMUNIT-TEST NEXT","slug":"xtmunit-test-next"},{"name":"XTMUNIT-TEST GROUP LOAD","slug":"xtmunit-test-group-load"},{"name":"XT ECLIPSE M EDITOR","slug":"xt-eclipse-m-editor"},{"name":"XTDEBUG DO LINE","slug":"xtdebug-do-line"},{"name":"XTDEBUG NEXT","slug":"xtdebug-next"},{"name":"XTDEBUG START","slug":"xtdebug-start"},{"name":"XTDEBUG SYMBOL TABLE","slug":"xtdebug-symbol-table"},{"name":"XTDEBUG ADD BREAKPOINT","slug":"xtdebug-add-breakpoint"},{"name":"XTDEBUG ADD WATCH","slug":"xtdebug-add-watch"},{"name":"XTDEBUG DELETE BREAK","slug":"xtdebug-delete-break"},{"name":"XTDEBUG DELETE WATCH","slug":"xtdebug-delete-watch"},{"name":"XTDEBUG READ INPUT","slug":"xtdebug-read-input"}];

interface RpcResult { ok: boolean; rpcUsed?: string[]; data?: string[]; error?: string; }

export default function ToolkitPanel({ dfn: propDfn = '46' }: { dfn?: string }) {
  ...
  const callRpc = useCallback(async (slug: string) => {
    ...
    setSelectedRpc(slug);
    try {
      const res = await fetch(`${API_BASE}/vista/xt/rpc/${slug}?dfn=${dfn}`, { credentials: 'include' });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-xs text-muted-foreground">13 RPCs available</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 border rounded-lg p-3 max-h-96 overflow-y-auto">
          ...
          {RPCS.length === 0 ? (
            <p className="text-xs text-muted-foreground">No RPCs registered for XT.</p>
          ) : (
      ...
      <div className="text-xs text-muted-foreground">
        API: <code>${API_BASE}/vista/xt/rpc/&lt;slug&gt;?dfn=&lt;n&gt;</code>
      </div>
    </div>
  );
}