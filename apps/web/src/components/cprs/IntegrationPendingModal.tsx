...
export default function IntegrationPendingModal({ action, onClose }: IntegrationPendingModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {action.pendingNote && (
          <p style={{ margin: '0 0 12px', fontSize: 13, color: '#aaa' }}>{action.pendingNote}</p>
        )}

        <h4 style={{ margin: '12px 0 6px', fontSize: 13, color: '#88c0d0' }}>Target RPCs</h4>
            ...
            {targets.length === 0 ? (
              ...
              targets.map((rpc) => (
                <tr key={rpc} style={{ borderBottom: '1px solid #333' }}>
                  ...
                  <td style={{ padding: 4 }}>
                    <VivianBadge status={action.vivianPresence?.[rpc] ?? 'unknown'} />
                  </td>
                </tr>
              ))
            )}
