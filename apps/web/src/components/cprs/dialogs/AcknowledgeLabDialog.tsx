...
export default function AcknowledgeLabDialog() {
  ...
  async function handleAcknowledge() {
    if (!labIds.length) { setError('No lab results selected for acknowledgement'); return; }
    setSaving(true);
    setError('');
        ...
        <div className={styles.modalBody}>
          ...
          {success && (
            <div style={{
              ...
              {syncStatus === 'synced' ? `${labIds.length} lab result(s) acknowledged in VistA` : `${labIds.length} acknowledgement(s) stored locally (VistA sync pending)`}
            </div>
          )}
          ...
          {labIds.length > 0 && (
            <ul style={{ fontSize: 12, margin: '8px 0', paddingLeft: 20 }}>
              {labIds.slice(0, 10).map((id) => <li key={id}>Lab ID: {id}</li>)}
          ...
          <div className={styles.modalFooter}>
            ...
            <button className={`${styles.btn} ${styles.btnPrimary}`} onClick={handleAcknowledge} disabled={saving}>
