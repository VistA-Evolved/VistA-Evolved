...
async function testButtonClick(page: Page, btn: Locator, label: string): Promise<ClickResult> {
  ...
  const dialogSelector = "[role='dialog'], [class*='modal'], [class*='Modal'], [class*='dialog'], [class*='Dialog']";
  ...
  if (afterText !== beforeText) {
    ...
    const pendingMatch = (afterText || "").match(/pending|integration|not available|coming soon|not yet/i);
    if (pendingMatch) {
  ...
  const toasts = page.locator("[class*='toast'], [class*='Toast'], [role='alert'], [class*='notification']");
  ...
  for (const screen of CPRS_SCREENS) {
    test(`${screen.name} -- no dead clicks on visible buttons`, async ({ page }) => {
      ...
      if (deadClicks.length > 0) {
        const report = deadClicks.map(
          (d) => `  "${d.label}": ${d.details || "no response detected"}`
        ).join("\n");
        expect(
          deadClicks,
          `Dead clicks found on ${screen.name}:\n${report}`
        ).toHaveLength(0);
      }
  ...
  for (const screen of ADMIN_SCREENS) {
    test(`${screen.name} -- no dead clicks on tab buttons`, async ({ page }) => {
      ...
      const tabs = page.locator(
        "[role='tab']:visible, button:visible:not([disabled])"
      );
    ...
    for (const url of pages) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (text && text.length < 8 && text.match(/pending/i)) {
          ...
          console.warn(`Bare "pending" message at ${url}: "${text}" (consider adding target/next-step)`);
        }
      }
    }
