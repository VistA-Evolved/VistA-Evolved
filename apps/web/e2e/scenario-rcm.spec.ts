...
test.describe('Scenario 2: RCM User Workflow', () => {
  ...
  test('navigate to RCM admin and verify page loads', async ({ page }) => {
    ...
    const hasRcmContent = text.match(
      /Claim Workqueue|Payer|Revenue Cycle|RCM|Billing|Connectors|Audit/i
    );
  ...
  test('payer directory tab loads payer list', async ({ page }) => {
    ...
    const hasPayerContent = text.match(
      /payer|aetna|cigna|united|blue|humana|philhealth|search|registry|directory|pending/i
    );
  ...
  test('claims workqueue tab loads', async ({ page }) => {
    ...
    const hasClaimContent = text.match(
      /claim|Workqueue|export|status|draft|pending|EXPORT.ONLY|No claims/i
    );
  ...
  test('draft from VistA tab renders', async ({ page }) => {
    ...
    if (await draftTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const hasDraftContent = text.match(
        /patient|IEN|encounter|RPC|fetch|draft|pending|integration|VistA/i
      );
  ...
  test('connectors tab shows EDI pipeline status', async ({ page }) => {
    ...
    if (await connTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const hasConnContent = text.match(
        /connector|pipeline|clearinghouse|sandbox|health|status|pending|EDI/i
      );
  ...
  test('audit trail tab shows hash chain status', async ({ page }) => {
    ...
    if (await auditTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const hasAuditContent = text.match(/audit|chain|valid|hash|integrity|no entries|seq|trail/i);
  ...
  test('export-only safety banner checks', async ({ page }) => {
    ...
    const hasExportBanner = text.match(
      /EXPORT.ONLY|export.only|safety|submission.*disabled|review.only/i
    );
