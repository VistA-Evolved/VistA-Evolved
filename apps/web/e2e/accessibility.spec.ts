  ...
  test('login page passes critical a11y checks', async ({ page }) => {
    ...
    if (critical.length > 0) {
      const report = critical
        ...
        .join('\n');
      expect
        .soft(critical, `Login page a11y critical/serious violations:\n${report}`)
        .toHaveLength(0);
    }
  ...
  test('patient search passes critical a11y checks', async ({ page }) => {
    ...
    if (critical.length > 0) {
      const report = critical
        ...
        .join('\n');
      expect
        .soft(critical, `Patient search a11y critical/serious violations:\n${report}`)
        .toHaveLength(0);
    }
  });
  ...
  test('CPRS chart shell passes critical a11y checks', async ({ page }) => {
    ...
    if (critical.length > 0) {
      const report = critical
        ...
        .join('\n');
      expect
        .soft(critical, `Chart shell a11y critical/serious violations:\n${report}`)
        .toHaveLength(0);
    }
  });
});
