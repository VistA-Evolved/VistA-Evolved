...
test.describe("Clinical Flow Dead-Click Enforcement", () => {
  ...
  test("Cover Sheet — all sections display data or pending message", async ({ page }) => {
    ...
    const hasContent = (text || "").includes("Allerg") ||
  ...
  test("Problems tab — list renders and buttons respond", async ({ page }) => {
    ...
    const text = await main.textContent();
    const hasProblemsContent = (text || "").includes("Problem") ||
                                (text || "").includes("Active") ||
                                (text || "").includes("No ") ||
                                (text || "").includes("Add") ||
                                (text || "").includes("pending") ||
    ...
    if (await addBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const dialog = page.locator("[role='dialog'], [class*='modal'], [class*='Modal'], [class*='dialog']");
      const afterText = await (page.locator("main").or(page.locator("[class*='content']")).first()).textContent();
      const responded = (await dialog.count()) > 0 ||
                        afterText !== text ||
  ...
  test("Meds tab — list renders", async ({ page }) => {
    ...
    const text = await main.textContent();
    const hasMedsContent = (text || "").includes("Med") ||
                            (text || "").includes("Active") ||
  ...
  test("Notes tab — list renders and new note button responds", async ({ page }) => {
    ...
    const text = await main.textContent();
    const hasNotesContent = (text || "").includes("Note") ||
    ...
    const newBtn = page.locator("button").filter({ hasText: /New|Create|Write/i }).first();
    if (await newBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      expect(urlChanged || dialogOpened || stateChanged,
        "New Note button must navigate, open dialog, or change state").toBeTruthy();
