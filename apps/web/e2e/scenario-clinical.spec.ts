...
test.describe("Scenario 1: Clinician Clinical Workflow", () => {
  ...
  test("login -> patient search -> cover sheet -> problems -> add problem", async ({ page }) => {
    ...
    const heading = page.locator("text=Select a Patient").or(
      page.locator("text=Patient Search")
    ).first();
    ...
    // Open chart button
    const openBtn = page.locator("button").filter({ hasText: /Open Chart|Select/i }).first();
    ...
    if (await addBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const showsPending = (afterText || "").match(/pending|integration|not available|coming soon/i);
