...
test.describe("Secure Messaging — Clinician Flows", () => {
  ...
  test("Inbox tab renders and fetches /messaging/inbox", async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse((r) => r.url().includes("/messaging/inbox") && r.status() === 200, { timeout: 10_000 }),
      inboxBtn.click(),
    ]);
  ...
  test("Compose + Send → POST /messaging/compose fires + success UI shown", async ({ page }) => {
    ...
    const recipientInput = page.locator("input[placeholder*='mail group'], input[placeholder*='DUZ']").first();
  ...
  test("Sent tab fetches /messaging/sent after composing", async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse((r) => r.url().includes("/messaging/sent") && r.status() === 200, { timeout: 10_000 }),
      sentBtn.click(),
    ]);
  ...
  test("Mail groups fetch fires on compose tab", async ({ page }) => {
    ...
    const [response] = await Promise.all([
      page.waitForResponse(
        (r) => r.url().includes("/messaging/mail-groups"),
        { timeout: 10_000 },
      ),
      composeBtn.click(),
    ]);
...
test.describe("Secure Messaging — Portal Posture", () => {
  test("Portal send → POST /messaging/portal/send or pending with target RPCs", async ({ page }) => {
    ...
    const bodyInput = page.locator("textarea[placeholder*='message'], textarea[placeholder*='Message']").first();
