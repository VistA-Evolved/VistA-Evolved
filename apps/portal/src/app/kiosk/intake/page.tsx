...
export default function KioskStartPage() {
  ...
  const bigBtn: React.CSSProperties = {
    width: "100%", padding: "20px 32px", borderRadius: "12px",
  ...
  const bigInput: React.CSSProperties = {
    ...
    border: "2px solid #e5e7eb", fontSize: "20px", textAlign: "center",
    letterSpacing: "2px", marginTop: "12px",
  };

  const tabBtn = (active: boolean): React.CSSProperties => ({
    flex: 1, padding: "16px", fontSize: "18px", fontWeight: 600,
    ...
    <div style={containerStyle}>
        ...
        <div>
          <div style={{ marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              {[{ value: "en", label: "English" }, { value: "tl", label: "Tagalog" }].map((lang) => (
                <button
                  key={lang.value}
                  onClick={() => setLanguage(lang.value)}
                  style={{
                    flex: 1, padding: "16px", borderRadius: "8px", fontSize: "18px",
                    border: language === lang.value ? "3px solid #2563eb" : "2px solid #e5e7eb",
                    background: language === lang.value ? "#eff6ff" : "#fff",
                    cursor: "pointer", fontWeight: language === lang.value ? 700 : 400,
                  }}
                >
                  {lang.label}
                </button>
      ...
      <div style={{ marginTop: "48px", textAlign: "center" }}>
        <p style={{ fontSize: "12px", color: "#9ca3af" }}>
          VistA-Evolved Patient Intake System
        </p>
      </div>
    </div>
  );
