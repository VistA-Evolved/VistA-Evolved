...
export default function ActivityPage() {
  ...
  const loadEntries = useCallback(async (appendMode = false) => {
    try {
      setLoading(true);
    ...
    <div className="container">
      ...
      <div className="card" style={{ marginBottom: "1rem" }}>
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "center" }}>
          <div>
            <label style={{ fontSize: "0.75rem", display: "block", marginBottom: "0.25rem", color: "var(--portal-text-muted)" }}>Event Type</label>
            <select value={eventFilter} onChange={(e) => setEventFilter(e.target.value)} style={{ padding: "0.5rem", borderRadius: "4px", border: "1px solid #ccc" }}>
              {FILTER_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <div>
            <label style={{ fontSize: "0.75rem", display: "block", marginBottom: "0.25rem", color: "var(--portal-text-muted)" }}>Since</label>
            <input type="date" value={sinceFilter} onChange={(e) => setSinceFilter(e.target.value)} style={{ padding: "0.5rem", borderRadius: "4px", border: "1px solid #ccc" }} />
          </div>
          {(eventFilter || sinceFilter) && (
            <button onClick={() => { setEventFilter(""); setSinceFilter(""); }} style={{ background: "none", border: "none", color: "#3182ce", fontSize: "0.8125rem", cursor: "pointer", alignSelf: "flex-end", padding: "0.5rem 0" }}>
              Clear filters
            </button>
          )}
        </div>
      ...
      <div className="card">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: "0.5rem" }}>
                    <p style={{ margin: 0, fontSize: "0.875rem", fontWeight: 500 }}>
                      ...
                      {entry.isProxy && (
                        <span style={{ fontSize: "0.6875rem", background: "#fef3c7", color: "#92400e", padding: "0.0625rem 0.375rem", borderRadius: "8px", marginLeft: "0.375rem" }}>
                          Proxy
                        </span>
                      )}
                    </p>
                    <span style={{ fontSize: "0.75rem", color: "var(--portal-text-muted)", whiteSpace: "nowrap" }}>
                      {formatTimestamp(entry.timestamp)}
                    </span>
                  </div>
                  <p style={{ margin: "0.125rem 0 0", fontSize: "0.8125rem", color: "var(--portal-text-muted)" }}>
                    {entry.description}
                  </p>
                  <p style={{ margin: "0.125rem 0 0", fontSize: "0.6875rem", color: "var(--portal-text-muted)" }}>
                    {entry.actorName}{entry.ipAddress ? ` - ${entry.ipAddress}` : ""}
                  </p>
                </div>
              </div>
            ))}
        ...
        {hasMore && entries.length > 0 && (
          <div style={{ textAlign: "center", paddingTop: "1rem" }}>
            <button onClick={loadMore} disabled={loading} style={{ fontSize: "0.875rem", color: "#3182ce", background: "none", border: "1px solid #3182ce", borderRadius: "4px", padding: "0.375rem 1rem", cursor: "pointer" }}>
