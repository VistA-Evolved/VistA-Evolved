...
export default function IntakeSessionPage() {
  ...
  async function submitAnswers() {
    const batch = nextItems.map((item) => ({
      linkId: item.linkId,
      text: item.text,
      answer: answers[item.linkId] !== undefined
  ...
  async function submitIntake() {
    ...
    if (res.ok) {
      ...
      setNotice({ type: "success", text: "Your intake has been submitted. Your care team will review it." });
    } else {
  ...
  const btnSecondary: React.CSSProperties = {
    background: "transparent", color: "var(--portal-primary, #2563eb)",
    border: "1px solid var(--portal-primary, #2563eb)", borderRadius: "6px",
    padding: "8px 20px", cursor: "pointer", fontSize: "14px",
  };

  const inputStyle: React.CSSProperties = {
    width: "100%", padding: "10px 12px", borderRadius: "6px",
    border: "1px solid var(--portal-border, #e5e7eb)", fontSize: "15px",
    marginTop: "6px",
  };

  const redFlagStyle: React.CSSProperties = {
    background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "6px",
    padding: "8px 12px", marginTop: "6px", fontSize: "13px", color: "#991b1b",
  };
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "24px 16px" }}>
        <div style={{ ...cardStyle, borderColor: "#10b981" }}>
          ...
          <p style={{ color: "var(--portal-text-muted, #6b7280)", marginBottom: "16px" }}>
            Thank you for completing your pre-visit questionnaire.
            Your care team will review your responses before your appointment.
          </p>
          {summary?.sections?.redFlags?.length > 0 && (
            <div style={redFlagStyle}>
              <strong>Note:</strong> Your responses indicate items that your care team
              will prioritize during your visit.
            </div>
          )}
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
        ...
        <button style={btnSecondary} onClick={saveDraft}>Save Draft</button>
      </div>

      {/* Progress bar */}
      {progress && (
        <div style={{ marginBottom: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "var(--portal-text-muted, #6b7280)", marginBottom: "4px" }}>
        ...
        <div>
          {nextItems.map((item) => (
            <div key={item.linkId} style={cardStyle}>
              <label style={{ display: "block", fontSize: "15px", fontWeight: 500, marginBottom: "4px" }}>
                {item.text}
                    ...
                    return (
                      <button
                        ...
                        style={{
                          padding: "10px 24px",
                          borderRadius: "6px",
                          border: selected ? "2px solid var(--portal-primary, #2563eb)" : "1px solid var(--portal-border, #e5e7eb)",
                  ...
                  {item.answerOption.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          padding: "10px 16px", textAlign: "left", borderRadius: "6px",
                          border: selected ? "2px solid var(--portal-primary, #2563eb)" : "1px solid var(--portal-border, #e5e7eb)",
                          background: selected ? "#eff6ff" : "transparent",
              ...
              {item.type === "display" && (
                <p style={{ fontSize: "14px", color: "var(--portal-text-muted, #6b7280)", fontStyle: "italic" }}>
          ...
          <div style={{ display: "flex", gap: "12px", marginTop: "16px" }}>
            ...
            <button style={btnSecondary} onClick={saveDraft}>Save for Later</button>
          </div>
        </div>
      ) : (
          ...
          <div style={cardStyle}>
            ...
            <div style={{ marginBottom: "16px" }}>
              <p style={{ fontSize: "14px", fontWeight: 500 }}>
                {qrItems.length} questions answered across {progress?.sectionsComplete?.length || 0} sections
              </p>
            </div>
