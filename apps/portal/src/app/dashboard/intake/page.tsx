...
export default function IntakeStartPage() {
  ...
  async function loadSessions() {
    ...
    try {
      ...
      if (res.ok && res.sessions) {
        setSessions(
          res.sessions.filter(
            (s: any) => s.status !== 'filed' && s.status !== 'filed_pending_integration'
          )
        );
      }
    } catch {
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', padding: '24px 16px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '8px' }}>{t('title')}</h1>
      ...
      {/* Start new intake */}
      <div style={cardStyle}>
        <h2 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '12px' }}>{t('startNew')}</h2>
        <p
          style={{
            fontSize: '14px',
        ...
        >
          {t('language')} <strong>{tLang(locale)}</strong>
        ...
        {questions.length > 0 && (
          <div style={{ marginBottom: '16px' }}>
            ...
            <ul
              style={{ paddingLeft: '20px', margin: '0 0 8px', fontSize: '13px', color: '#6b7280' }}
            >
              ...
              {questions.length > 5 && (
                <li style={{ fontStyle: 'italic' }}>+{questions.length - 5} more...</li>
              )}
            </ul>
          </div>
        )}
