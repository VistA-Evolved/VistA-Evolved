...
export default function ShareRecordsPage() {
  ...
  const [newShareResult, setNewShareResult] = useState<{
    token: string;
    accessCode: string;
  } | null>(null);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
      ...
      <p style={{ color: '#666', marginBottom: 24 }}>
        Create secure, time-limited links to share selected health records with providers or family
        members.
      </p>
          ...
          <p style={{ margin: '4px 0', fontSize: 14 }}>
            ...
            >
              {typeof window !== 'undefined'
                ? `${window.location.origin}/share/${newShareResult.token}`
                : `(share link)`}
            </code>
          </p>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 6,
            border: 'none',
            background: creating ? '#6c757d' : '#007bff',
            color: '#fff',
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {shares.map((share) => {
            ...
            const isActive =
              !share.revokedAt && !share.locked && new Date(share.expiresAt) >= new Date();
            return (
              <div
                key={share.id}
                style={{
