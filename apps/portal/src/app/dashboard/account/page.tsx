    ...
    <div className="container">
      ...
      <div className="card" style={{ marginBottom: "1rem" }}>
        ...
        {session && (
          <div>
            ...
            <p><strong>Patient Profiles:</strong> {session.profiles.length}</p>
            <p><strong>MFA:</strong> {session.mfaEnabled ? "Enabled" : "Not enabled"}</p>
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ marginBottom: "1rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
          ...
          {devices.length > 0 && (
            <button onClick={handleRevokeAll} style={{ fontSize: "0.8125rem", color: "#e53e3e", background: "none", border: "none", cursor: "pointer" }}>
        ...
        {devices.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {devices.map((d) => (
              <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.5rem", background: "var(--portal-bg-alt, #f7f7f7)", borderRadius: "4px" }}>
                <div>
                  <p style={{ margin: 0, fontSize: "0.875rem" }}><strong>{d.deviceType}</strong> - {d.ipAddress}</p>
                  <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--portal-text-muted)" }}>
                    Last active: {new Date(d.lastActiveAt).toLocaleString()}
                  </p>
                </div>
                <button onClick={() => handleRevokeDevice(d.id)} style={{ fontSize: "0.75rem", color: "#e53e3e", background: "none", border: "1px solid #e53e3e", borderRadius: "4px", padding: "0.25rem 0.5rem", cursor: "pointer" }}>
                  Revoke
                </button>
              </div>
            ))}
