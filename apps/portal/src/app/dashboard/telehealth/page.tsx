...
async function runDeviceCheck(): Promise<DeviceCheckResult> {
  ...
  const ready =
    camera === 'granted' && microphone === 'granted' && webrtc && browser === 'supported';
...
export default function TelehealthPage() {
  ...
  const loadAppointments = useCallback(async () => {
    setLoading(true);
    const res = await apiFetch<{ upcoming: Appointment[]; past: Appointment[] }>(
      '/portal/appointments'
    );
    if (res.ok && res.data) {
      const upcoming = (res.data.upcoming || []).filter(
        (a: Appointment) => a.appointmentType === 'telehealth'
      );
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      ...
      if (res.ok && res.data?.waiting) {
        ...
        if (
          res.data.waiting.status === 'provider_joined' ||
          res.data.waiting.status === 'in_progress'
        ) {
          clearInterval(poll);
        }
      }
  ...
  const handleProceedToWaiting = async () => {
    ...
    const res = await apiFetch<{ room: TelehealthRoom | null }>(
  ...
  const handleJoinVisit = async () => {
    if (!room) return;
    const res = await apiFetch<{ joinUrl: string }>(
      `/portal/telehealth/rooms/${room.roomId}/join`,
      { method: 'POST' }
    );
    ...
    <div className="container">
          ...
          {loading ? (
            <div className="card">
              <p style={{ color: 'var(--portal-text-muted)' }}>
                Loading telehealth appointments...
              </p>
            </div>
            ...
            <div className="card">
              ...
              <div className="empty-state">
                ...
                <p>
                  You don&apos;t have any video visit appointments scheduled. Contact your care team
                  to schedule one.
                </p>
              </div>
            </div>
      ...
      {view === 'device-check' && selectedAppt && (
        <div className="card">
          ...
          >
            Let&apos;s make sure your camera, microphone, and internet connection are ready for your
            visit with <strong>{selectedAppt.providerName}</strong>.
          </p>
          ...
          {deviceResult && (
            <div style={{ marginTop: '1rem' }}>
              ...
              >
                ...
                <DeviceItem
                  label="WebRTC"
                  status={deviceResult.webrtc ? 'supported' : 'unsupported'}
                />
              </div>
              ...
              {deviceResult.issues.length > 0 && (
                ...
                >
                  ...
                  <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.25rem' }}>
                    {deviceResult.issues.map((issue, i) => (
                      <li key={i}>{issue}</li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="card">
          ...
          >
            ...
            >
              ...
              <br />
              {selectedAppt.clinicName} with {selectedAppt.providerName}
              <br />
              {new Date(selectedAppt.scheduledAt).toLocaleDateString()} at{' '}
              {new Date(selectedAppt.scheduledAt).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
              })}
            </div>
      ...
      >
        ...
        encrypted end-to-end. No personal health information is included in meeting links. Recording
        requires explicit consent and is not enabled in this version.
      </div>
    </div>
  );
}
...
function DeviceItem({ label, status }: { label: string; status: string }) {
  ...
  return (
    ...
    >
      ...
      <span>
        <strong>{label}:</strong> {status.replace(/_/g, ' ')}
      </span>
    </div>
  );
}
