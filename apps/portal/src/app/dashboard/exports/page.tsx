...
export default function ExportPage() {
  ...
  const [shcCapabilities, setShcCapabilities] = useState<{ enabled: boolean; datasets: { id: string; label: string; available: boolean }[] } | null>(null);
  ...
  const handleShcExport = async (dataset: string) => {
    ...
    setShcResult(null);
    try {
      const res = await fetch(`${API_BASE}/portal/export/shc/${dataset}`, { credentials: "include" });
      const data = await res.json();
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 24 }}>
      ...
      {/* Full Record Exports */}
      <div style={{ background: "#f8f9fa", border: "1px solid #dee2e6", borderRadius: 8, padding: 20, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <button
            ...
            style={{
              ...
              color: "#fff", cursor: downloading ? "not-allowed" : "pointer", fontSize: 14,
            }}
          >
          ...
          <button
            ...
            style={{
              padding: "10px 20px", borderRadius: 6, border: "1px solid #007bff",
          ...
          {SECTIONS.map((sec) => (
            <button
              ...
              style={{
                ...
                background: "#fff", cursor: downloading ? "not-allowed" : "pointer",
                textAlign: "left", fontSize: 14, display: "flex", alignItems: "center", gap: 8,
              }}
            >
              <span>{sec.icon}</span>
      ...
      {/* SMART Health Cards */}
      <div style={{ background: "#f0f7ff", border: "1px solid #b8daff", borderRadius: 8, padding: 20 }}>
        <h2 style={{ fontSize: 18, margin: "0 0 8px" }}>
          SMART Health Cards
          <span style={{ fontSize: 12, marginLeft: 8, color: "#856404", background: "#fff3cd", padding: "2px 8px", borderRadius: 12 }}>
            Preview
          </span>
        </h2>
        <p style={{ color: "#666", fontSize: 14, marginBottom: 16 }}>
        ...
        {shcCapabilities && !shcCapabilities.enabled && (
          <div style={{ padding: 12, background: "#fff3cd", border: "1px solid #ffc107", borderRadius: 6, marginBottom: 12, fontSize: 13 }}>
            SMART Health Cards are not currently enabled. Contact your administrator to enable this feature.
          </div>
        )}
        ...
        {shcCapabilities?.datasets.map((ds) => (
          <div key={ds.id} style={{ marginBottom: 12 }}>
            <button
              ...
              style={{
                padding: "10px 20px", borderRadius: 6, border: "1px solid #28a745",
                background: ds.available ? "#28a745" : "#ccc", color: "#fff",
                cursor: ds.available ? "pointer" : "not-allowed", fontSize: 14,
              }}
            >
        ...
        {shcResult && (
          <div style={{ marginTop: 16, background: "#d4edda", border: "1px solid #c3e6cb", borderRadius: 8, padding: 16 }}>
            <h3 style={{ margin: "0 0 8px", fontSize: 16, color: "#155724" }}>Health Card Generated</h3>
            {shcResult.meta?.devMode && (
              <p style={{ margin: "0 0 8px", fontSize: 12, color: "#856404", background: "#fff3cd", padding: "4px 8px", borderRadius: 4, display: "inline-block" }}>
                Development mode — not a production credential
              </p>
            )}
            <div style={{ marginTop: 8 }}>
              <label style={{ fontWeight: 600, fontSize: 13 }}>SHC URI (for QR code):</label>
              <pre style={{ background: "#fff", padding: 8, borderRadius: 4, fontSize: 11, wordBreak: "break-all", maxHeight: 100, overflow: "auto", margin: "4px 0" }}>
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: "#666" }}>
              ...
              <span>Issued: {shcResult.meta?.issuedAt ? new Date(shcResult.meta.issuedAt).toLocaleString() : "N/A"}</span>
            </div>
            <button
