...
export default function ProfilePage() {
  ...
  return (
    <div className="container">
      ...
      {notice && (
        <div style={{ padding: "0.5rem 0.75rem", borderRadius: 4, marginBottom: "1rem", background: "#dcfce7", color: "#166534", fontSize: "0.8125rem" }}>
          {notice}
        </div>
      )}
        ...
        <>
              ...
              <div className="card">
                ...
                {demo ? (
                  <div style={{ fontSize: "0.875rem" }}>
                    <div style={{ marginBottom: "0.375rem" }}><strong>Name:</strong> {demo.name}</div>
                    <div style={{ marginBottom: "0.375rem" }}><strong>Sex:</strong> {demo.sex}</div>
                    <div><strong>Date of Birth:</strong> {demo.dob}</div>
                  </div>
                ) : (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div className="card">
                ...
                {Object.entries(settings.notifications || {}).map(([key, val]) => (
                  <label key={key} style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.375rem", fontSize: "0.875rem" }}>
                    <input type="checkbox" checked={val as boolean} onChange={(e) => handleNotifToggle(key, e.target.checked)} />
              ...
              <div className="card">
                ...
                <div style={{ marginBottom: "0.5rem" }}>
                  ...
                  <select
                    ...
                    style={{ display: "block", padding: "0.375rem", borderRadius: 4, border: "1px solid #cbd5e1", marginTop: "0.25rem" }}
                  >
                ...
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.875rem", marginBottom: "0.375rem" }}>
                  <input type="checkbox" checked={settings.display?.highContrast || false} onChange={(e) => handleDisplayChange("highContrast", e.target.checked)} />
                  High Contrast
                </label>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.875rem" }}>
                  <input type="checkbox" checked={settings.display?.compactView || false} onChange={(e) => handleDisplayChange("compactView", e.target.checked)} />
                  Compact View
                </label>
              </div>
            </div>
          ...
          {tab === "sharing" && (
            <div>
              <div className="card" style={{ marginBottom: "1rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                  ...
                  <button onClick={handleCreateShare}
                    style={{ padding: "0.375rem 0.75rem", background: "#2563eb", color: "#fff", border: "none", borderRadius: 4, fontWeight: 600, cursor: "pointer", fontSize: "0.8125rem" }}>
                    Create Share Link
                  </button>
                </div>
                <p style={{ fontSize: "0.8125rem", color: "#64748b" }}>
                  Create a secure, time-limited link to share selected health records with another provider or caregiver.
                  The recipient will need the access code and your date of birth to view the records.
                </p>

                {newShareToken && (
                  <div style={{ marginTop: "0.75rem", padding: "0.75rem", background: "#f0f9ff", borderRadius: 6, border: "1px solid #bae6fd" }}>
                    <div style={{ fontWeight: 600, fontSize: "0.875rem", marginBottom: "0.375rem" }}>Share Created</div>
                    <div style={{ fontSize: "0.8125rem", marginBottom: "0.25rem" }}>
                      <strong>Access Code:</strong>{" "}
                      <code style={{ background: "#e0f2fe", padding: "0.125rem 0.375rem", borderRadius: 3, fontWeight: 700 }}>{newShareCode}</code>
                    </div>
                    <div style={{ fontSize: "0.75rem", color: "#0c4a6e" }}>
                      Save this code now — it will not be shown again. Share the link and code with your recipient.
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="card">
                <h3 style={{ margin: "0 0 0.75rem" }}>Active Shares ({shares.filter((s: any) => !s.revokedAt && new Date(s.expiresAt) > new Date()).length})</h3>
                {shares.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {shares.map((s: any) => {
                      ...
                      return (
                        <div key={s.id} style={{ padding: "0.5rem", background: "#f8fafc", borderRadius: 6, border: "1px solid #e2e8f0" }}>
                          ...
                          <div style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.125rem" }}>
                            Sections: {(s.sections || []).join(", ")} · Expires: {new Date(s.expiresAt).toLocaleDateString()}
                            · Accessed: {s.accessCount || 0} time(s)
                          </div>
                          {!revoked && !expired && (
                            <button onClick={() => handleRevoke(s.id)}
                              style={{ fontSize: "0.75rem", color: "#dc2626", background: "transparent", border: "none", cursor: "pointer", marginTop: "0.25rem", textDecoration: "underline" }}>
                              Revoke
                            </button>
                          )}
                        </div>
