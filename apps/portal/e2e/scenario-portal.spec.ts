...
test.describe('Scenario 3: Patient Portal Workflow', () => {
  ...
  test('login page renders with form fields', async ({ page }) => {
    ...
    await expect(
      page.locator('input[id="username"]').or(page.locator("input[type='text']")).first()
    ).toBeVisible();

    await expect(
      page.locator('input[id="password"]').or(page.locator("input[type='password']")).first()
    ).toBeVisible();
  ...
  test('health records page shows data or honest pending', async ({ page }) => {
    ...
    const hasContent = (text || '').match(
      /health|record|allerg|vital|condition|result|pending|EHR|Local/i
    );
  ...
  test('medications page shows data or honest pending', async ({ page }) => {
    ...
    const hasContent = (text || '').match(/medic|prescription|refill|active|pending|no medic/i);
  ...
  test('appointments page shows data or honest pending', async ({ page }) => {
    ...
    const hasContent = (text || '').match(/appoint|schedul|upcoming|past|pending|no appoint/i);
  ...
  test('messages page shows data or honest pending', async ({ page }) => {
    ...
    const hasContent = (text || '').match(/message|inbox|secure|compose|pending|no message/i);
  ...
  test('telehealth page loads without error', async ({ page }) => {
    ...
    const hasContent = (text || '').match(/teleh|video|visit|room|device|pending|no.*visit/i);
