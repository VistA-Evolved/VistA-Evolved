...
interface LoginResult {
  ...
  session: { patientDfn: string; patientName: string };
}
...
test.describe("Health record sections", () => {
  ...
  for (const section of PENDING_SECTIONS) {
    test(`GET /portal/health/${section} returns integration pending`, async ({
      request,
    }) => {
      const res = await authedGet(
        request,
...
test.describe("PDF export", () => {
  ...
  test("export section PDF returns application/pdf", async ({ request }) => {
    const res = await authedGet(
      request,
      "/portal/export/section/allergies",
      cookie
    );
    expect(res.status()).toBe(200);
  ...
  test("export full record PDF returns application/pdf", async ({
    request,
  }) => {
...
test.describe("Share lifecycle", () => {
  ...
  test("create → preview → verify → revoke → denied", async ({ request }) => {
    ...
    const previewRes = await request.get(
      `${API}/portal/share/preview/${token}`
    );
    ...
    const revokeRes = await authedPost(
      request,
      `/portal/shares/${shareId}/revoke`,
      cookie
    );
    expect(revokeRes.status()).toBe(200);

    // 5. After revoke — verify fails
    const verifyRes2 = await request.post(
      `${API}/portal/share/verify/${token}`,
      {
        data: { accessCode, patientDob },
      }
...
test.describe("Secure messaging", () => {
  ...
  test("create draft → send → appears in sent", async ({ request }) => {
    ...
    // Verify in drafts
    const draftsRes = await authedGet(
      request,
      "/portal/messages/drafts",
      cookie
    );
    ...
    // Send
    const sendRes = await authedPost(
      request,
      `/portal/messages/${msgId}/send`,
      cookie
    );
    expect(sendRes.status()).toBe(200);

    // Verify in sent
    const sentRes = await authedGet(
      request,
...
test.describe("Appointments", () => {
  ...
  test("request appointment → appears in list", async ({ request }) => {
    ...
    const cancelRes = await authedPost(
      request,
      `/portal/appointments/${apptId}/cancel`,
      cookie,
      { reason: "E2E cleanup" }
    );
  ...
  test("request appointment missing fields returns 400", async ({
    request,
  }) => {
...
test.describe("Proxy and sensitivity", () => {
  ...
  test("sensitivity evaluate blocks flagged categories", async ({
    request,
  }) => {
    const res = await authedPost(
      request,
      "/portal/proxy/evaluate",
      cookie,
      {
        isProxy: true,
        isMinor: false,
...
test.describe("Audit trail", () => {
  ...
  test("audit events have hashed patient IDs (no raw DFN)", async ({
    request,
  }) => {
    const res = await authedGet(request, "/portal/audit/events", cookie);
    const body = await res.json();
...
test.describe("Share page exists", () => {
  test("share/[token] page returns 200 for any token", async ({
    request,
  }) => {
