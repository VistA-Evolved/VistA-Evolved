...
if (!skipGates) {
  ...
  logSection("TypeScript: API", tscApiPass ? "pass" : "fail", tscApiPass ? "clean" : `${gateResults.typecheck.api.errors} errors`);
...
if (existsSync(certPosturePath)) {
  postureSnapshot.domains.certification = { exists: true, file: "apps/api/src/posture/certification-posture.ts" };
...
logSection("Posture Modules", postureModuleCount >= 5 ? "pass" : "fail",
...
const auditFiles = [
  { name: "immutable", file: "apps/api/src/lib/immutable-audit.ts", sink: "logs/immutable-audit.jsonl" },
  { name: "imaging", file: "apps/api/src/services/imaging-audit.ts", sink: "logs/imaging-audit.jsonl" },
...
for (const { name, file, sink } of auditFiles) {
  ...
  if (sourceExists) {
    ...
    hashChainPresent = content.includes("sha256") || content.includes("SHA-256") || content.includes("prevHash");
  }
...
logSection("Audit Trail Sources", allTrailsPresent ? "pass" : "fail",
...
logSection("Compliance Docs", complianceCount >= 4 ? "pass" : "fail",
  `${complianceCount}/${requiredComplianceDocs.length} present`);

writeFileSync(join(outDir, "compliance-docs.json"), JSON.stringify({
  collectedAt: new Date().toISOString(),
...
logSection("Config Snapshots", Object.values(configSnapshot.configs).filter(c => c.exists).length >= 3 ? "pass" : "fail",
...
logSection("Runbook Coverage", runbooks.length >= 10 ? "pass" : "fail",
  `${runbooks.length} runbooks`);

writeFileSync(join(outDir, "runbook-index.json"), JSON.stringify({
  collectedAt: new Date().toISOString(),
  runbooks,
...
if (!skipGates) {
  ...
  const gauntletOutput = safeExec("node qa/gauntlet/cli.mjs --suite fast 2>&1", { timeout: 180_000 });
  ...
  if (gauntletJson) {
    ...
    writeFileSync(join(outDir, "gauntlet-results.json"), JSON.stringify({
...
logSection("Architecture Docs", archCount >= 3 ? "pass" : "fail",
  `${archCount}/${archDocs.length} present`);

writeFileSync(join(outDir, "arch-docs.json"), JSON.stringify({
  collectedAt: new Date().toISOString(),
...
logSection("PHI Scan of Bundle", phiClean ? "pass" : "fail",
  phiClean ? "zero PHI detected" : "PHI found in output!");

writeFileSync(join(outDir, "phi-scan.json"), JSON.stringify({
  collectedAt: new Date().toISOString(),
  allClean: phiClean,
...
| File | SHA-256 | Size |
|------|---------|------|
${Object.entries(manifest.artifacts).map(([f, m]) => `| ${f} | ${m.sha256.slice(0, 16)}... | ${m.sizeBytes} bytes |`).join("\n")}
...
if (doZip) {
  ...
  try {
    ...
    if (process.platform === "win32") {
      safeExec(`powershell -Command "Compress-Archive -Path '${outDir}\\*' -DestinationPath '${zipPath}' -Force"`, { timeout: 60_000 });
    } else {
