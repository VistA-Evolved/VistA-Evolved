...
import { readdirSync, renameSync, readFileSync, existsSync, mkdirSync, copyFileSync, unlinkSync, statSync, rmdirSync, writeFileSync } from "node:fs";
...
function main() {
  ...
  const shadowFolders = readdirSync(PROMPTS)
    ...
    .filter((e) => {
      try { return statSync(join(PROMPTS, e)).isDirectory(); }
      catch { return false; }
    })
  ...
  console.log(`Found ${shadowFolders.length} shadow folders: ${shadowFolders[0]}..${shadowFolders[shadowFolders.length - 1]}\n`);
