...
writeFileSync(
  join(outDir, 'phi-leak-scan.json'),
  JSON.stringify(
    {
      pass: phiGate.pass,
...
const licenseGate = runGate('license report', 'pnpm licenses list --json 2>nul || echo {}', {
  allowFail: true,
});

writeFileSync(
  join(outDir, 'license-report.json'),
  JSON.stringify(
    {
...
try {
  ...
  gates.push({
    ...
    pass: true,
    output: 'skipped -- tool not installed',
    durationMs: 0,
  });
  writeFileSync(
    join(outDir, 'sbom.json'),
    JSON.stringify({
      note: 'SBOM generation skipped -- install @cyclonedx/cyclonedx-npm to enable',
    })
  );
}
...
/* ------------------------------------------------------------------ */
console.log('\n--- Vulnerability Scan ---');
const vulnGate = runGate('dependency vuln scan', 'pnpm audit --json 2>nul || echo {}', {
  allowFail: true,
});

writeFileSync(
  join(outDir, 'vuln-scan.json'),
  JSON.stringify(
    {
