...
if (existsSync(RPC_REGISTRY)) {
  ...
  const excMatch = regSrc.match(/export\s+const\s+RPC_EXCEPTIONS[\s\S]*?=\s*[\[{]([\s\S]*?)[\]}]\s*(?:as\s+const\s*)?;/);
  if (excMatch) {
    const entries = excMatch[1].match(/name:\s*["'][^"']+["']/g) || excMatch[1].match(/"[^"]+"\s*:/g);
...
if (existsSync(ROUTINES_DIR)) {
  customRoutines = readdirSync(ROUTINES_DIR).filter((f) => f.endsWith(".m")).sort();
}
