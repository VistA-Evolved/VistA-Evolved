...
// -------------------------------------------------------------------
const adminRequired = [
  "/secrets/", "/tenant-security/", "/siem/", "/posture/", "/hardening/",
  "/facilities/", "/departments/", "/events/", "/webhooks/", "/plugins/",
  "/edge-gateways/", "/devices/", "/admin/", "/audit/",
];
let missingAdmin = [];
...
check(
  ...
  missingAdmin.length > 0 ? `missing: ${missingAdmin.join(", ")}` : `${adminRequired.length} verified`
);
...
// -------------------------------------------------------------------
const allPerms = [
  "clinical:read", "clinical:write", "rcm:read", "rcm:write", "rcm:admin",
  "imaging:read", "imaging:write", "imaging:admin", "analytics:read",
  "analytics:admin", "admin:system", "audit:read",
];
...
check(
  ...
  missingAdminPerms.length > 0 ? `missing: ${missingAdminPerms.join(", ")}` : `${allPerms.length} verified`
);
...
for (const fn of rbacGuards) {
  const found =
    rbacSrc.includes(`export function ${fn}`) ||
    rbacSrc.includes(`export async function ${fn}`);
