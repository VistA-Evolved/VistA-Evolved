...
for (const cat of categories) {
  ...
  check(`Runbook category: ${cat.name}`, matches.length > 0, `${matches.length} runbook(s)`);
}
...
check(
  'AGENTS.md has credentials section',
  agentsSrc.includes('Credentials') || agentsSrc.includes('credentials')
);
check(
  'AGENTS.md has architecture map',
  agentsSrc.includes('Architecture') || agentsSrc.includes('architecture')
);
check(
  'AGENTS.md has bug tracker reference',
  agentsSrc.includes('BUG-TRACKER') || agentsSrc.includes('Bug Tracker')
);
...
// -------------------------------------------------------------------
const verifyScripts = ['scripts/verify-rc.ps1', 'scripts/verify-latest.ps1'];
