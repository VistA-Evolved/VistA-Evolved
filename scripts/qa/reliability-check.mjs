...
check(
  'Circuit breaker force-open (outage sim)',
...
// 3. Store policy
check(
  'Store policy registry exists',
  existsSync(resolve(root, 'apps/api/src/platform/store-policy.ts')),
  'store-policy.ts'
);

check(
  'Store policy tracks in-memory stores',
...
check(
  '/ready readiness probe (CB-gated)',
...
writeFileSync(
  resolve(evDir, 'reliability-report.json'),
