...
export async function runRestoreDrillEvidence(
  ...
  if (fs.existsSync(restorePsPath)) {
    ...
    try {
      execSync(`powershell -ExecutionPolicy Bypass -File "${restorePsPath}" ${args.join(' ')}`, {
        cwd: ROOT,
        encoding: 'utf-8',
        timeout: 120_000,
        stdio: ['pipe', 'pipe', 'pipe'],
...
if (process.argv[1]?.endsWith('restore-drill-evidence.ts')) {
  ...
  if (!manifestPath) {
    console.error(
      'Usage: npx tsx scripts/ops/restore-drill-evidence.ts --manifest <path> [--skip-docker]'
    );
    process.exit(1);
  }
