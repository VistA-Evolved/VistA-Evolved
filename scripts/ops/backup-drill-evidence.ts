...
export async function runBackupDrillEvidence(skipDocker = false): Promise<BackupEvidenceReport> {
  ...
  // Evaluate pass/fail
  const backupOk = report.drill.backupStatus === "success" || report.drill.backupStatus === "partial";
  const restoreOk = report.drill.restoreStatus === "success" || report.drill.restoreStatus === "skipped-no-backup";
...
// CLI entry point
if (import.meta.url === `file:///${process.argv[1]?.replace(/\\/g, "/")}` || process.argv[1]?.endsWith("backup-drill-evidence.ts")) {
