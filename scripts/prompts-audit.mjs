...
function scanPrompts() {
  ...
  for (const name of dirs) {
    ...
    const entry = { name, prefix, numericPrefix: parseInt(prefix, 10), isMeta, hasImplement, hasVerify };
    folders.push(entry);
  ...
  const numericPrefixes = [...new Set(
    folders
      .filter(f => f.prefix && !f.isMeta)
      .map(f => parseInt(f.prefix, 10))
  )].sort((a, b) => a - b);
  ...
  return {
    ...
    prefixRange: numericPrefixes.length > 0
