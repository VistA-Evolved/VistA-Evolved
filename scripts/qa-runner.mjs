...
const SUITES = {
  smoke: [
    ...
    { name: "API integration tests", cmd: "cd apps/api && pnpm exec vitest run tests/qa-api-routes.test.ts" },
    { name: "E2E smoke", cmd: "cd apps/web && pnpm exec playwright test e2e/qa-smoke.spec.ts --reporter=list" },
  ],
  api: [
    { name: "API health", cmd: "node scripts/qa-gates/api-health.mjs" },
    { name: "API integration tests", cmd: "cd apps/api && pnpm exec vitest run tests/qa-api-routes.test.ts" },
    { name: "API security tests", cmd: "cd apps/api && pnpm exec vitest run tests/qa-security.test.ts" },
    { name: "API contract tests", cmd: "cd apps/api && pnpm exec vitest run tests/contract.test.ts" },
  ],
  web: [
  ...
  security: [
    ...
    { name: "Security tests", cmd: "cd apps/api && pnpm exec vitest run tests/qa-security.test.ts" },
