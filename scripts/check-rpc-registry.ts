...
if (existsSync(VIVIAN_INDEX)) {
  ...
  vivianNames = new Set((vivianData.rpcs || []).map((r: { name: string }) => r.name.toUpperCase()));
...
pass(
  'registry-parsed',
  `Registry: ${registryNames.length} RPCs, ${exceptionNames.length} exceptions`
);
...
if (notInVivianOrException.length === 0) {
  ...
  fail(
    'registry-in-vivian',
    `${notInVivianOrException.length} registry entries not in Vivian or exceptions`,
    notInVivianOrException
  );
}
...
if (unknownCalls.length === 0) {
  pass(
    'no-unknown-rpcs',
    `All ${calledRpcs.size} unique RPC calls reference known registry entries`
  );
} else {
  fail(
    'no-unknown-rpcs',
...
if (orphaned.length === 0) {
  ...
  warn(
    'no-orphans',
    `${orphaned.length} registry entries not directly called in source (may be used via lookup)`,
    orphaned
  );
}
