...
async function main() {
  ...
  console.log(
    "This tool records live RPC responses and saves sanitized fixtures."
  );
  console.log(
    "Requires: VistA Docker + .env.local credentials + VISTA_CONTRACT_MODE=record\n"
  );

  const mode = process.env.VISTA_CONTRACT_MODE;
  if (mode !== "record") {
    console.error(
      "ERROR: Set VISTA_CONTRACT_MODE=record to use this tool."
    );
    console.error(
      "  VISTA_CONTRACT_MODE=record npx tsx scripts/vista-contracts-record.ts"
    );
  ...
  try {
    const broker = await import(
      "../apps/api/src/vista/rpcBrokerClient.js"
    );
    ...
    console.error(
      "ERROR: Cannot import rpcBrokerClient. Run from repo root."
    );
    process.exit(1);
  }

  let sanitizeRpcOutput: (
    rpcName: string,
    lines: string[]
  ) => string[];
  try {
    const contracts = await import(
      "../apps/api/src/vista/contracts/index.js"
    );
  ...
  for (const plan of RECORD_PLANS) {
    ...
    try {
      ...
      console.log(
        `  -> ${sanitized.length} lines saved to ${safeName}/success.json`
      );
      recorded++;
    } catch (err) {
      console.error(
        `  FAILED: ${plan.rpcName} — ${err instanceof Error ? err.message : err}`
      );
      failed++;
    }
  }
  ...
  console.log(
    "Review fixtures in apps/api/tests/fixtures/vista/ before committing."
  );
}
