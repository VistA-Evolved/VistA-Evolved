...
addSection(2, 'TypeScript Compilation', {
  pass: tscPass,
  apps: Object.fromEntries(Object.entries(tscResults).map(([k, v]) => [k, v.pass])),
});
...
if (existsSync(rpcScript)) {
  ...
  if (rpcReport) {
    writeFileSync(join(outDir, 'rpc-contract-report.json'), JSON.stringify(rpcReport, null, 2));
  }
}
...
if (existsSync(invScript)) {
  ...
  if (invReport) {
    writeFileSync(join(outDir, 'invariants-report.json'), JSON.stringify(invReport, null, 2));
  }
}
...
if (existsSync(secScript)) {
  ...
  const secReport = safeReadJson(join(ROOT, 'artifacts', 'security', 'security-gauntlet.json'));
  secResult = {
    pass: res.ok,
  ...
  if (secReport) {
    writeFileSync(join(outDir, 'security-gauntlet.json'), JSON.stringify(secReport, null, 2));
  }
}
...
if (existsSync(phiScript)) {
  ...
  const phiReport = safeReadJson(join(ROOT, 'artifacts', 'privacy', 'phi-audit-report.json'));
  phiResult = {
    pass: res.ok,
  ...
  if (phiReport) {
    writeFileSync(join(outDir, 'phi-audit-report.json'), JSON.stringify(phiReport, null, 2));
  }
}
...
if (existsSync(drillScript) && !skipLive) {
  ...
  const gameDayReport = safeReadJson(join(ROOT, 'artifacts', 'dr', 'gameday-results.json'));
  ...
  if (gameDayReport) {
    writeFileSync(join(outDir, 'gameday-results.json'), JSON.stringify(gameDayReport, null, 2));
  }
} else if (skipLive) {
...
writeFileSync(join(outDir, 'audit-chain.json'), JSON.stringify(auditChainResult, null, 2));
...
writeFileSync(join(outDir, 'safety-case-xref.json'), JSON.stringify(safetyCaseResult, null, 2));
...
writeFileSync(join(outDir, 'bundle-index.json'), JSON.stringify(bundleIndex, null, 2));
...
summaryLines.push('', '---', '', '*Generated by generate-certification-evidence-v2.mjs*');
