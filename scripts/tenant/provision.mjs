...
async function main() {
  ...
  try {
    ...
    console.error(
      `  WARN: Tenant creation via API failed (${err.message}). Continuing with module seeding...`
    );
  }
  ...
  try {
    ...
    console.log(
      `  Seeded ${seedResult.modulesSeeded} modules (${seedResult.totalEnabled} total enabled).`
    );
  } catch (err) {
    console.error(
      `  WARN: Module seeding via API failed (${err.message}). Seeding individually...`
    );
