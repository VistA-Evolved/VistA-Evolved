...
function safeExec(cmd, cwd, timeoutMs = 120_000) {
  try {
    ...
    return { ok: false, output: (err.stdout || "") + "\n" + (err.stderr || ""), exitCode: err.status ?? 1 };
  }
}
...
for (const dir of fixtureDirs) {
  ...
  try {
    ...
    const structureFingerprint = {
      ...
      avgLineLength: response.length > 0
        ? Math.round(response.reduce((s, l) => s + String(l).length, 0) / response.length)
        : 0,
      fieldCountPerLine: response.length > 0
        ? response.map((l) => String(l).split("^").length)
        : [],
    };
...
const report = {
  ...
  overallPass: vitestResult.ok && driftResults.every((d) => !d.drift) && parserChecks.every((c) => c.pass),
};

writeFileSync(join(outputDir, "invariants-report.json"), JSON.stringify(report, null, 2));
writeFileSync(join(outputDir, "drift-report.json"), JSON.stringify({
