...
function vivianStatus(
  rpc: string,
  vivian: Set<string>,
  exceptions: Set<string>
): Wave1Action['vivianPresence'] {
...
function main() {
  ...
  const plan = {
    _meta: {
      ...
      description:
        'Wave 1 (READ) wiring plan -- authoritative source for screens, actions, endpoints, RPCs',
  ...
  console.log(
    `Wave 56 plan: ${screens.length} screens, ${allActions.length} actions (${wired} wired, ${pending} pending)`
  );
