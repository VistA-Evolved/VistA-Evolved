...
gate("FHIR bridge exports convertHl7ToFhir", () => {
  ...
  return src.includes("export function convertHl7ToFhir") &&
...
gate("Channel health exports getChannelHealthSummary", () => {
  ...
  return src.includes("export function getChannelHealthSummary") &&
...
gate("Outbound builder exports all message builders", () => {
  ...
  return src.includes("export function buildAdtMessage") &&
...
gate("Core HL7 engine files exist", () => {
  const requiredFiles = [
    "index.ts", "types.ts", "parser.ts", "ack-generator.ts",
    "mllp-server.ts", "mllp-client.ts", "domain-mapper.ts",
    "message-event-store.ts", "dead-letter-enhanced.ts",
    "tenant-endpoints.ts",
  ];
...
gate("Routing subsystem files exist", () => {
  ...
  return existsSync(join(routingDir, "registry.ts")) &&
...
gate("Message pack files exist", () => {
  ...
  return existsSync(join(packsDir, "adt-pack.ts")) &&
...
gate("FHIR bridge defines FhirBundle type", () => {
  const src = readFileSync(join(HL7_DIR, "fhir-bridge.ts"), "utf-8");
  return src.includes("export interface FhirBundle") &&
