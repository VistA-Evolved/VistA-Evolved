...
function checkKeyCoverage(enKeys, localeData, locale, appName) {
  ...
  if (missing.length === 0) {
    ...
    warn("key-coverage", `${appName}/${locale}: ${coverage}% (missing ${missing.length}: ${missing.slice(0, 5).join(", ")})`);
  } else {
...
for (const pack of packs) {
  const missingWeb = pack.locales.filter(
    (l) => !existsSync(join(WEB_MESSAGES_DIR, `${l}.json`))
  );
  ...
  if (missingWeb.length === 0 && missingPortal.length === 0) {
    pass("pack-coverage", `${pack.countryCode}: all ${pack.locales.length} locales have message files`);
  } else {
