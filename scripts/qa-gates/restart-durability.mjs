...
gate(
  'CREATE TABLE auth_session in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS auth_session')
);
gate(
  'CREATE TABLE rcm_work_item in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS rcm_work_item')
);
gate(
  'CREATE TABLE rcm_work_item_event in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS rcm_work_item_event')
);

console.log('\nMigration DDL (Phase 115):');
gate(
  'CREATE TABLE portal_message in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS portal_message')
);
gate(
  'CREATE TABLE portal_appointment in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS portal_appointment')
);
gate(
  'CREATE TABLE telehealth_room in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS telehealth_room')
);
gate(
  'CREATE TABLE imaging_work_order in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS imaging_work_order')
);
gate(
  'CREATE TABLE imaging_study_link in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS imaging_study_link')
);
gate(
  'CREATE TABLE imaging_unmatched in migrate',
  migrate.includes('CREATE TABLE IF NOT EXISTS imaging_unmatched')
);
gate(
  'CREATE TABLE idempotency_key in migrate',
...
gate(
  'session-repo exports createAuthSession',
  sessionRepo?.includes('export function createAuthSession') ?? false
);
gate(
  'session-repo exports findSessionByTokenHash',
  sessionRepo?.includes('export function findSessionByTokenHash') ?? false
);

const wqRepo = readSrc('apps/api/src/platform/db/repo/workqueue-repo.ts');
gate('workqueue-repo.ts exists', wqRepo !== null);
gate(
  'workqueue-repo exports createWorkItem',
  wqRepo?.includes('export function createWorkItem') ?? false
);
gate(
  'workqueue-repo exports listWorkItems',
  wqRepo?.includes('export function listWorkItems') ?? false
);
...
gate(
  'portal-message-repo exports insertMessage',
  pmRepo?.includes('export function insertMessage') ?? false
);
...
gate(
  'portal-appointment-repo exports insertAppointment',
  paRepo?.includes('export function insertAppointment') ?? false
);
...
gate(
  'telehealth-room-repo exports insertRoom',
  trRepo?.includes('export function insertRoom') ?? false
);
...
gate(
  'imaging-worklist-repo exports insertWorkOrder',
  iwRepo?.includes('export function insertWorkOrder') ?? false
);

const iiRepo = readSrc('apps/api/src/platform/db/repo/imaging-ingest-repo.ts');
gate('imaging-ingest-repo.ts exists', iiRepo !== null);
gate(
  'imaging-ingest-repo exports insertStudyLink',
  iiRepo?.includes('export function insertStudyLink') ?? false
);
gate(
  'imaging-ingest-repo exports insertUnmatched',
  iiRepo?.includes('export function insertUnmatched') ?? false
);
...
gate(
  'session-store does NOT use raw Map as primary',
...
gate(
  'workqueue-store has initWorkqueueRepo',
  wqStore.includes('export function initWorkqueueRepo')
);
gate(
  'workqueue-store does NOT use raw Map as primary',
...
gate(
  'capability-matrix has initCapabilityAudit',
  capMatrix.includes('export function initCapabilityAudit')
);
...
gate(
  'portal-messaging has initMessageRepo',
  portalMsg.includes('export async function initMessageRepo') ||
    portalMsg.includes('export function initMessageRepo')
);
...
gate(
  'portal-appointments has initAppointmentRepo',
  portalAppt.includes('export function initAppointmentRepo')
);
gate('portal-appointments uses appointmentCache', portalAppt.includes('appointmentCache'));
gate(
  'portal-appointments no raw appointmentStore',
  !portalAppt.includes('appointmentStore = new Map')
);

const roomStore = readSrc('apps/api/src/telehealth/room-store.ts') ?? '';
gate(
  'room-store has initTelehealthRoomRepo',
  roomStore.includes('export function initTelehealthRoomRepo')
);
...
gate(
  'rcm_claim CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS rcm_claim')
);
gate(
  'rcm_remittance CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS rcm_remittance')
);
gate(
  'rcm_claim_case CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS rcm_claim_case')
);
gate(
  'edi_acknowledgement CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS edi_acknowledgement')
);
gate(
  'edi_claim_status CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS edi_claim_status')
);
gate(
  'edi_pipeline_entry CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS edi_pipeline_entry')
);
...
gate(
  'pg rcm-claim-repo exports insertClaim',
  pgRcmClaimRepo?.includes('export async function insertClaim') ?? false
);
gate(
  'pg rcm-claim-repo exports findClaimById',
  pgRcmClaimRepo?.includes('export async function findClaimById') ?? false
);
...
gate(
  'pg rcm-claim-case-repo exports insertClaimCase',
  pgRcmCaseRepo?.includes('export async function insertClaimCase') ?? false
);

const pgEdiAckRepo = readSrc('apps/api/src/platform/pg/repo/edi-ack-repo.ts');
gate('pg edi-ack-repo.ts exists', pgEdiAckRepo !== null);
gate(
  'pg edi-ack-repo exports insertAck',
  pgEdiAckRepo?.includes('export async function insertAck') ?? false
);
gate(
  'pg edi-ack-repo exports insertStatusUpdate',
  pgEdiAckRepo?.includes('export async function insertStatusUpdate') ?? false
);
gate(
  'pg edi-ack-repo exports findAckByIdempotencyKey',
  pgEdiAckRepo?.includes('export async function findAckByIdempotencyKey') ?? false
);
...
gate(
  'pg edi-pipeline-repo exports insertPipelineEntry',
  pgEdiPipeRepo?.includes('export async function insertPipelineEntry') ?? false
);
gate(
  'pg edi-pipeline-repo exports updateEntry',
  pgEdiPipeRepo?.includes('export async function updateEntry') ?? false
);

console.log('\nPhase 126 store delegation:');
const ackProc = readSrc('apps/api/src/rcm/edi/ack-status-processor.ts') ?? '';
gate(
  'ack-status-processor has initAckStatusRepo',
  ackProc.includes('export function initAckStatusRepo')
);
...
gate(
  'index.ts imports pg rcm-claim-repo (PG)',
  startup.includes('platform/pg/repo/rcm-claim-repo')
);
gate(
  'index.ts imports pg rcm-claim-case-repo (PG)',
  startup.includes('platform/pg/repo/rcm-claim-case-repo')
);
...
gate(
  'pgPortalPatientSetting table in pg-schema',
  pgSchema.includes('pgTable("portal_patient_setting"')
);
gate('pgTelehealthRoom table in pg-schema', pgSchema.includes('pgTable("telehealth_room"'));
gate(
  'pgTelehealthRoomEvent table in pg-schema',
  pgSchema.includes('pgTable("telehealth_room_event"')
);

console.log('\nPhase 127 PG migration DDL:');
gate(
  'portal_message CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS portal_message')
);
gate(
  'portal_access_log CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS portal_access_log')
);
gate(
  'portal_patient_setting CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS portal_patient_setting')
);
gate(
  'telehealth_room CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS telehealth_room')
);
gate(
  'telehealth_room_event CREATE TABLE in pg-migrate',
  pgMigrate.includes('CREATE TABLE IF NOT EXISTS telehealth_room_event')
);
...
gate(
  'pg portal-message-repo exports insertMessage',
  pgPortalMsgRepo?.includes('export async function insertMessage') ?? false
);
gate(
  'pg portal-message-repo exports findMessageById',
  pgPortalMsgRepo?.includes('export async function findMessageById') ?? false
);
...
gate(
  'pg portal-access-log-repo exports insertAccessLog',
  pgPortalAlogRepo?.includes('export async function insertAccessLog') ?? false
);

const pgPortalSettingRepo = readSrc(
  'apps/api/src/platform/pg/repo/pg-portal-patient-setting-repo.ts'
);
gate('pg portal-patient-setting-repo.ts exists', pgPortalSettingRepo !== null);
gate(
  'pg portal-patient-setting-repo exports upsertSetting',
  pgPortalSettingRepo?.includes('export async function upsertSetting') ?? false
);
...
gate(
  'pg telehealth-room-repo exports insertRoom',
  pgThRoomRepo?.includes('export async function insertRoom') ?? false
);
...
gate(
  'pg telehealth-room-event-repo exports insertRoomEvent',
  pgThEventRepo?.includes('export async function insertRoomEvent') ?? false
);

console.log('\nPhase 127 store interfaces loosened:');
gate(
  'portal-messaging uses interface MsgRepo (not typeof import)',
  portalMsg.includes('interface MsgRepo')
);
gate(
  'room-store uses interface RoomRepo (not typeof import)',
  roomStore.includes('interface RoomRepo')
);
gate(
  'access-log-store AccessLogRepo returns any',
  readSrc('apps/api/src/portal-iam/access-log-store.ts')?.includes('countAllAccessLogs(): any') ??
    false
);
...
gate(
  'portal-settings has initSettingsRepo',
  settingsStore.includes('export function initSettingsRepo')
);
gate(
  'portal-settings has SettingsRepo interface',
  settingsStore.includes('export interface SettingsRepo')
);
gate(
  'portal-settings has write-through',
  settingsStore.includes('_settingsRepo.upsertSetting') || settingsStore.includes('_settingsRepo')
);
...
gate(
  'index.ts wires PG portal patient setting repo',
  startup.includes('pg-portal-patient-setting-repo')
);
...
gate(
  'PG barrel exports pgPortalPatientSettingRepo',
  pgBarrel.includes('pgPortalPatientSettingRepo')
);
...
gate(
  'index.ts wires pg-scheduling-request-repo (PG)',
  startup.includes('pg-scheduling-request-repo')
);
