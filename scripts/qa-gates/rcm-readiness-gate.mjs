...
gate('G4 Connectors scanned', (matrix.connectors || []).length > 0, 'No connectors found');

/* Gate 5: At least 1 country pack scanned */
gate('G5 Country packs scanned', (matrix.countryPacks || []).length > 0, 'No country packs found');
...
gate('G8 Payer records', (matrix.payers || []).length > 0, 'No payer records found');

/* Gate 9: Connectors summary math coherent */
const s = matrix.summary?.connectors || {};
const sumParts =
  (s.implemented || 0) + (s.stub || 0) + (s.integrationPending || 0) + (s.needsCredentials || 0);
gate(
  'G9 Summary math',
  sumParts <= (s.total || 0),
