...
const totalRegistered = criticalCount + cacheCount + rateLimiterCount + registryCount + auditCount + devOnlyCount;
...
if (cacheStoresWithoutLimits.length > 0) {
  if (strict) {
    fail(`${cacheStoresWithoutLimits.length} cache stores lack TTL or maxSize: ${cacheStoresWithoutLimits.join(", ")}`);
  } else {
    warn(`${cacheStoresWithoutLimits.length} cache stores lack TTL or maxSize: ${cacheStoresWithoutLimits.join(", ")}`);
  }
} else {
...
function walkDir(dir, ext = ".ts") {
  ...
  try {
    ...
    for (const e of entries) {
      ...
      } else if (e.name.endsWith(ext) && !e.name.endsWith(".test.ts") && !e.name.endsWith(".spec.ts")) {
        results.push(full);
      }
    }
...
for (const filePath of sourceFiles) {
  ...
  if (moduleLevelMaps > 0) {
    ...
    const isRegistered = filePatterns.some((fp) => relPath.includes(fp) || fp.includes(relPath.replace(".ts", "")));
...
if (missingExports.length > 0) {
  ...
  ok(`store-policy.ts: ${requiredExports.length}/${requiredExports.length} required exports present`);
}
...
if (criticalNoMigration.length > 0) {
  fail(`${criticalNoMigration.length} critical+in_memory_only stores lack migrationTarget: ${criticalNoMigration.join(", ")}`);
} else {
