...
gate('1. fhir-bundle-parser.ts exists', () => existsSync(join(MIG, 'fhir-bundle-parser.ts')));

gate('2. ccda-parser.ts exists', () => existsSync(join(MIG, 'ccda-parser.ts')));

gate('3. reconciliation.ts exists', () => existsSync(join(MIG, 'reconciliation.ts')));
...
gate(
  ...
  () =>
    fhirSrc.includes('entry') && fhirSrc.includes('resource') && fhirSrc.includes('resourceType')
);
...
gate(
  ...
  () => ccdaSrc.includes('extractPatientDemographics') && ccdaSrc.includes('recordTarget')
);

gate(
  '12. Extracts problems/conditions section',
  () => ccdaSrc.includes('extractProblemsSection') && ccdaSrc.includes('11450-4')
);

gate(
  '13. Returns CcdaImportResult with integration-pending markers',
  () => ccdaSrc.includes('CcdaImportResult') && ccdaSrc.includes('integration-pending')
);
...
gate(
  '20. Enforces dependency order (patients before dependents)',
  () =>
