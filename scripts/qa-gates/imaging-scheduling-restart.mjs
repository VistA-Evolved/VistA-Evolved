...
gate("pg-imaging-worklist-repo exports insertWorkOrder", pgIwRepo?.includes("export async function insertWorkOrder") ?? false);
gate("pg-imaging-worklist-repo exports findWorkOrderById", pgIwRepo?.includes("export async function findWorkOrderById") ?? false);
gate("pg-imaging-worklist-repo exports findByAccessionNumber", pgIwRepo?.includes("export async function findByAccessionNumber") ?? false);
gate("pg-imaging-worklist-repo exports findByPatientDfn", pgIwRepo?.includes("export async function findByPatientDfn") ?? false);
gate("pg-imaging-worklist-repo exports updateWorkOrder", pgIwRepo?.includes("export async function updateWorkOrder") ?? false);
...
gate("pg-imaging-ingest-repo exports insertStudyLink", pgIiRepo?.includes("export async function insertStudyLink") ?? false);
gate("pg-imaging-ingest-repo exports insertUnmatched", pgIiRepo?.includes("export async function insertUnmatched") ?? false);
gate("pg-imaging-ingest-repo exports findLinkagesByPatient", pgIiRepo?.includes("export async function findLinkagesByPatient") ?? false);
gate("pg-imaging-ingest-repo exports findLinkageByStudyUid", pgIiRepo?.includes("export async function findLinkageByStudyUid") ?? false);
gate("pg-imaging-ingest-repo exports markResolved", pgIiRepo?.includes("export async function markResolved") ?? false);
...
gate("pg-scheduling-request-repo exports insertSchedulingRequest", pgSrRepo?.includes("export async function insertSchedulingRequest") ?? false);
gate("pg-scheduling-request-repo exports findSchedulingRequestById", pgSrRepo?.includes("export async function findSchedulingRequestById") ?? false);
gate("pg-scheduling-request-repo exports findAllActiveRequests", pgSrRepo?.includes("export async function findAllActiveRequests") ?? false);
gate("pg-scheduling-request-repo exports updateSchedulingRequest", pgSrRepo?.includes("export async function updateSchedulingRequest") ?? false);
...
gate("pg-scheduling-lock-repo exports acquireLock", pgSlRepo?.includes("export async function acquireLock") ?? false);
gate("pg-scheduling-lock-repo exports releaseLock", pgSlRepo?.includes("export async function releaseLock") ?? false);
gate("pg-scheduling-lock-repo exports cleanupExpiredLocks", pgSlRepo?.includes("export async function cleanupExpiredLocks") ?? false);
...
gate("imaging-worklist getWorklistItem is async", imgWl.includes("async function getWorklistItem") || imgWl.includes("export async function getWorklistItem"));
gate("imaging-worklist findByAccession is async", imgWl.includes("async function findByAccession") || imgWl.includes("export async function findByAccession"));
gate("imaging-worklist findByPatientDfn is async", imgWl.includes("async function findByPatientDfn") || imgWl.includes("export async function findByPatientDfn"));
gate("imaging-worklist getAllWorklistItems is async", imgWl.includes("export async function getAllWorklistItems"));
gate("imaging-worklist uses Promise.resolve for reads", imgWl.includes("await Promise.resolve(_repo"));
...
gate("imaging-ingest getLinkagesForPatient is async", imgIng.includes("export async function getLinkagesForPatient"));
gate("imaging-ingest getAllUnmatched is async", imgIng.includes("export async function getAllUnmatched"));
...
gate("vista-adapter initSchedulingRepo is async", schedAdapter.includes("export async function initSchedulingRepo"));
gate("vista-adapter has SchedulingLockRepo interface", schedAdapter.includes("export interface SchedulingLockRepo"));
gate("vista-adapter has initSchedulingLockRepo export", schedAdapter.includes("export function initSchedulingLockRepo"));
gate("vista-adapter persistEntry uses Promise.resolve", schedAdapter.includes("Promise.resolve") && schedAdapter.includes("persistEntry"));
gate("vista-adapter acquireBookingLock writes to PG", schedAdapter.includes("lockRepo.acquireLock"));
gate("vista-adapter releaseBookingLock writes to PG", schedAdapter.includes("lockRepo.releaseLock"));
gate("vista-adapter cleanup calls PG cleanupExpiredLocks", schedAdapter.includes("lockRepo.cleanupExpiredLocks"));
gate("vista-adapter getRequestStore is async", schedAdapter.includes("export async function getRequestStore"));
...
// ── 8. Lock TTL ─────────────────────────────────────────────
console.log("\nLock TTL semantics:");
gate("pg-scheduling-lock-repo deletes expired before acquire",
  (pgSlRepo?.includes("delete") || pgSlRepo?.includes("DELETE")) && pgSlRepo?.includes("expires_at") ? true : false);
gate("pg-scheduling-lock-repo unique constraint on lock_key",
