...
if (devContent) {
  ...
  check("dev-resource-limits", devContent.includes("limits") && devContent.includes("memory"), "Dev sandbox has resource limits");
  check("dev-named-volume", devContent.includes("volumes:") && devContent.includes("vista-globals"), "Dev sandbox has named volume for persistence");
}
...
if (distroContent) {
  ...
  check("distro-resource-limits", distroContent.includes("limits") && distroContent.includes("memory"), "Distro lane has resource limits");
  check("distro-no-baked-creds", !distroContent.includes("PROV123"), "No baked credentials in distro compose");
}
...
writeFileSync(
  join(artifactDir, "vista-container-gate.json"),
