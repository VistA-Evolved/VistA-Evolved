...
for (const pat of PHI_LOG_PATTERNS) {
  for (const srcPath of srcPaths) {
    ...
    const result = safeExec(
      `findstr /S /R /N "${pat.pattern}" *.ts 2>nul`,
      full,
      30_000
    );
    if (result.output.trim()) {
      ...
      for (const line of lines) {
        grepFindings.push({ check: "phi-in-logs", pattern: pat.description, file: srcPath, line: line.trim() });
        totalViolations++;
      }
    }
  }
...
const PHI_DATA_PATTERNS = [
  ...
  { regex: /\b(19[3-9]\d|20[0-2]\d)-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])\b/, name: "Date of birth (ISO)" },
];
    ...
    for (const fname of ["success.json", "empty.json"]) {
      ...
      for (const pat of PHI_DATA_PATTERNS) {
        if (pat.regex.test(content)) {
          ...
          if (pat.name === "VistA patient name (non-ANONYMOUS)" && /PROVIDER,ANONYMOUS/.test(content)) continue;
          grepFindings.push({ check: "phi-in-fixtures", pattern: pat.name, file: `${dir.name}/${fname}` });
          fixtureViolations++;
        }
      }
    }
...
const credResult = safeExec('findstr /S /R /N "PROV123\\|PHARM123\\|NURSE123" apps\\api\\src\\*.ts 2>nul', ROOT, 30_000);
...
console.log(`  Audit emitters checked: ${auditEmitters.length}, All sanitized: ${sanitizerFindings.every((s) => s.hasSanitizer)}`);
...
const phiGrepText = grepFindings.length === 0
  ? "PHI GREP: CLEAN — No violations found\n"
  : grepFindings.map((f) => `[${f.check}] ${f.pattern || ""} — ${f.file || f.emitter || ""} ${f.line || ""}`).join("\n");
...
writeFileSync(join(outputDir, "access-control-proof.json"), JSON.stringify(accessControlEvidence, null, 2));
