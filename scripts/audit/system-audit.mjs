...
function nodeVersion() {
  try { return process.version; }
  catch { return "unknown"; }
}

function pnpmVersion() {
  try { return execSync("pnpm --version", { cwd: ROOT, encoding: "utf8" }).trim(); }
  catch { return "unknown"; }
}
...
function buildUiInventory() {
  ...
  for (const app of apps) {
    if (!existsSync(app.dir)) { result.push({ app: app.name, error: "not found" }); continue; }

    const pages = walkFiles(app.dir, ["page.tsx", "page.ts"]);
    const routes = pages.map(p => {
      const route = "/" + rel(p).replace(`apps/${app.name}/src/app/`, "").replace(/\/page\.tsx?$/, "").replace(/\[(\.\.\.)?([\w]+)\]/g, ":$2");
      return route === "/" + "" ? "/" : route;
    });
    ...
    const patterns = [
      ...
      { name: "disabled_no_reason", re: /disabled(?!.*(?:loading|submitting|pending|validation))/i },
    ];
    ...
    const tagRoutes = (route) => {
      ...
      if (/rcm|claim|payer|denial|payment|remittance|loa|philhealth|hmo/i.test(route)) tags.push("RCM");
...
function buildApiInventory() {
  ...
  const routeFiles = [
    ...
    ...walkFiles(join(apiSrc, "telehealth"), [".ts"]).filter(f => /route/i.test(f) || basename(f) === "index.ts"),
  ];
...
function buildRpcUsage() {
  const apiSrc = join(ROOT, "apps/api/src");
  const rpcCallPattern = /(?:callRpc|safeCallRpc|safeCallRpcWithList|cachedRpc|resilientRpc)\s*\(\s*["'`]([^"'`]+)["'`]/g;
  ...
  for (const file of allTsFiles) {
    try {
      ...
      while ((match = re.exec(content)) !== null) {
        ...
        rpcCalls.push({ rpcName, file: rel(file), line: content.substring(0, match.index).split("\n").length });
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      for (const pat of patterns) {
        const hits = grepFile(file, pat.re);
        if (hits.length > 0) {
          results[pat.name].push({ file: rel(file), count: hits.length, samples: hits.slice(0, 2) });
        }
      }
    }
  }
...
function buildGapMatrix(audit) {
  const { rpcUsage, persistenceInventory, apiInventory, uiInventory, externalSystems, knownGaps } = audit;
  ...
  const domains = [
    {
      ...
      status: "wired",
      evidence: [
        ...domainEndpoints("auth").slice(0, 5).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
        ...domainEndpoints("iam").slice(0, 5).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
      ],
      topGaps: [
        { gap: "OIDC is opt-in, not default", severity: "med", evidenceFiles: ["apps/api/src/auth/oidc-provider.ts"] },
        { gap: "Passkey data delegated to Keycloak (not available in sandbox)", severity: "low", evidenceFiles: ["apps/api/src/auth/biometric/passkeys-provider.ts"] },
      ],
      nextActions: ["Enable OIDC by default in production", "Test Keycloak integration with WorldVistA"],
    },
    {
      domain: "CPRS_UI",
      status: "wired",
      ...
      topGaps: [
        { gap: "Multiple admin pages may have placeholder content", severity: "med", evidenceFiles: [] },
      ],
    ...
    },
    {
      domain: "ORDERS_CPOE",
      status: "partial",
      evidence: domainEndpoints("vista").filter(e => /order/i.test(e.path)).slice(0, 5).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
      topGaps: [
        { gap: "Order signing workflow may be incomplete in sandbox", severity: "med", evidenceFiles: ["apps/api/src/routes/cprs/orders-cpoe.ts"] },
      ],
    ...
    },
    {
      ...
      status: "partial",
      evidence: domainEndpoints("interop").slice(0, 5).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
      topGaps: [
        { gap: "Custom M routines must be installed in Docker", severity: "med", evidenceFiles: ["services/vista/ZVEMIOP.m"] },
      ],
    ...
    },
    {
      domain: "REPORTING",
      status: "local_only",
      evidence: domainEndpoints("other").filter(e => /report/i.test(e.path)).slice(0, 5).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
      topGaps: [
        { gap: "Report cache is in-memory", severity: "low", evidenceFiles: ["apps/api/src/routes/reporting.ts"] },
      ],
    ...
    },
    {
      ...
      status: "partial",
      evidence: [
        { file: "apps/api/src/services/ai-shield.ts", type: "service", note: "PHI redaction + guardrails" },
      ],
      topGaps: [
    ...
    },
    {
      ...
      status: "wired",
      evidence: domainEndpoints("admin").slice(0, 10).map(e => ({ route: `${e.method} ${e.path}`, file: e.file })),
      topGaps: [],
    ...
    },
    {
      ...
      topGaps: [
        { gap: "Audit JSONL files are append-only but not externally replicated", severity: "med", evidenceFiles: ["apps/api/src/lib/immutable-audit.ts"] },
      ],
...
function buildHumanSummary(audit, gapMatrix) {
  const lines = [];
  const { meta, rpcUsage, persistenceInventory, apiInventory, uiInventory, ciInventory, knownGaps } = audit;
  ...
  const plannedDomains = gapMatrix.domains.filter(d => d.status === "planned" || d.status === "mock");
  ...
  for (const d of wiredDomains) {
    const ev = d.evidence.slice(0, 2).map(e => e.route || e.file || e.metric || "").join(", ");
  ...
  lines.push("- **By tag:** " + Object.entries(tagCounts).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k}(${v})`).join(", "));
  lines.push("");
  ...
  for (const app of uiInventory) {
    ...
    lines.push(`- **${app.app}:** ${app.pageCount} pages, ${app.deadClickTotal} dead-click markers`);
  }
  lines.push("");
  ...
  for (const wf of ciInventory.workflows || []) {
    lines.push(`- **${wf.file}:** triggers=[${wf.triggers.join(",")}] gates=[${wf.gates.join(",")}]`);
  }
  lines.push("");
  ...
  for (const d of gapMatrix.domains) {
    for (const g of d.topGaps) {
      nextItems.push({ domain: d.domain, gap: g.gap, severity: g.severity, files: g.evidenceFiles });
    }
  }
  nextItems.sort((a, b) => {
...
async function main() {
  ...
  console.log(`  RPCs used: ${rpcUsage.uniqueRpcsUsed} / ${rpcUsage.registeredRpcsCount} registered`);
  console.log(`  In-memory stores: ${persistenceInventory.inMemoryMaps.count} (${persistenceInventory.highRiskMapStores} high-risk)`);
  console.log(`  SQLite tables: ${persistenceInventory.sqliteTables.count}`);
  console.log(`  UI pages: web=${uiInventory.find(u => u.app === "web")?.pageCount || 0} portal=${uiInventory.find(u => u.app === "portal")?.pageCount || 0}`);
