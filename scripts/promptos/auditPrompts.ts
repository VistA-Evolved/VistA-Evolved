...
if (gaps.length === 0) {
  pass(
    'contiguous',
    `Prefix sequence is contiguous (${sortedPrefixes[0]}..${sortedPrefixes[sortedPrefixes.length - 1]})`
  );
} else {
...
for (const folder of phaseFolders) {
  ...
  if (implementFile) {
    ...
    warn(
      'has-implement',
      `Has prompt.md but no ${prefix}-01-IMPLEMENT.md (legacy pattern)`,
      folder
    );
  } else {
    ...
    if (hasAnyImplement) {
      ...
      warn(
        'has-implement',
        `Has ${files.length} .md file(s) but no standard IMPLEMENT naming (legacy)`,
        folder
      );
    } else {
...
for (const folder of phaseFolders) {
  ...
  for (const file of files) {
    ...
    if (folderPhaseMatch) {
      ...
      if (
        !header.includes(`Phase ${phaseNum}`) &&
        !header.includes(`phase ${phaseNum}`) &&
        !header.toLowerCase().includes(`phase ${phaseNum.toLowerCase()}`)
      ) {
...
function emitAndExit() {
  ...
  const txt = results
    ...
    .join('\n');
  writeFileSync(
    join(ARTIFACTS_DIR, 'prompts-audit.txt'),
    `PromptOS Audit -- ${new Date().toISOString()}\n\n${txt}\n\nTotal: ${passCount} pass, ${failCount} fail, ${warnCount} warn\n`
  );
