...
import { readdirSync, statSync, existsSync, readFileSync, writeFileSync, mkdirSync, copyFileSync, renameSync } from "fs";
...
for (const folder of phaseFolders) {
  ...
  const folderPath = join(PROMPTS_DIR, folder);
  const files = readdirSync(folderPath).filter((f) => f.endsWith(".md") && f !== "prompt.md" && f !== "README.md");
