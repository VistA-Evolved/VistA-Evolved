...
export async function run(opts = {}) {
  ...
  const missingExports = requiredExports.filter(
    (e) => !guardSrc.includes(`export function ${e}`) && !guardSrc.includes(`export class ${e}`) && !guardSrc.includes(`export const ${e}`)
  );
  if (missingExports.length > 0) {
    details.push(`tenant-guard.ts: missing exports: ${missingExports.join(", ")}`);
    status = "fail";
  } else {
    details.push(`tenant-guard.ts: ${requiredExports.length}/${requiredExports.length} exports present`);
  }
  ...
  const criticalTables = [
    ...
    "rcm_work_item", "auth_session", "tenant_payer",
  ];
  if (tablesMatch) {
  ...
  if (existsSync(pgMigratePath)) {
    const pgSrc = readFileSync(pgMigratePath, "utf8");
    const hasAutoEnable = pgSrc.includes("NODE_ENV") && pgSrc.includes("production") && pgSrc.includes("PLATFORM_PG_URL");
    if (hasAutoEnable) {
  ...
  if (existsSync(posturePath)) {
    ...
    const hasTenantPosture = postureSrc.includes("tenant-posture") || postureSrc.includes("admin/tenant-posture");
    if (hasTenantPosture) {
  ...
  if (strict) {
    ...
    const repoFiles = existsSync(REPO_DIR)
      ? readdirSync(REPO_DIR).filter((f) => f.endsWith(".ts") && !f.startsWith("index") && !f.startsWith("tenant-"))
      : [];
    ...
    for (const f of repoFiles) {
      ...
      if (src.includes("from") && (src.includes(".select(") || src.includes(".get(") || src.includes(".all("))) {
