...
export async function run(opts = {}) {
  ...
  if (missingExports.length > 0) {
    ...
    details.push(
      `store-policy.ts: ${requiredExports.length}/${requiredExports.length} exports present`
    );
  }
  ...
  if (totalRegistered < 80) {
    details.push(`FAIL: Store inventory has ${totalRegistered} entries (expected 80+)`);
    status = 'fail';
  } else {
    details.push(
  ...
  for (const rawBlock of entryBlocks) {
    ...
    if (
      classMatch &&
      classMatch[1] === 'critical' &&
      durMatch &&
      durMatch[1] === 'in_memory_only' &&
