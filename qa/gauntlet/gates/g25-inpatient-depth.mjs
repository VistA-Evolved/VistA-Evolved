...
export async function run(_ctx) {
  ...
  if (!medRecContent.includes("integration-pending")) issues.push("Med-rec missing integration-pending markers");
  ...
  if (!existsSync(dischargeFile)) {
    ...
    if (!dischargeContent.includes("DG ADT DISCHARGE")) issues.push("Discharge missing DG ADT DISCHARGE target");
  ...
  if (!existsSync(marFile)) {
    ...
    if (!marContent.includes("PSB VALIDATE ORDER")) issues.push("MAR safety missing PSB VALIDATE ORDER target");
  ...
  const allContent = medRecContent + readFileSync(dischargeFile, "utf-8") + readFileSync(marFile, "utf-8");
