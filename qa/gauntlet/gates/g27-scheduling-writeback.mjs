...
export async function run(_ctx) {
  ...
  if (!routesSrc) {
    ...
    for (const ep of ["/scheduling/writeback/policy", "/scheduling/writeback/entries",
                       "/scheduling/writeback/verify/:ref", "/scheduling/writeback/readiness"]) {
  ...
  for (const rpc of ["SDES GET APPT BY APPT IEN", "SDES CREATE APPOINTMENTS", "SDES CANCEL APPOINTMENT 2"]) {
  ...
  for (const status of ["requested", "pending_approval", "approved", "scheduled", "failed", "integration_pending"]) {
  ...
  if (storePolicy) {
    if (storePolicy.includes("scheduling-writeback-entries")) details.push("Store: scheduling-writeback-entries");
