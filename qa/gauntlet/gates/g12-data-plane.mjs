...
export async function run(opts = {}) {
  ...
  if (missingExports.length > 0) {
    ...
    details.push(
      `runtime-mode.ts: ${requiredExports.length}/${requiredExports.length} exports present`
    );
  }
  ...
  if (!existsSync(oidcProviderPath)) {
    ...
    if (!hasValidate) {
      ...
      details.push(
        'oidc-provider.ts: validateOidcConfig + OIDC_CLIENT_ID check present (Phase 153)'
      );
    }
  }
