...
export async function run(opts = {}) {
  ...
  check(
    'Contract: clinical endpoints',
    contracts?.includes('clinical endpoint contracts') ?? false
  );
  ...
  if (goldenTrace) {
    try {
      ...
      check(
        'Golden trace criticalRpcs array',
        Array.isArray(golden.registrySnapshot?.criticalRpcs)
      );
      ...
      check(
        'Golden trace: no credentials',
        !text.includes('PROV123') && !text.includes('NURSE123')
      );
