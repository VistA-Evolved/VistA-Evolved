...
export async function run(opts = {}) {
  ...
  function check(label, ok) {
    if (ok) { p++; }
    else { f++; details.push(`FAIL: ${label}`); status = "fail"; }
  }
  ...
  check("pgSchedulingWaitlistRequest in schema", pgSchema.includes('pgTable("scheduling_waitlist_request"'));
  check("pgSchedulingBookingLock in schema", pgSchema.includes('pgTable("scheduling_booking_lock"'));
  ...
  check("imaging_work_item DDL", pgMigrate.includes("CREATE TABLE IF NOT EXISTS imaging_work_item"));
  check("imaging_ingest_event DDL", pgMigrate.includes("CREATE TABLE IF NOT EXISTS imaging_ingest_event"));
  check("scheduling_waitlist_request DDL", pgMigrate.includes("CREATE TABLE IF NOT EXISTS scheduling_waitlist_request"));
  check("scheduling_booking_lock DDL", pgMigrate.includes("CREATE TABLE IF NOT EXISTS scheduling_booking_lock"));
  ...
  check("vista-adapter initSchedulingRepo async", sched.includes("export async function initSchedulingRepo"));
  check("vista-adapter SchedulingLockRepo iface", sched.includes("export interface SchedulingLockRepo"));
  check("vista-adapter initSchedulingLockRepo", sched.includes("export function initSchedulingLockRepo"));
  check("vista-adapter getRequestStore async", sched.includes("export async function getRequestStore"));
