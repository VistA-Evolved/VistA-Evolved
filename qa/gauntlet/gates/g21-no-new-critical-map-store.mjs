...
export async function run(opts = {}) {
  ...
  if (!baseline) {
    details.push(
      'FAIL: critical-map-baseline.json missing. Run: node qa/gauntlet/gates/g21-no-new-critical-map-store.mjs --update-baseline'
    );
  ...
  if (delta > 0) {
    ...
    const newStores = currentStores.filter((s) => !baselineSet.has(`${s.file}:${s.variable}`));
    ...
    details.push(
      'Register in store-policy.ts with migrationTarget, then update baseline: node qa/gauntlet/gates/g21-no-new-critical-map-store.mjs --update-baseline'
    );
    status = 'fail';
  } else if (delta < 0) {
