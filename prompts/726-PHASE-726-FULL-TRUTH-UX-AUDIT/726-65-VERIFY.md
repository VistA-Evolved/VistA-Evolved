# Phase 726-65 Verify - Exports Slice

## Goal

Verify `/cprs/admin/exports` against the live canonical stack and prove that the browser reflects real exports route truth in both authenticated and unauthenticated states.

## Verification Steps

1. Verify Docker/API health with `curl.exe -s http://127.0.0.1:3001/health` and `curl.exe -s http://127.0.0.1:3001/vista/ping`.
2. Log in as `PRO1234 / PRO1234!!` and fetch the live route set used by the exports page.
3. Browser-prove the authenticated page against the live exports state.
...
- Live route outputs for the exports endpoints used by the page.
- Authenticated browser proof for the exports sections.
