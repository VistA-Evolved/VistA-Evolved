# Phase 726-66 Verify - Denial Cases Slice

## Goal

Verify `/cprs/admin/denial-cases` against the live canonical stack and prove that the browser reflects real denial-case route truth in both authenticated and unauthenticated states.

## Verification Steps

1. Verify Docker/API health with `curl.exe -s http://127.0.0.1:3001/health` and `curl.exe -s http://127.0.0.1:3001/vista/ping`.
2. Log in as `PRO1234 / PRO1234!!` and fetch the live route set used by the denial-cases page.
3. Browser-prove the authenticated page against the live denial-case state.
...
- Live route outputs for the denial-case endpoints used by the page.
- Authenticated browser proof for the denial-case sections.
