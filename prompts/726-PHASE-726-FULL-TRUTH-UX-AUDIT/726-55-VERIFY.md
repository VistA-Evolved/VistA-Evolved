# Phase 726-55 Verify - Admin Queue Slice

## Verification Goal

Prove `/cprs/admin/queue` against the live canonical API, confirm authenticated and unauthenticated behavior for every browser-visible surface, and record only browser-backed truth.

## Verification Steps

1. Verify Docker, API, and VistA are reachable on the canonical VEHU stack.
2. Authenticate with `PRO1234 / PRO1234!!` and capture the live outputs for every queue route the page calls.
3. Capture the same routes unauthenticated and confirm the real auth failure contract.
4. Browser-prove `/cprs/admin/queue` in the authenticated session across each tab or workflow the page exposes.
...
1. Every browser-visible queue surface is corroborated against its live route contract.
2. Any unauthenticated queue load fails closed with truthful auth/load messaging.
