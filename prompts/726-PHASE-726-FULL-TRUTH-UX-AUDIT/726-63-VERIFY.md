# Phase 726-63 Verify - LOA Queue Slice

## Goal

Verify `/cprs/admin/loa-queue` against the live canonical stack and prove that the browser reflects real LOA queue route truth in both authenticated and unauthenticated states.

## Verification Steps

1. Verify Docker/API health with `curl.exe -s http://127.0.0.1:3001/health` and `curl.exe -s http://127.0.0.1:3001/vista/ping`.
2. Log in as `PRO1234 / PRO1234!!` and fetch the live route set used by the LOA queue page.
3. Browser-prove the authenticated page against the live LOA queue state.
...
- Live route outputs are captured for the LOA queue endpoints used by the page.
