# Phase 726-10 Implement - Modules Slice

## Goal

Browser-audit `/cprs/admin/modules` on the canonical VEHU stack, corroborate the page state against the live module-entitlement and capability routes, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API is healthy before browser work.
2. Inventory the frontend modules page and the backing admin module, capability, and adapter route families.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/modules` and inspect the visible tabs, tables, toggles, and forms.
5. Corroborate browser-visible state against the live admin routes used by the page.
6. Exercise at least one meaningful interaction such as tab switching, filtering, or entitlement/flag editing posture if it is safe and truthful to do so.
7. If a real truth defect appears, patch the smallest correct source surface.
8. Re-run the browser proof after any fix on the same canonical stack.
9. Update the Phase 726 browser audit artifact, runtime audit overrides, ops summary, and notion update only after clean live proof.
10. Regenerate `pnpm audit:ui-estate:runtime` and `pnpm audit:ui-estate:truth`.

## Files Touched

- `apps/web/src/app/cprs/admin/modules/page.tsx`
- `apps/api/src/routes/module-entitlement-routes.ts`
- `apps/api/src/modules/module-registry.ts`
- `apps/api/src/modules/capability-service.ts`
- `apps/api/src/platform/db/repo/module-repo.ts`
- `artifacts/phase726-p1-browser-control-audit.md`
- `data/ui-estate/runtime-ui-audit-overrides.json`
- `ops/summary.md`
- `ops/notion-update.json`