# Phase 726-12 Implement - Adapters Slice

## Goal

Browser-audit `/cprs/admin/adapters` on the canonical VEHU stack, corroborate the page against the live adapter-health and runtime-matrix routes, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API is healthy before browser work.
2. Inventory the adapters frontend page and backing route files.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/adapters` and inspect the visible tabs, counts, filters, and status tables.
5. Corroborate browser-visible state against the live adapter, runtime-matrix, and RPC coverage routes used by the page.
6. Exercise at least one meaningful interaction such as tab switching or RPC availability filtering.
