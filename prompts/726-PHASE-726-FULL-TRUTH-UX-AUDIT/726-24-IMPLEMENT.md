# Phase 726-24 Implement - PH HMO Console Slice

## Goal

Browser-audit `/cprs/admin/ph-hmo-console` on the canonical VEHU stack, corroborate the page against the live PH HMO registry routes, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API is healthy before browser work.
2. Inventory the PH HMO Console frontend page and backing PH HMO route files.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/ph-hmo-console` and inspect the Registry, Capabilities, Packets, and Validation tabs.
5. Corroborate browser-visible state against the live routes used by the page.
6. Exercise at least one meaningful low-risk interaction such as tab switching, search/filtering, opening detail, or generating a live packet.
