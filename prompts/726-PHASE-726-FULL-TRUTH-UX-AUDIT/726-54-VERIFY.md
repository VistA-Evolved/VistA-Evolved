# Phase 726-54 Verify - Admin RPC Debug Slice

## Verification Goal

Prove `/cprs/admin/rpc-debug` against the live canonical API, confirm authenticated and unauthenticated behavior for every browser-visible surface exercised in the slice, and record only browser-backed truth.

## Verification Steps

1. Verify Docker, API, and VistA are reachable on the canonical VEHU stack.
2. Authenticate with `PRO1234 / PRO1234!!` and capture the live outputs for the `/vista/rpc-debug/*` and `/vista/rpc-catalog` routes used by the page.
3. Capture the same endpoints unauthenticated and confirm the real auth failure contract.
4. Browser-prove `/cprs/admin/rpc-debug` in the authenticated session across the stat cards, filters, and action table exercised in the slice.
...
1. Every browser-visible rpc-debug surface exercised in the slice is corroborated against its live route contract.
2. Any unauthenticated rpc-debug load fails closed with truthful auth/load messaging.
