# Phase 726-47 Implement - Ops Slice

## Goal

Browser-audit `/cprs/admin/ops` on the canonical VEHU stack, verify the exact operations dashboard contract it uses, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API and VistA connectivity before browser work.
2. Inventory the ops frontend page and the backend routes it calls.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/ops` and inspect every tab, card, and workflow surfaced by the page.
5. Corroborate the browser-visible operations state against the live API routes used by the page.
