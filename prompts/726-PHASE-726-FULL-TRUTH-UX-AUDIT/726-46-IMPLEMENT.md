# Phase 726-46 Implement - Onboarding Slice

## Goal

Browser-audit `/cprs/admin/onboarding` on the canonical VEHU stack, verify the exact onboarding dashboard contract it uses, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API and VistA connectivity before browser work.
2. Inventory the onboarding frontend page and the backend routes it calls.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/onboarding` and inspect every tab, card, table, and workflow surfaced by the page.
5. Corroborate the browser-visible onboarding state against the live API routes used by the page.
