# Phase 726-15 Implement - Billing Slice

## Goal

Browser-audit `/cprs/admin/billing` on the canonical VEHU stack, corroborate the page against the live billing plans, subscription, usage, and health routes, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API is healthy before browser work.
2. Inventory the billing frontend page and backing billing route/provider files.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/billing` and inspect the visible tabs, cards, forms, and status regions.
5. Corroborate browser-visible state against the live billing plans, subscription, usage, and health routes used by the page.
6. Exercise at least one meaningful interaction such as tab switching or a safe subscription action if it is truthful and low-risk.
7. If a real truth defect appears, patch the smallest correct source surface.
8. Re-run the browser proof after any fix on the same canonical stack.
9. Update the Phase 726 browser audit artifact, runtime audit overrides, ops summary, and notion update only after clean live proof.
10. Regenerate `pnpm audit:ui-estate:runtime` and `pnpm audit:ui-estate:truth`.

## Files Touched

- `apps/web/src/app/cprs/admin/billing/page.tsx`
- `apps/api/src/billing/billing-routes.ts`
- `apps/api/src/billing/index.ts`
- `apps/api/src/billing/mock-provider.ts`
- `artifacts/phase726-p1-browser-control-audit.md`
- `data/ui-estate/runtime-ui-audit-overrides.json`
- `ops/summary.md`
- `ops/notion-update.json`