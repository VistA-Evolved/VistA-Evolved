# Phase 726-50 Implement - Admin RCM Slice

## Goal

Browser-audit `/cprs/admin/rcm` on the canonical VEHU stack, verify the exact RCM console contract it uses, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API and VistA connectivity before browser work.
2. Inventory the admin RCM frontend page and the backend routes it calls.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/rcm` and inspect every browser-visible tab, table, card, and workflow exposed by the page.
5. Corroborate the browser-visible RCM state against the live API routes used by the page.
