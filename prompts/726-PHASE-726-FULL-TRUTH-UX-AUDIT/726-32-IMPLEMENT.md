# Phase 726-32 Implement - PhilHealth Setup Slice

## Goal

Browser-audit `/cprs/admin/philhealth-setup` on the canonical VEHU stack, corroborate the page against the live PhilHealth facility setup routes, and fix only evidence-backed truth defects.

## Implementation Steps

1. Reconfirm the canonical API is healthy before browser work.
2. Inventory the philhealth-setup frontend page and backing route files.
3. Authenticate in the browser as `PRO1234 / PRO1234!!`.
4. Open `/cprs/admin/philhealth-setup` and inspect facility details, provider accreditations, and readiness checklist state.
5. Corroborate browser-visible state against the live `/rcm/philhealth/setup*` routes used by the page.
