# Phase 726-64 Verify - HMO Portal Slice

## Goal

Verify `/cprs/admin/hmo-portal` against the live canonical stack and prove that the browser reflects real HMO portal route truth in both authenticated and unauthenticated states.

## Verification Steps

1. Verify Docker/API health with `curl.exe -s http://127.0.0.1:3001/health` and `curl.exe -s http://127.0.0.1:3001/vista/ping`.
2. Log in as `PRO1234 / PRO1234!!` and fetch the live route set used by the HMO portal page.
3. Browser-prove the authenticated page against the live HMO portal state.
...
- Live route outputs are captured for the HMO portal endpoints used by the page.
