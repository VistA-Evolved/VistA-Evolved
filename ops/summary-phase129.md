...
5. **BUG-069** -- `registry.ts`: Made `payerId` optional in eligibility schema (cron sends minimal payload)
6. **BUG-070** -- `governance.ts`: Moved PHI check before zod parsing (zod strips unknown keys)
7. **BUG-071** -- `governance.ts`: Fixed JSONB parsing in `getRecentJobRuns` (PG driver returns object)
8. **BUG-072** -- `pg-migrate.ts`: Added DROP FUNCTION before CREATE OR REPLACE in v7
