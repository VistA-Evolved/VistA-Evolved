...
export default function (data) {
  ...
  group('auth', () => {
    ...
    if (!check(sessionRes, { 'session 200 or 401': (r) => r.status === 200 || r.status === 401 }))
      failedChecks.add(1);
  });
  ...
  group('clinical', () => {
    ...
    if (!check(patientList, { 'patient-list ok': (r) => r.status === 200 || r.status === 401 }))
      failedChecks.add(1);

    const allergies = http.get(`${BASE_URL}/vista/allergies?dfn=3`);
    clinicalLatency.add(allergies.timings.duration);
    if (!check(allergies, { 'allergies ok': (r) => r.status === 200 || r.status === 401 }))
      failedChecks.add(1);
  });
  ...
  group('admin', () => {
    ...
    if (!check(caps, { 'capabilities ok': (r) => r.status === 200 || r.status === 401 }))
      failedChecks.add(1);

    const modules = http.get(`${BASE_URL}/api/modules`);
    adminLatency.add(modules.timings.duration);
    if (!check(modules, { 'modules ok': (r) => r.status === 200 || r.status === 401 }))
      failedChecks.add(1);
  });
