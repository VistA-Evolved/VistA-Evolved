...
const ALLOWLIST: { file: RegExp; linePattern: RegExp; reason: string }[] = [
  ...
  { file: /nursing\/page/, linePattern: /setTimeout.*setShowCreate\(false\)/, reason: "Auto-close success modal after real API call" },
];
...
function isAllowlisted(relFile: string, lineText: string): boolean {
  return ALLOWLIST.some(
    (a) => a.file.test(relFile) && a.linePattern.test(lineText),
  );
}
