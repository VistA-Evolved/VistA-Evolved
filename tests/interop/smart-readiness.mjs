...
async function safeFetch(url, opts = {}) {
  try {
    ...
    return { status: 0, json: async () => ({}), text: async () => e.message, ok: false, headers: new Map() };
  }
}
...
async function testSmartConfiguration() {
  ...
  if (stu2Result.passed && stu2Result.body) {
    ...
    results.push(assert("authorization_endpoint present",
      !!cfg.authorization_endpoint,
      `Got: ${cfg.authorization_endpoint || "missing"}`));

    results.push(assert("token_endpoint present",
      !!cfg.token_endpoint,
      `Got: ${cfg.token_endpoint || "missing"}`));

    results.push(assert("capabilities array present",
    ...
    if (Array.isArray(cfg.capabilities)) {
      ...
      for (const cap of wantCaps) {
        results.push(assert(`capability: ${cap}`,
    ...
    // Scopes
    results.push(assert("scopes_supported present",
    ...
    if (Array.isArray(cfg.scopes_supported)) {
      ...
      for (const s of wantScopes) {
        ...
        results.push(assert(`scope: ${s}`, found,
  ...
  results.push(assert("SMART configuration (integration-pending)", false,
...
async function testOidcDiscovery() {
  ...
  if (result.passed && result.body) {
    ...
    results.push(assert("issuer present", !!cfg.issuer,
      `Got: ${cfg.issuer || "missing"}`));

    results.push(assert("jwks_uri present", !!cfg.jwks_uri,
      `Got: ${cfg.jwks_uri || "missing"}`));

    results.push(assert("response_types_supported includes code",
    ...
    results.push(assert("OIDC discovery (integration-pending)", false,
...
async function testTokenCors() {
  ...
  results.push(assert("Token OPTIONS returns CORS headers",
    allowOrigin === "*" || allowOrigin.includes("example.com") ||
...
async function testLaunchParams() {
  ...
  if (body && body.rest) {
    ...
    if (sec) {
      const oauthExt = sec.extension?.find(e =>
        e.url === "http://fhir-registry.smarthealthit.org/StructureDefinition/oauth-uris"
      );
      results.push(assert("CapabilityStatement has SMART oauth-uris extension",
        !!oauthExt,
      ...
      if (oauthExt?.extension) {
        ...
        results.push(assert("oauth-uris has authorize endpoint",
          !!authExt?.valueUri, `Got: ${authExt?.valueUri || "missing"}`));
        results.push(assert("oauth-uris has token endpoint",
          !!tokenExt?.valueUri, `Got: ${tokenExt?.valueUri || "missing"}`));
      }
    } else {
      results.push(assert("SMART security in CapabilityStatement (integration-pending)",
        false, "CapabilityStatement.rest[0].security not present"));
    }
  } else {
    results.push(assert("FHIR metadata for SMART (integration-pending)",
      false, "Cannot parse CapabilityStatement for SMART check"));
  }
}
