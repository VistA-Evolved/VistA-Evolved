...
async function testListPacks() {
  ...
  if (!result.passed || !result.body) {
    results.push(
      assert('HL7 pack list (integration-pending)', false, 'HL7 pack endpoints not reachable')
    );
    return [];
  }
  ...
  results.push(
    assert('At least 1 pack registered', packs.length >= 1, `Got ${packs.length} packs`)
  );
  ...
  for (const p of packs.slice(0, 5)) {
    ...
    results.push(
      assert(`Pack "${packId}" has id/name`, !!packId, `keys: ${Object.keys(p).join(',')}`)
    );
  }
...
async function testGetPack(packId) {
  ...
  if (result.passed && result.body) {
    ...
    results.push(
      assert(
        `Pack ${packId} has segment definitions or structure`,
        !!hasSegments,
...
async function testValidateMessage() {
  ...
  if (result.passed && result.body) {
    ...
    results.push(
      assert(
        'Validation returns valid/ok field',
        isValid !== undefined,
    ...
    if (validation.segments) {
      results.push(
        assert(
          'Validation returns parsed segments',
...
async function testSegmentOrdering() {
  ...
  if (body) {
    ...
    const orderingErrors = errors.filter(
      (e) => (typeof e === 'string' && /order|sequence/i.test(e)) || e?.type === 'ordering'
    );
    results.push(
      assert(
        'Well-ordered ADT has no ordering errors',
    ...
    results.push(
      assert('Segment ordering validation', false, 'Could not parse validation response')
    );
  }
