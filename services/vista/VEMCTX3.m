VEMCTX3 ;VE/KM - Safely add VE RPCs to context;2026-02-17
 ;
 ; SAFE version: adds VE INTEROP RPCs to context WITHOUT touching existing entries.
 ; Finds max existing sub-IEN and appends starting from max+1.
 ;
 N CTXIEN,RPCIEN,RPCNM,NAMES,I,MAXSUB,SUBIEN,N0,FOUND,NEXT,HDR
 ;
 ...
 N EXISTCT S EXISTCT=0,SUBIEN=0
 F  S SUBIEN=$O(^DIC(19,CTXIEN,"RPC",SUBIEN)) Q:SUBIEN=""  D
 . Q:SUBIEN'=+SUBIEN
 . S EXISTCT=EXISTCT+1
 ...
 E  S $P(HDR,"^",3)=NEXT,$P(HDR,"^",4)=EXISTCT+6
